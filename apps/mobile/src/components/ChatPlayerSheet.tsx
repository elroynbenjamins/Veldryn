...
export function ChatPlayerSheet({
 ...
 async function addFriend(){await runRelationship(async()=>{const result=await sendFriendRequest(message.account_id);if(result==='sent'||result==='already_pending')setRelationshipAndNotify({relationship:'outgoing_pending'});else if(result==='already_friends')setRelationshipAndNotify({relationship:'friend'});else await loadRelationship();});}
 function confirmRemoveFriend(){Alert.alert('Remove '+message.sender_name+'?','They will be removed from your Friends list. You can send a new request later.',[{text:'Cancel',style:'cancel'},{text:'Remove friend',style:'destructive',onPress:()=>void runRelationship(async()=>{await removeFriend(message.account_id);setRelationshipAndNotify({relationship:'none'});})}]);}
 ...
 function confirmBlock(){Alert.alert('Block '+message.sender_name+'?','Their messages will be hidden and they will be removed from your social lists.',[{text:'Cancel',style:'cancel'},{text:'Block',style:'destructive',onPress:async()=>{setBusy(true);try{await setPlayerBlocked(message.account_id,true);setRelationshipAndNotify({relationship:'none'});onBlocked(message.account_id);onClose();}catch(error){Alert.alert('Block player',error instanceof Error?error.message:'Unable to block player.');}finally{setBusy(false)}}}]);}
