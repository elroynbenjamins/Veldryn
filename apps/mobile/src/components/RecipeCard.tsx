...
export function RecipeCard({state,recipe,status,onCraft,onNavigate,onCraftPrerequisites}:{state:GameState;recipe:Recipe;status:ReturnType<typeof recipeAvailability>;onCraft:(id:string)=>void;onNavigate?:(destination:WorkingTowardDestination)=>void;onCraftPrerequisites?:(recipeId:string)=>void}){
 ...
 return <View style={s.card}>
  <Pressable accessibilityRole="button" accessibilityState={{expanded}} accessibilityLabel={recipe.name+(status.ready?', ready to craft':', requirements missing')} onPress={()=>setExpanded(v=>!v)} style={s.head}>
   <ItemArtwork itemId={output.id} size={48}/><View style={s.copy}><Text style={s.title}>{output.name}</Text><Text style={s.sub}>Makes {f(recipe.output.quantity)} · {recipe.skillId} Lv. {recipe.level}</Text><Text style={craftReady?s.ready:s.sub}>{craftReady?(timed?(willWait?'Ready to queue':'Ready to start'):'Ready to craft'):forgeFull?'Forge + backlog full':`${readyInputs}/${status.inputs.length} materials ready`}</Text></View><UiIcon name={expanded?'close':'next'} size={24}/>
  </Pressable>
