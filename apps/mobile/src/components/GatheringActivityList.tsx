...
export function GatheringActivityList({state,skillId,skillLevel,offlineHours,queueFull,onGather,onQueueGather}:{state:GameState;skillId:GatheringSkillId;skillLevel:number;offlineHours:number;queueFull:boolean;onGather:(id:string)=>void;onQueueGather:(id:string)=>void}){
 ...
 const regional=all.filter(row=>row.zoneId===region.id).map(activity=>({activity,view:metrics(state,activity,offlineHours)}));
 ...
 return <>
  ...
  {regional.map(({activity,view})=>{
   const active=state.activity?.targetId===activity.id,unlocked=skillLevel>=activity.unlockLevel,fastestXp=fastest?.activity.id===activity.id,resource=itemDef(activity.itemId),pacing=view.pacing;
   return <Panel key={activity.id} accentColor={active?C.good:fastestXp?C.info:undefined}>
    <View style={s.heading}><ResourceArtwork itemId={activity.itemId} size={58}/><View style={s.flex}><View style={s.nameRow}><Text style={s.name}>{activity.name}</Text>{fastestXp?<Text style={s.fastTag}>FASTEST XP</Text>:null}{!unlocked?<Text style={s.lockTag}>LV {activity.unlockLevel}</Text>:null}</View><Text style={s.sub}>{resource.name} · every {view.cycle.toFixed(1)}s</Text></View><View style={[s.tierBadge,pacing.atRecommendedTier?s.tierReady:s.tierSlow]}><Text style={s.tierText}>T{activity.recommendedToolTier}</Text></View></View>
...
function makeStyles(C:ThemeColors){return StyleSheet.create({
 ...
 heading:{minHeight:62,flexDirection:'row',alignItems:'center',gap:spacing.sm},nameRow:{flexDirection:'row',alignItems:'center',flexWrap:'wrap',gap:6},name:{...typography.title,color:C.text},sub:{...typography.caption,color:C.muted},fastTag:{fontSize:9,color:C.info,fontWeight:'900',letterSpacing:.6},lockTag:{fontSize:9,color:C.warning,fontWeight:'900',letterSpacing:.6},
 tierBadge:{minWidth:42,height:36,alignItems:'center',justifyContent:'center',borderWidth:1,borderRadius:8},tierReady:{borderColor:C.good,backgroundColor:C.goodSurface},tierSlow:{borderColor:C.warning,backgroundColor:C.warningSurface},tierText:{...typography.bodyStrong,color:C.text},
