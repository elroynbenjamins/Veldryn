...
import {noviceSetFor,noviceRecipeId} from '../src/content/novice-sets';
let state=createCharacter(newGame(0),'BASTION','Tester');
state={...state,character:{...state.character!,level:5,gold:1000},bank:{...state.bank,stacks:[{itemId:'COPPER_ORE',quantity:500},{itemId:'GREENWOOD_LOG',quantity:500},{itemId:'MOSS_FIBER',quantity:500}]}};
for(const slot of noviceSetFor('BASTION').slots)state=craftRecipe(state,noviceRecipeId('BASTION',slot));
