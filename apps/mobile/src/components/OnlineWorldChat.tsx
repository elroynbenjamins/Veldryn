...
export function OnlineWorldChat({playerName,language,embedded=false}:{playerName:string;language:Language;embedded?:boolean}){
 ...
 const send=async()=>{const body=text.trim();if(!body||busy)return;if(chatEmoteCount(body)>8){Alert.alert('World chat','Use at most 8 emotes in one message.');return}setBusy(true);try{await postWorldMessage(WORLD_CHANNELS[channel].id,body,playerName);setText('');await load();}catch(reason){Alert.alert('World chat',reason instanceof Error?reason.message:'Unable to send message.');}finally{setBusy(false)}};
 const content=<View style={s.root}>
  ...
  <ScrollView style={s.log} contentContainerStyle={s.logInner} keyboardShouldPersistTaps="handled">{rows.length?rows.map(row=><View key={row.id} style={s.messageRow}><Pressable accessibilityRole="button" accessibilityLabel={`Open ${row.sender_name}'s player profile`} onPress={()=>setSelected(row)} style={s.nameButton}><GuildTaggedPlayerName numberOfLines={1} style={s.name} name={row.sender_name} guildTag={row.guild_tag} tagColorId={row.guild_tag_color_id}/><Text style={s.profileMark}>›</Text></Pressable><Text style={s.time}>{new Date(row.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</Text><ChatMessageText body={row.body} mentionName={playerName}/></View>):<Text style={s.empty}>{ot(language,'chat.none')}</Text>}</ScrollView>
