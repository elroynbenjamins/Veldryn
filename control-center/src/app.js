...
function renderDashboard() {
  ...
  const playerHealth=playerEventHealthView(d.playerEventHealth),activity=d.playerActivity||null,lifecycle=d.playerLifecycle||null;
  const signupStage=lifecycleStage(lifecycle,'signup'),characterStage=lifecycleStage(lifecycle,'character'),storyStage=lifecycleStage(lifecycle,'story1'),dungeonStage=lifecycleStage(lifecycle,'dungeon');
  return shell(`
    ...
    ${lifecycle?`<div class="card" style="margin-top:14px"><div class="card-head"><div><h3>New-player activation · ${fmtNumber(lifecycle.windowDays)}d</h3><div class="tiny muted">Signup → early progression → first co-op dungeon</div></div><button class="btn btn-sm btn-ghost" data-nav="health">Open lifecycle analytics</button></div><div class="card-body"><div class="grid grid-4">
      <div class="preview-box"><div class="tiny muted">SIGNUPS</div><strong>${fmtNumber(signupStage.count)}</strong><div class="tiny faint">${fmtNumber(lifecycle.signupTypes?.anonymous)} guest/anonymous · ${fmtNumber(lifecycle.signupTypes?.registered)} registered</div></div>
      <div class="preview-box"><div class="tiny muted">CHARACTER CREATED</div><strong>${fmtPct(signupStage.count?characterStage.count/signupStage.count:null)}</strong><div class="tiny faint">${fmtNumber(characterStage.count)} accounts</div></div>
      <div class="preview-box"><div class="tiny muted">FIRST STORY CLAIM</div><strong>${fmtPct(signupStage.count?storyStage.count/signupStage.count:null)}</strong><div class="tiny faint">Onboarding proxy · ${fmtNumber(storyStage.count)} accounts</div></div>
      <div class="preview-box"><div class="tiny muted">FIRST DUNGEON</div><strong>${fmtPct(signupStage.count?dungeonStage.count/signupStage.count:null)}</strong><div class="tiny faint">${fmtNumber(dungeonStage.count)} accounts</div></div>
    </div><div class="tiny muted" style="margin-top:8px">Marketing campaign/referral attribution is not instrumented yet, so this starts at account signup rather than acquisition source.</div></div></div>`:''}
...
function fmtHours(value){return value===null||value===undefined||!Number.isFinite(Number(value))?'—':Number(value)<1?`${Math.round(Number(value)*60)}m`:`${Number(value).toFixed(Number(value)<10?1:0)}h`;}
function lifecycleStage(result,key){return (result?.stages||[]).find(row=>row.key===key)||{count:0,label:key};}
function regionLabel(regionId){return ({GREENFIELDS:'Greenfields',SILVERBROOK:'Silverbrook',IRONWOOD:'Ironwood Forest',OLD_MINES:'Old Mines',KINGS_ROAD:"King's Road",SUNSCAR:'Sunscar',FROSTMARCH:'Frostmarch',ASHLANDS:'Ashlands',UNKNOWN:'Unknown'})[regionId]||regionId;}
...
function renderHealth() {
  ...
  return shell(`
    ...
    ${lifecycle?`<div class="grid grid-4" style="margin-bottom:14px">
      <div class="card metric"><div class="label">D60 retention</div><div class="value">${fmtPct(longRetention.d60Retention)}</div><div class="foot">${fmtNumber(longRetention.d60Retained)} / ${fmtNumber(longRetention.d60Cohort)} matured cohort</div></div>
      <div class="card metric"><div class="label">D90 retention</div><div class="value">${fmtPct(longRetention.d90Retention)}</div><div class="foot">${fmtNumber(longRetention.d90Retained)} / ${fmtNumber(longRetention.d90Cohort)} matured cohort</div></div>
      <div class="card metric"><div class="label">Median signup → character</div><div class="value">${fmtHours(lifecycle.timing?.medianHoursToCharacter)}</div><div class="foot">Among signups that created one</div></div>
      <div class="card metric"><div class="label">Median signup → dungeon</div><div class="value">${fmtHours(lifecycle.timing?.medianHoursToFirstDungeon)}</div><div class="foot">Among signups that started co-op</div></div>
    </div>`:''}
    ...
    ${lifecycle?`<div class="card" style="margin-bottom:14px"><div class="card-head"><div><h3>New-player lifecycle funnel</h3><div class="tiny muted">Last ${fmtNumber(lifecycle.windowDays)} days · account-level aggregates only</div></div><div class="actions"><span class="pill">${fmtNumber(lifecycle.signupTypes?.anonymous)} guest</span><span class="pill">${fmtNumber(lifecycle.signupTypes?.registered)} registered</span></div></div><div class="table-wrap"><table><thead><tr><th>Stage</th><th>Accounts</th><th>% of signups</th></tr></thead><tbody>${lifecycleStages.map((row,index)=>{const signups=Number(lifecycleStages[0]?.count||0);return `<tr><td><strong>${h(row.label)}</strong>${row.key==='story1'?'<div class="tiny muted">Current onboarding-completion proxy</div>':''}</td><td>${fmtNumber(row.count)}</td><td>${index===0?'100%':fmtPct(signups?Number(row.count)/signups:null)}</td></tr>`}).join('')}</tbody></table></div><div class="card-body" style="padding-top:8px"><div class="tiny muted">Activity-ledger stages only become complete after analytics deployment. Campaign, ad, referral and store-source attribution is not yet persisted.</div></div></div>
    <div class="grid grid-2" style="margin-bottom:14px">
      <div class="card"><div class="card-head"><div><h3>Level progression reach</h3><div class="tiny muted">Active accounts with a character · last ${fmtNumber(activePop.windowDays||lifecycle.windowDays)}d</div></div><span class="pill">${fmtNumber(activePop.withCharacter)} characters</span></div><div class="table-wrap"><table><thead><tr><th>Reached</th><th>Accounts</th><th>Reach</th></tr></thead><tbody>${(activePop.levelReach||[]).map(row=>`<tr><td>Level ${fmtNumber(row.level)}+</td><td>${fmtNumber(row.count)}</td><td>${fmtPct(row.rate)}</td></tr>`).join('')}</tbody></table></div></div>
      <div class="card"><div class="card-head"><div><h3>Current region distribution</h3><div class="tiny muted">Where recently active accounts are currently saved</div></div><span class="pill">${fmtNumber(activePop.accounts)} active</span></div><div class="table-wrap"><table><thead><tr><th>Region</th><th>Accounts</th><th>Share</th></tr></thead><tbody>${(activePop.regions||[]).map(row=>`<tr><td><strong>${h(regionLabel(row.regionId))}</strong><div class="mono tiny faint">${h(row.regionId)}</div></td><td>${fmtNumber(row.count)}</td><td>${fmtPct(activePop.accounts?Number(row.count)/Number(activePop.accounts):null)}</td></tr>`).join('')}</tbody></table></div></div>
    </div>
    <div class="grid grid-2" style="margin-bottom:14px">
      <div class="card"><div class="card-head"><h3>Level bands</h3><span class="pill">Current progression</span></div><div class="table-wrap"><table><thead><tr><th>Band</th><th>Accounts</th></tr></thead><tbody>${(activePop.levelBands||[]).map(row=>`<tr><td>${h(row.band)}</td><td>${fmtNumber(row.count)}</td></tr>`).join('')}</tbody></table></div></div>
