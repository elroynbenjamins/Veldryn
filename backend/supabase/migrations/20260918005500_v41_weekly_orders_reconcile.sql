-- VELDRYN V41 reconciliation: account-wide Weekly Hunt / Profession Orders.
-- Monday 00:00 UTC rollover, server-owned progress and idempotent reward receipts.
-- Does not change Offline Reserve, Parties, Contracts, Guild Projects or Live Dungeons.
begin;
...
create table if not exists private.v41_weekly_order_accounts(
  ...
  check((state->>'schemaVersion')::integer=41),
  check(ends_at>starts_at)
);
create table if not exists private.v41_weekly_order_progress_receipts(
  account_id uuid not null references private.v41_weekly_order_accounts(account_id) on delete cascade,
...
create table if not exists private.v41_weekly_order_claims(
  ...
  primary key(account_id,claim_key)
);
