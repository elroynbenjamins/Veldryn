...
type NonBossKind=Exclude<ExpeditionNodeType,'boss'>;
type ContentPools=Record<NonBossKind,readonly string[]>;

const ROOTBOUND:ContentPools={
...
function themed(prefix:string):ContentPools{
  const ids=(kind:string)=>[1,2,3].map(index=>`${prefix}_${kind}_${String(index).padStart(2,'0')}`);
...
export const COOP_ROUTE_CONTENT:Readonly<Record<string,ContentPools>>=Object.freeze({
...
export function coopContentPool(expeditionId:string,kind:NonBossKind):readonly string[]{
  const pool=COOP_ROUTE_CONTENT[expeditionId]?.[kind];if(!pool||pool.length<3||new Set(pool).size!==pool.length)throw new Error('invalid_expedition_content_pool');return pool;
}
