...
export function QuestScreen({state,onClaim,onClaimContract,onNavigate,onOpenWeeklyOrder,onPinWeeklyOrder,onQueueWeeklyOrder,onStopWeeklyOrder}:{state:GameState;onClaim:(id:string)=>void;onClaimContract:(period:SeasonalPeriod,id:string)=>void;onNavigate:(destination:QuestDestination)=>void;onOpenWeeklyOrder:(order:WeeklyOrder)=>void;onPinWeeklyOrder:(order:WeeklyOrder)=>void;onQueueWeeklyOrder:(order:WeeklyOrder)=>void;onStopWeeklyOrder:(order:WeeklyOrder)=>void}){
  ...
  return <ScrollView contentContainerStyle={s.root} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
    ...
    {fallenKnightWeekly&&<Panel><View style={s.contractHead}><Text style={s.seasonName}>Break the Last Oath</Text><View style={[s.rarity,{borderColor:fallenKnightWeekly.bountyAwarded?C.good:C.warning}]}><Text style={[s.rarityText,{color:fallenKnightWeekly.bountyAwarded?C.good:C.warning}]}>WEEKLY BOSS</Text></View></View><Text style={s.sub}>Defeat the Fallen Knight once after the story clear. That victory secures the weekly Oathglass Bounty cache.</Text><StatBar reduceMotion={state.settings.reduceMotion} label="Fallen Knight weekly bounty" current={Math.min(fallenKnightWeekly.rewardedClears,fallenKnightWeekly.bountyTarget)} max={fallenKnightWeekly.bountyTarget}/><Text style={fallenKnightWeekly.bountyAwarded?s.claimed:s.contractHint}>{fallenKnightWeekly.bountyAwarded?'✓ Oathglass Bounty secured':`${fallenKnightWeekly.bountyTarget-fallenKnightWeekly.rewardedClears} victory${fallenKnightWeekly.bountyTarget-fallenKnightWeekly.rewardedClears===1?'':'ies'} remaining · 1 Tempering Core + 1 Oathglass Fragment + bonus boss materials`}</Text><Text style={s.contractHint}>Weekly boss rewards · {fallenKnightWeekly.rewardedClears}/{fallenKnightWeekly.cap} clears · {fallenKnightWeekly.remaining} remaining</Text><GameButton compact title="Open Fallen Knight" tone="secondary" onPress={()=>onNavigate({tab:'World',zoneId:'KINGS_ROAD',label:"King's Road",hint:'Open the Fallen Knight encounter from the current region.'})}/></Panel>}
