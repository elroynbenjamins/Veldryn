...
export interface CoopRewardEntitlement {id:string;runId:string;mode:CoopMode;recipientAccountId:string;characterId?:string;kind:'participant'|'echo_assistance';rewardStage:string;tier:ExpeditionTier;mapBaseMarks:number;state:RewardState;periodDateKey:string;periodWeekKey:string;earnedAtMs:number;claimed:boolean;rewardMarks?:number;}
export class MemoryRewardIntegrityRepository{
 entitlements=new Map<string,CoopRewardEntitlement>();wallets=new Map<string,number>();daily=new Map<string,{enhanced:number;echo:number}>();weekly=new Map<string,{enhanced:number;echo:number}>();receipts=new Map<string,{entitlementId:string;marks:number}>();purchases=new Map<string,{accountId:string;cost:number}>();
...
export class CoopRewardService{
 ...
 claim(accountId:string,entitlementId:string,requestId:string):{marks:number;idempotentReplay:boolean}{
  const receiptKey=`${accountId}:${requestId}`;const prior=this.repository.receipts.get(receiptKey);if(prior){if(prior.entitlementId!==entitlementId)throw new Error('idempotency_conflict');return{marks:prior.marks,idempotentReplay:true};}
  ...
  if(entitlement.claimed){const marks=entitlement.rewardMarks??0;this.repository.receipts.set(receiptKey,{entitlementId,marks});return{marks,idempotentReplay:true};}
  const dailyKey=`${accountId}:${entitlement.periodDateKey}`,weeklyKey=`${accountId}:${entitlement.periodWeekKey}`;const daily=this.repository.daily.get(dailyKey)??{enhanced:0,echo:0},weekly=this.repository.weekly.get(weeklyKey)??{enhanced:0,echo:0};let marks=0;
  ...
  entitlement.claimed=true;entitlement.rewardMarks=marks;this.repository.daily.set(dailyKey,daily);this.repository.weekly.set(weeklyKey,weekly);this.repository.wallets.set(accountId,(this.repository.wallets.get(accountId)??0)+marks);this.repository.receipts.set(receiptKey,{entitlementId,marks});return{marks,idempotentReplay:false};
 }
