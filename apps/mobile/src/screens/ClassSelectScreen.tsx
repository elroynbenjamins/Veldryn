...
export function ClassSelectScreen({language='en',onLanguage,onSelect,onCancel,cancelLabel,onExistingAccount}:{language?:GameState['settings']['language'];onLanguage?:(language:GameState['settings']['language'])=>void;onSelect:(id:ClassId,name:string,body:BodyPresentation)=>Promise<void>|void;onCancel?:()=>void;cancelLabel?:string;onExistingAccount?:()=>Promise<void>|void}){
 ...
 return <KeyboardAvoidingView style={s.screen} behavior={Platform.OS==='ios'?'padding':'height'}>
  ...
  <View style={s.footer}>
   ...
   {step==='class'&&onExistingAccount?<Pressable accessibilityRole="button" disabled={saving} onPress={()=>void onExistingAccount()} style={({pressed})=>[s.existingAccount,pressed&&{opacity:.7}]}><Text style={s.existingAccountText}>Already played VELDRYN? Sign in</Text></Pressable>:null}
  </View>
