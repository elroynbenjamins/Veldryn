...
export function ExplorationPanel({state,onCommand}:{state:GameState;onCommand:(command:GameCommand)=>Promise<void>}){
 ...
 return <Panel>
  <View style={s.heading}><View style={s.flex}><Text style={s.eyebrow}>SCOUTING ROUTES</Text><Text style={s.title}>Explore the world</Text><Text style={s.body}>Scouting is the fastest way to train Exploration. Complete regional surveys to reveal encounters and discover routes into later regions. Combat also grants a small amount of passive Exploration XP.</Text></View><View style={s.regionBadge}><Text style={s.regionBadgeLabel}>CURRENT</Text><Text numberOfLines={2} style={s.regionBadgeValue}>{region?.name??regionId}</Text></View></View>
  {routes.map(route=>{const zone=WORLD_ZONES.find(item=>item.id===route.zoneId),nextZone=route.unlockRegionId?WORLD_ZONES.find(item=>item.id===route.unlockRegionId):undefined,here=regionId===route.zoneId,levelReady=c.level>=route.requiredLevel,ready=here&&levelReady,monsterFound=!route.unlockMonsterId||state.unlockedMonsterIds.includes(route.unlockMonsterId),regionFound=!route.unlockRegionId||(state.discoveredRegionIds??['GREENFIELDS']).includes(route.unlockRegionId),discovered=monsterFound&&regionFound;const stateLabel=discovered?'DISCOVERED':!levelReady?`CHAR LV ${route.requiredLevel}`:!here?'TRAVEL':'READY';return <View key={route.id} style={[s.route,ready&&s.routeReady]}>
   <View style={s.routeTop}><View style={s.flex}><View style={s.inline}><Text style={s.routeName}>{route.name}</Text><Text style={[s.tag,ready?s.tagReady:discovered?s.tagFound:s.tagLocked]}>{stateLabel}</Text></View><Text style={s.routeMeta}>{zone?.name??route.zoneId} · {route.seconds}s · {route.xp} Exploration XP</Text>{route.unlockMonsterId?<Text style={s.reward}>{monsterFound?'Encounter revealed':'Reveals a regional encounter'}</Text>:null}{nextZone?<Text style={s.reward}>{regionFound?`${nextZone.name} route discovered`:`Discovers the route to ${nextZone.name}`}</Text>:null}</View></View>
   <GameButton compact title={discovered&&ready?'Scout again':ready?'Scout':!levelReady?`Requires character level ${route.requiredLevel}`:`Travel to ${zone?.name??route.zoneId}`} disabled={!ready} tone={ready?'primary':'secondary'} onPress={()=>void onCommand({type:'explore',args:{id:route.id}})}/>
  </View>})}
 </Panel>;
}
