...
export function WorldScreen({state,onTravel,onOpenCombat,onOpenSkills,onCoop,onRegionalRewardsChanged,onOpenWeeklyOrder,onOpenContracts,goalRegionId}:Props){
  ...
  return <ScrollView contentContainerStyle={s.root}>
    ...
    <View style={s.unlockCard}><View style={s.unlockHead}><View style={s.flex}><Text style={s.unlockLabel}>{next?'NEXT REGION UNLOCK':'REGION PROGRESSION'}</Text><Text style={s.unlockTitle}>{next?next.name:'All authored regions unlocked'}</Text></View>{next?<Text style={s.unlockLevel}>Lv {level}/{next.minLevel}</Text>:<Text style={s.unlockDone}>COMPLETE</Text>}</View>{next?<><View style={s.unlockTrack}><View style={[s.unlockFill,{width:(nextUnlockProgress+'%') as any}]}/></View><Text style={s.unlockMeta}>{nextLockReason}</Text></>:<Text style={s.unlockMeta}>Every currently authored region can be travelled to.</Text>}</View>
    {travelRegions.map(zone=>{
      const availability=regionTravelAvailability(state,zone),unlocked=availability==='available',inDevelopment=availability==='inDevelopment',summary=regionActivitySummary(state,zone.id),goalTarget=goalRegionId===zone.id,lockReason=regionTravelLockReason(state,zone);
