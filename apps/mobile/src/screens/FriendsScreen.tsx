...
export function FriendsScreen(){
 const T=useGameTheme(),s=useMemo(()=>makeStyles(T),[T]);
  ...
  if(initialLoading)return <View style={s.root}><Text style={s.heading}>Friends</Text><ActivityIndicator accessibilityLabel="Loading friends" color={T.accent}/></View>;
  ...
  return <ScrollView contentContainerStyle={s.root} keyboardShouldPersistTaps="handled">
    ...
    {section==='Find'&&<Panel><Text style={s.title}>Find players</Text><Text style={s.sub}>Search by the beginning of a player’s display name.</Text><View style={s.searchRow}><SearchField accessibilityLabel="Player name" value={query} onChangeText={changeQuery} onSubmitEditing={()=>void search()} maxLength={20} autoCapitalize="none" placeholder="Player name" placeholderTextColor={T.muted} style={s.flex}/><GameButton title="Search" disabled={busy||query.trim().length<2} onPress={()=>void search()}/></View>
...
function Avatar({name}:{name:string}){
 const T=useGameTheme(),s=useMemo(()=>makeStyles(T),[T]);return <View style={s.avatar}><Text style={s.avatarText}>{name.trim().slice(0,1).toUpperCase()||'?'}</Text></View>}
...
function TabChip({label,selected,onPress}:{label:string;selected:boolean;onPress:()=>void}){
 const T=useGameTheme(),s=useMemo(()=>makeStyles(T),[T]);return <Pressable accessibilityRole="tab" accessibilityState={{selected}} onPress={onPress} style={({pressed})=>[s.tabChip,selected&&s.tabChipSelected,pressed&&s.pressed]}><Text style={[s.tabText,selected&&s.tabTextSelected]}>{label}</Text></Pressable>}

const makeStyles=(T:ThemePalette)=>StyleSheet.create({root:{padding:spacing.lg,gap:spacing.md},header:{flexDirection:'row',alignItems:'center',gap:spacing.md},sectionTabs:{flexDirection:'row',flexWrap:'wrap',gap:6},tabChip:{minHeight:40,paddingHorizontal:13,justifyContent:'center',borderWidth:1,borderColor:T.line,borderRadius:99,backgroundColor:T.bg},tabChipSelected:{borderColor:T.action,backgroundColor:T.actionSurface},tabText:{fontSize:12,fontWeight:'700',color:T.muted},tabTextSelected:{color:T.actionText},pressed:{opacity:.76},heading:{...typography.hero,color:T.text},title:{...typography.title,color:T.text},sub:{...typography.body,color:T.muted},errorCard:{gap:4,padding:spacing.md,borderWidth:1,borderColor:T.bad,borderRadius:radii.md,backgroundColor:T.dark?'#321B22':'#FDECEE'},errorLabel:{...typography.caption,color:T.bad,fontWeight:'900',letterSpacing:1},error:{...typography.body,color:T.text},searchRow:{flexDirection:'row',alignItems:'center',gap:spacing.sm,marginTop:spacing.sm},input:{flex:1,minHeight:48,borderWidth:1,borderColor:T.line,borderRadius:radii.md,backgroundColor:T.panel2,color:T.text,paddingHorizontal:spacing.md,fontSize:16},row:{flexDirection:'row',alignItems:'center',gap:spacing.sm,borderTopWidth:1,borderTopColor:T.line,paddingVertical:spacing.sm},avatar:{width:42,height:42,borderRadius:21,borderWidth:1,borderColor:T.accent,backgroundColor:T.panel2,alignItems:'center',justifyContent:'center'},avatarText:{...typography.title,color:T.accent},flex:{flex:1,minWidth:0},name:{...typography.bodyStrong,color:T.text},status:{...typography.caption,color:T.accent,maxWidth:88,textAlign:'right'},empty:{...typography.body,color:T.muted,paddingVertical:spacing.md,textAlign:'center'},miniActions:{gap:spacing.xs},titleText:{...typography.caption,color:T.accent},manage:{width:48,height:48,alignItems:'center',justifyContent:'center'},manageText:{fontSize:22,color:T.accent,fontWeight:'900'}});
