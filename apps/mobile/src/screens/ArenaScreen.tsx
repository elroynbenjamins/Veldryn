...
export function ArenaScreen({state,onChange}:{state:GameState;onChange:(next:GameState)=>void}){
 const {colors:C,equipmentColors}=useTheme();const s=useMemo(()=>createStyles(C,equipmentColors),[C,equipmentColors]);
  ...
  return <ScrollView contentContainerStyle={s.root}>
    ...
    <Panel><Text style={s.title}>Build your squad</Text><Text style={s.copy}>Choose three characters from this account for Front, Middle, and Back. Any role mix is valid. Ranked matches remain disabled until the authenticated Arena service is enabled.</Text><View style={[s.statusPill,status.ready?s.statusReady:s.statusWarning]}><Text style={status.ready?s.good:s.warning}>{status.ready?'Formation ready':status.reason}</Text></View><Text style={s.note}>{status.selectedCount}/3 selected · minimum level {ARENA_MIN_LEVEL}</Text></Panel>
    {ARENA_POSITIONS.map((position,index)=>{const selected=entries.find(entry=>entry.character.id===status.ids[index]);return <Panel key={position}><Text style={s.position}>{index+1} · {position.toUpperCase()}</Text>{selected?<><Text style={s.member}>{selected.character.name}</Text><Text style={s.note}>{CLASSES.find(def=>def.id===selected.character.classId)?.name} · Level {selected.character.level}</Text><GameButton title="Clear slot" tone="secondary" onPress={()=>onChange(setArenaSquadSlot(state,index as 0|1|2))}/></>:<Text style={s.note}>No character selected.</Text>}<View style={s.choices}>{entries.map(entry=>{const used=status.ids.includes(entry.character.id)&&entry.character.id!==selected?.character.id;const low=entry.character.level<ARENA_MIN_LEVEL;return <ArenaChoice key={entry.character.id} label={`${entry.character.name} · Lv ${entry.character.level}`} selected={entry.character.id===selected?.character.id} disabled={used||low} onPress={()=>onChange(setArenaSquadSlot(state,index as 0|1|2,entry.character.id))}/>})}</View></Panel>})}
