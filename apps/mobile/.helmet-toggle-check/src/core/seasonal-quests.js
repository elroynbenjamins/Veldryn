...
function periodKey(period, date = new Date()) { if (period === 'weekly') {
    const copy = new Date(date);
    copy.setHours(0, 0, 0, 0);
    copy.setDate(copy.getDate() - copy.getDay() + 1);
    return `${copy.getFullYear()}-W${Math.ceil(copy.getDate() / 7)}`;
} return `${date.getFullYear()}-${date.getMonth() + 1}`; }
function pick(list, seed, index) { let hash = 0; for (const char of `${seed}:${index}`)
    hash = (hash * 31 + char.charCodeAt(0)) >>> 0; return list[hash % list.length]; }
function seasonalQuestBoard(state, period, date = new Date()) {
    ...
    const key = periodKey(period, date), count = period === 'weekly' ? 3 : 4;
    const role = roleTemplates[classDef.role];
    const specific = classTemplates[character.classId] ?? [];
    const rows = [...specific, ...Array.from({ length: count }, (_, index) => pick(role, key, index))].slice(0, count);
    return rows.map((row, index) => {
        const [name, description, base, tag] = row;
        const multiplier = period === 'monthly' ? 2 : 1;
        const required = typeof base === 'number' ? base * multiplier : base;
        let progress = 0;
        if (tag === 'combat')
            progress = character.xp > 0 ? Math.min(required, Math.floor(character.xp / 80)) : 0;
        if (tag === 'gathering')
            progress = Math.min(required, state.skills.reduce((sum, skill) => sum + skill.level, 0));
        if (tag === 'crafting')
            progress = Math.min(required, Object.values(character.equipment).filter(Boolean).length + ((character.craftedNoviceItemIds ?? []).length));
        if (tag === 'exploration')
            progress = Math.min(required, state.unlockedMonsterIds.length);
