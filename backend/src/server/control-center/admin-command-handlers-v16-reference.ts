import type { AdminCommandHandler, AdminCommandResult, AdminCommandRow } from './admin-command-worker-v17_3';
import { activateRegionContentVersionV21, type RegionActivationStoreV21 } from '../content/region-content-admin-v21';

export interface MutationReceipt<T=Record<string,unknown>> { before?:T; after?:T; detail?:Record<string,unknown>; }
export interface VeldrynAdminDomain {
  forceLogout(accountId:string,idem:string):Promise<void>;
  scheduleAccountDeletion(accountId:string,graceHours:number,idem:string):Promise<MutationReceipt>;
  cancelAccountDeletion(accountId:string,idem:string):Promise<MutationReceipt>;
  muteChat(accountId:string,durationMinutes:number,code:string,idem:string):Promise<MutationReceipt>;
  unmuteChat(accountId:string,idem:string):Promise<MutationReceipt>;
  hideChatMessage(messageId:string,code:string,idem:string):Promise<MutationReceipt>;
  restoreChatMessage(messageId:string,idem:string):Promise<MutationReceipt>;
  suspendAccount(accountId:string,durationMinutes:number,code:string,idem:string):Promise<MutationReceipt>;
  unsuspendAccount(accountId:string,idem:string):Promise<MutationReceipt>;
  renameCharacter(characterId:string,newName:string,idem:string):Promise<MutationReceipt<{name?:string}>>;
  adjustGold(target:{accountId?:string;characterId?:string},amount:number,idem:string):Promise<MutationReceipt<{balance?:number}>>;
  setGold(target:{accountId?:string;characterId?:string},balance:number,idem:string):Promise<MutationReceipt<{balance?:number}>>;
  adjustCurrency(accountId:string,currencyId:string,amount:number,idem:string):Promise<MutationReceipt<{balance?:number}>>;
  setCurrency(accountId:string,currencyId:string,balance:number,idem:string):Promise<MutationReceipt<{balance?:number}>>;
  adjustPremiumCurrency(accountId:string,currencyId:string,amount:number,externalReference:string,idem:string):Promise<MutationReceipt<{balance?:number}>>;
  adjustItem(characterId:string,itemId:string,quantityDelta:number,bound:boolean,idem:string):Promise<MutationReceipt<{quantity?:number}>>;
  setItemQuantity(characterId:string,itemId:string,quantity:number,bound:boolean,idem:string):Promise<MutationReceipt<{quantity?:number}>>;
  grantRewardBundle(accountId:string,bundleId:string,quantity:number,idem:string):Promise<MutationReceipt>;
  reissueRewardClaim(accountId:string,claimId:string,idem:string):Promise<MutationReceipt>;
  adjustSkillXp(characterId:string,skillId:string,xpDelta:number,idem:string):Promise<MutationReceipt<{xp?:number;level?:number}>>;
  setSkillXp(characterId:string,skillId:string,xp:number,idem:string):Promise<MutationReceipt<{xp?:number;level?:number}>>;
  recalculateProgression(accountId:string,idem:string):Promise<MutationReceipt>;
  cancelStuckActivity(characterId:string,activityId:string,idem:string):Promise<MutationReceipt>;
  grantEntitlementOverride(accountId:string,entitlementId:string,expiresAt:string|null,externalReference:string,idem:string):Promise<MutationReceipt>;
  revokeEntitlementOverride(accountId:string,entitlementId:string,idem:string):Promise<MutationReceipt>;
  grantCollectible(target:{accountId?:string;characterId?:string},type:string,id:string,idem:string):Promise<MutationReceipt>;
  revokeCollectible(target:{accountId?:string;characterId?:string},type:string,id:string,idem:string):Promise<MutationReceipt>;
  grantCompanion(accountId:string,companionId:string,idem:string):Promise<MutationReceipt>;
  revokeCompanion(accountId:string,instanceId:string,idem:string):Promise<MutationReceipt>;
  adjustCompanionXp(accountId:string,instanceId:string,xpDelta:number,idem:string):Promise<MutationReceipt<{xp?:number;level?:number}>>;
  setCompanionXp(accountId:string,instanceId:string,xp:number,idem:string):Promise<MutationReceipt<{xp?:number;level?:number}>>;
  removePartyMember(accountId:string,partyId:string,idem:string):Promise<MutationReceipt>;
  disbandParty(partyId:string,idem:string):Promise<MutationReceipt>;
  renameParty(partyId:string,newName:string,idem:string):Promise<MutationReceipt<{name?:string}>>;
  removeGuildMember(accountId:string,guildId:string,idem:string):Promise<MutationReceipt>;
  transferGuildLeader(accountId:string,guildId:string,idem:string):Promise<MutationReceipt>;
  disbandGuild(guildId:string,idem:string):Promise<MutationReceipt>;
  renameGuild(guildId:string,newName:string,idem:string):Promise<MutationReceipt<{name?:string}>>;
  cancelDungeonRun(target:{accountId?:string;characterId?:string},runId:string,refundEntry:boolean,idem:string):Promise<MutationReceipt>;
  releaseDungeonQueue(accountId:string,queueId:string,idem:string):Promise<MutationReceipt>;
  syncAdminContentCatalog(idem:string):Promise<MutationReceipt>;
  regionContent?:RegionActivationStoreV21;
}

const req=(c:AdminCommandRow,key:string)=>{const v=c.parameters_json[key]; if(v===undefined||v===null||v==='')throw new Error(`invalid_${key}`); return v;};
const account=(c:AdminCommandRow)=>{if(!c.target_account_id)throw new Error('invalid_target_account');return c.target_account_id;};
const character=(c:AdminCommandRow)=>{if(!c.target_character_id)throw new Error('invalid_target_character');return c.target_character_id;};
const target=(c:AdminCommandRow)=>({...(c.target_account_id?{accountId:c.target_account_id}:{}),...(c.target_character_id?{characterId:c.target_character_id}:{})});
const out=(summary:string,r:MutationReceipt,reversal?:AdminCommandResult['reversal']):AdminCommandResult=>({summary,detail:{before:r.before??null,after:r.after??null,...(r.detail??{})},...(reversal?{reversal}:{})});

export function buildVeldrynAdminHandlers(domain:VeldrynAdminDomain):Record<string,AdminCommandHandler>{
  const handlers:Record<string,AdminCommandHandler> = {
  ...
  if(domain.regionContent){
    ...
    const activate:AdminCommandHandler=async(c,x)=>{
      const regionId=String(req(c,'regionId')),contentVersion=String(req(c,'contentVersion'));
      const result=await activateRegionContentVersionV21(regionContent,{regionId,contentVersion});
