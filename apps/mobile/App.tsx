...
function VeldrynApp(){
  ...
  useEffect(()=>{if(!masteryNotices.length||collected||forgeResults)return;const timer=setTimeout(()=>setMasteryNotices([]),4500);return()=>clearTimeout(timer)},[masteryNotices,collected,forgeResults]);
  ...
  useEffect(()=>{const subscription=AppState.addEventListener('change',nextStatus=>{if(serverGameplayEnabled)return;const wasAway=appStateRef.current==='background'||appStateRef.current==='inactive';appStateRef.current=nextStatus;if(!wasAway||nextStatus!=='active'||settlingRef.current)return;const current=stateRef.current;if(!current?.activity)return;const settled=settleStartupActivity(current,Date.now());if(!settled.reward)return;settlingRef.current=true;stateRef.current=settled.state;setState(settled.state);setCollected({reward:settled.reward,activity:settled.activity,welcomeBack:true,progressionMoments:rewardProgressionMoments(current,settled.state)});void repo.save(settled.state).catch(()=>Alert.alert('Save pending','Your returned rewards are safe in memory, but could not be saved yet.')).finally(()=>{settlingRef.current=false})});return()=>subscription.remove()},[]);
  ...
  async function perform(command?:GameCommand){try{const before=stateRef.current,result=await online.execute(command);stateRef.current=result.state;setState(result.state);queueCustomizationUnlocks(before,result.state);if(result.reward)presentCollected(result.reward,result.activity??null,before,result.state);else queueMasteryNotice(before,result.state);return result;}catch(error){Alert.alert('Online action',error instanceof Error?error.message:'Please retry.');return null;}}
...
const next=discoverCharacterSkins(candidate);stateRef.current=next;setState(next);queueCustomizationUnlocks(current,next);try{await repo.save(next)}catch{Alert.alert('Local save failed','Progress is still in memory. Keep the app open and try another action to save again.')}}
  ...
  {!collected&&!forgeResults?.some(row=>row.qualityProc)&&masteryNotices.length?<View style={s.masteryNotice}><ActionFeedback message={masteryRankNoticeMessage(masteryNotices)} tone={masteryNotices.some(moment=>moment.mastered)?'success':'info'} reduceMotion={state.settings.reduceMotion} compact/></View>:null}
