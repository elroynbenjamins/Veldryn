...
}
{
 ...
 throws(()=>rerollAccountCharacter(state,state.character!.id,'HEXWEAVER','Nova','female','rowan',2000),'confirmation is case-sensitive');
 equal(JSON.stringify(state),before,'failed confirmation leaves state untouched');
}
{
 const state=two();
 state.character!.activityQueue=[{kind:'combat',targetId:'MOSS_RAT'}];
...
}
{
 ...
 state.character!.gearEnhancements={[weapon]:{rank:0,failures:0,statGemId:'WARD_SHARD',gemIds:['WARD_SHARD']}};
...
}
{
 ...
 const inactive=state.otherCharacters![0].character;
 const deleted=deleteAccountCharacter(state,inactive.id,inactive.name,2000);
