...
export interface NoviceSetDef {id:string;classId:ClassId;name:string;weaponName:string;weaponAttack:number;offhandName?:string;slots:GearSlot[];setBonus:{name:string;attack:number;defense:number;hp:number;description:string};theme:{accent:string;identity:string;material:string};}
