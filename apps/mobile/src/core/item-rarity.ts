...
export interface GearRarityMeta{id:ItemRarity;label:string;chance:number;color:string;lightTextColor:string;surface:string;statMultiplier:number;borderWidth:number;glowOpacity:number;symbol:string;}
...
export function rarityNameColor(rarity:ItemRarity,darkTheme:boolean,fallback:string){const meta=rarityMeta(rarity);return rarity==='common'?fallback:darkTheme?meta.color:meta.lightTextColor;}
