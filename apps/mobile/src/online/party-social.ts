...
export const partySocialRepository:PartySocialRepository={
 ...
 browseRecruitment:async filters=>(await rpc<RecruitmentRow[]>('browse_recruitment_v16',{p_filters:filters})).map(card),
 publishRecruitment:async input=>card(await rpc<RecruitmentRow>('publish_recruitment_post_v16',{
...
export async function ownRecruitmentPosts(){const identity=await partySocialIdentity();if(!identity)return [];const {data,error}=await client().from('recruitment_posts').select('*').eq('owner_account_id',identity.accountId).order('created_at',{ascending:false}).limit(20);if(error)throw error;return (data as RecruitmentRow[]).map(card);}
...
export type PartyChatMessage={id:string;account_id:string;sender_name:string;body:string;created_at:string};
export async function partyChatMessages(id:string){const {data,error}=await client().from('chat_messages').select('id,account_id,sender_name,body,created_at').eq('channel_type','party').eq('channel_id',id).order('created_at',{ascending:false}).limit(50);if(error)throw error;return (data??[]).reverse() as PartyChatMessage[];}
