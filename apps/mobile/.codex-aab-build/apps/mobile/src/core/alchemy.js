...
exports.storedQuantity = storedQuantity;
exports.alchemyAvailability = alchemyAvailability;
exports.startAlchemyBatch = startAlchemyBatch;
exports.previewAlchemyReward = previewAlchemyReward;
exports.alchemyRefund = alchemyRefund;
exports.normalizeAlchemyBatch = normalizeAlchemyBatch;
exports.normalizePreparation = normalizePreparation;
exports.preparationEffects = preparationEffects;
exports.spendPreparationEncounter = spendPreparationEncounter;
...
function alchemyAvailability(state, recipeId, batches = 1) {
    const recipe = (0, alchemy_1.alchemyRecipeDef)(recipeId), countValid = Number.isSafeInteger(batches) && batches >= 1 && batches <= alchemy_1.MAX_ALCHEMY_BATCHES;
    ...
    const goldMax = recipe?.gold ? Math.floor((state.character?.gold ?? 0) / recipe.gold) : alchemy_1.MAX_ALCHEMY_BATCHES;
    const maxBatches = Math.max(0, Math.min(alchemy_1.MAX_ALCHEMY_BATCHES, materialMax, goldMax));
    const reason = !recipe ? 'Unknown alchemy recipe.' : !state.character ? 'Create a character first.' : !countValid ? `Choose 1–${alchemy_1.MAX_ALCHEMY_BATCHES} whole batches.` :
...
function startAlchemyBatch(state, recipeId, batches, nowMs) {
    if (!Number.isFinite(nowMs) || nowMs < 0)
        throw new Error('Invalid brewing time.');
    if (state.activity)
        throw new Error('Settle and stop the current activity before reserving a brew.');
    ...
    const bonuses = (0, permanent_boosts_1.characterPermanentMultipliers)(state);
    const brew = { version: 1, recipeId, totalBatches: batches, remainingBatches: batches,
...
function previewAlchemyReward(state, elapsed) {
    ...
    if (activity?.kind !== 'alchemy' || !brew)
        throw new Error('Missing reserved alchemy batch.');
    ...
    const xp = Math.max(0, Math.min(whole, (0, progression_1.totalXpAtLevel)(100) - (state.skills.find(s => s.skillId === 'alchemy')?.xp ?? 0)));
...
function normalizeAlchemyBatch(value, targetId) {
    const b = value, recipe = (0, alchemy_1.alchemyRecipeDef)(targetId);
    const integer = (n, min, max) => Number.isSafeInteger(n) && Number(n) >= min && Number(n) <= max;
    const valid = !!b && !!recipe && b.version === 1 && b.recipeId === targetId && integer(b.totalBatches, 1, alchemy_1.MAX_ALCHEMY_BATCHES) && integer(b.remainingBatches, 1, b.totalBatches) &&
    ...
    if (!valid)
        throw new Error('Invalid reserved alchemy batch. Keep the save backup; do not reset or discard the reservation.');
...
function normalizePreparation(value) {
    if (value === undefined || value === null)
        return undefined;
    const raw = value, def = (0, alchemy_1.potionDef)(raw.itemId);
...
function preparationEffects(preparation) {
    const def = preparation && preparation.remainingEncounters > 0 ? (0, alchemy_1.potionDef)(preparation.itemId) : undefined;
...
function spendPreparationEncounter(preparation, itemId) {
    if (!itemId || !preparation || preparation.itemId !== itemId)
        return preparation;
