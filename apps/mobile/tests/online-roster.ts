...
equal(state.character?.id,firstId,'inactive reroll does not change active character');
state=executeGameCommand(state,{type:'roster_delete',args:{id:rerolledId,confirmation:'Reborn'}},5).state;
equal(state.otherCharacters?.length,0,'confirmed delete removes inactive character');
