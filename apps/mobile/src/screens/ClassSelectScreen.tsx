...
function makeLanguageStyles(C:ThemeColors){const equipmentColors=equipmentTheme(C);return StyleSheet.create({toggle:{alignSelf:'center',minWidth:94,minHeight:44,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:spacing.md,backgroundColor:'transparent',paddingHorizontal:spacing.sm},current:{fontSize:12,fontWeight:'700',maxWidth:74,color:C.text},mark:{fontSize:22,color:C.accent},chip:{minWidth:104,minHeight:40,paddingHorizontal:12,justifyContent:'center',alignItems:'center',borderWidth:1,borderColor:C.line,borderRadius:99,backgroundColor:C.bg},chipSelected:{borderColor:equipmentColors.selectedLine,backgroundColor:equipmentColors.selected},chipText:{fontSize:12,color:C.muted,fontWeight:'700'},chipTextSelected:{color:C.text},pressed:{opacity:.76}})}
...
export function ClassSelectScreen({language='en',onLanguage,onSelect,onCancel,cancelLabel,mode='create'}:{language?:GameState['settings']['language'];onLanguage?:(language:GameState['settings']['language'])=>void;onSelect:(id:ClassId,name:string,body:BodyPresentation)=>Promise<void>|void;onCancel?:()=>void;cancelLabel?:string;mode?:'create'|'reroll'}){
  ...
  async function finish(){
    ...
    catch{setSaveError(mode==='reroll'?'Your reroll could not be completed. Your choices are still here; please try again.':'Your character could not be saved. Your choices are still here; please try again.')}
  ...
  return <KeyboardAvoidingView style={s.screen} behavior={Platform.OS==='ios'?'padding':'height'}><ScrollView ref={scroll} style={s.scroll} contentContainerStyle={s.root} keyboardShouldPersistTaps="handled">
    ...
    <Pressable accessibilityRole="button" accessibilityState={{expanded:showLanguages}} onPress={()=>setShowLanguages(value=>!value)} style={languageS.toggle}><Text style={languageS.current}>{LANGUAGE_NAMES[language]}</Text><Text style={languageS.mark}>{showLanguages?'−':'+'}</Text></Pressable></View><Text style={s.kicker}>{mode==='reroll'?'REROLL CHARACTER':t(language,'onboarding.createFirst')}</Text>{showLanguages&&<View style={s.languageGrid}>{SUPPORTED_LANGUAGES.map(id=><LanguageChip key={id} label={LANGUAGE_NAMES[id]} selected={language===id} onPress={()=>{onLanguage?.(id);setShowLanguages(false)}}/>)}</View>}
    ...
    {step==='review'&&<View style={s.section}>
      <Text accessibilityRole="header" style={s.heading}>{mode==='reroll'?'Ready to reroll?':t(language,'onboarding.ready')}</Text><Text style={s.description}>{mode==='reroll'?'Review the replacement character. The previous character progress in this slot will be removed when you confirm.':t(language,'onboarding.reviewPermanent')}</Text><Text style={s.note}>Your first crafting goal: {noviceSetFor(selected.id).name}. Start with the chest piece in Skills → Novice set. The set is earned, not granted at creation.</Text>
      ...
      <CreationAction title={saving?(mode==='reroll'?'Rerolling character…':'Saving character…'):mode==='reroll'?`Reroll as ${safeName}`:`Create ${safeName}`} disabled={saving} onPress={()=>setConfirming(true)}/>
    </View>}
  </ScrollView><View style={s.footer}><View style={s.navigation}>{onCancel&&<View style={s.flex}><CreationAction title={cancelLabel??t(language,'common.back')} disabled={saving} secondary onPress={onCancel}/></View>}{step!=='class'&&<View style={s.flex}><CreationAction title={t(language,'common.back')} disabled={saving} secondary onPress={()=>setStep(step==='review'?'identity':'class')}/></View>}{step!=='review'&&<View style={s.flex}><CreationAction title={t(language,step==='class'?'onboarding.chooseIdentity':'onboarding.reviewCharacter')} disabled={saving||(step==='identity'&&!!nameError)} onPress={next}/></View>}</View></View><ConfirmModal visible={confirming} title={mode==='reroll'?`Reroll as ${safeName}?`:`Create ${safeName}?`} message={mode==='reroll'?`This replaces the selected character with a fresh ${body==='male'?'male':'female'} ${selected.name}. Character levels, carried Gold, Inventory, equipment, skills, quests, loadouts and upgrades from the old character are permanently removed.`:`${safeName} will enter Asterfall as a ${body==='male'?'male':'female'} ${selected.name}, carrying only the ${starter.name}. The class and presentation cannot be changed.`} confirmLabel={mode==='reroll'?'Confirm reroll':'Enter Asterfall'} onConfirm={finish} onCancel={()=>setConfirming(false)}/></KeyboardAvoidingView>;
}

function makeStyles(C:ThemeColors){const equipmentColors=equipmentTheme(C);return StyleSheet.create({
  screen:{flex:1,backgroundColor:C.bg},scroll:{flex:1},root:{width:'100%',maxWidth:520,alignSelf:'center',padding:16,paddingTop:12,gap:12,backgroundColor:C.bg},brandRow:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:8},wordmark:{flex:1,maxWidth:184,height:62},kicker:{fontSize:10,lineHeight:14,color:C.muted,fontWeight:'800',letterSpacing:2,textAlign:'center'},
  languageGrid:{flexDirection:'row',flexWrap:'wrap',gap:8},languageChoice:{minWidth:96,flexGrow:1},stepRow:{flexDirection:'row',justifyContent:'space-around',paddingVertical:8},stepWrap:{flexDirection:'row',alignItems:'center',justifyContent:'center',gap:6,flex:1},stepDot:{width:24,height:24,borderRadius:12,alignItems:'center',justifyContent:'center',backgroundColor:'transparent'},stepDotActive:{backgroundColor:C.selection},stepNumber:{fontSize:12,color:C.muted,fontWeight:'900',includeFontPadding:false},stepNumberActive:{color:C.text},stepLabel:{fontSize:10,color:C.muted,fontWeight:'700'},stepLabelActive:{color:C.accentSoft},
  section:{gap:12},heading:{fontFamily:Platform.OS==='ios'?'Georgia':'serif',fontSize:25,lineHeight:32,fontWeight:'500',color:C.accentSoft,textAlign:'center'},creationPreview:{backgroundColor:C.stage,borderTopLeftRadius:70,borderTopRightRadius:70,borderBottomLeftRadius:24,borderBottomRightRadius:24,overflow:'hidden'},creationPreviewContent:{alignItems:'center',paddingHorizontal:24,paddingVertical:24,gap:10},creationPortrait:{width:190,height:230},creationBadgeText:{fontSize:11,lineHeight:16,color:C.accentSoft,fontWeight:'700',textAlign:'center'},nameBlock:{gap:6},nameField:{borderWidth:1,borderColor:C.line,borderRadius:14,backgroundColor:C.inputBg,paddingHorizontal:18,paddingVertical:12},nameFieldFocused:{borderColor:C.selectionLine},nameFieldError:{borderColor:C.bad},label:{fontSize:10,lineHeight:16,color:C.muted,fontWeight:'800',letterSpacing:1},input:{minHeight:28,color:C.text,padding:0,margin:0,fontSize:17,fontWeight:'600',textAlignVertical:'center',includeFontPadding:false},inputMeta:{flexDirection:'row',justifyContent:'space-between',gap:8},saveErrorCard:{gap:4,padding:12,borderWidth:1,borderColor:C.bad,borderRadius:12,backgroundColor:C.badSurface},saveErrorLabel:{fontSize:10,lineHeight:14,color:C.bad,fontWeight:'900',letterSpacing:1},error:{fontSize:12,lineHeight:18,color:C.bad,flex:1},counter:{fontSize:12,lineHeight:18,color:C.muted},chips:{gap:8},chip:{minHeight:44,paddingHorizontal:14,borderRadius:18,justifyContent:'center',backgroundColor:C.panel2},chipText:{fontSize:14,fontWeight:'600',color:C.accentSoft},choiceRow:{flexDirection:'row',gap:8},flex:{flex:1},note:{fontSize:12,lineHeight:18,color:C.muted,textAlign:'center'},description:{fontSize:14,lineHeight:21,color:C.muted,textAlign:'center'},
  filterRow:{flexDirection:'row',gap:6},filter:{flex:1,minHeight:40,paddingHorizontal:8,borderWidth:1,borderColor:C.line,borderRadius:99,backgroundColor:C.bg,justifyContent:'center',alignItems:'center'},filterActive:{borderColor:equipmentColors.selectedLine,backgroundColor:equipmentColors.selected},filterText:{fontSize:12,fontWeight:'700',color:C.muted},filterTextActive:{color:C.text},loadout:{borderTopWidth:1,borderColor:C.line,paddingVertical:16,paddingHorizontal:8,flexDirection:'row',alignItems:'center',gap:12},gearName:{fontSize:15,lineHeight:21,color:C.accentSoft,fontWeight:'700'},weaponTag:{fontSize:11,fontWeight:'800',color:C.info},reviewCard:{backgroundColor:C.panel,borderRadius:20,padding:16,flexDirection:'row',alignItems:'center',gap:12},reviewCopy:{flex:1,gap:5},reviewName:{fontSize:20,lineHeight:26,color:C.accentSoft,fontWeight:'900'},reviewRole:{fontSize:14,lineHeight:20,fontWeight:'700'},reviewLine:{fontSize:13,lineHeight:19,color:C.muted},footer:{backgroundColor:C.bg,paddingHorizontal:20,paddingTop:8,paddingBottom:12},navigation:{width:'100%',maxWidth:488,alignSelf:'center',flexDirection:'row',gap:8},
});}
