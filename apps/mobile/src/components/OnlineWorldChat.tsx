...
export function OnlineWorldChat({playerName,language,unlockedEmoteIds=[],trayIds=[],bodyPresentation='male',onTrayChange,embedded=false,reduceMotion=false,selectedChannel}:{playerName:string;language:Language;unlockedEmoteIds?:readonly string[];trayIds?:readonly string[];bodyPresentation?:'male'|'female';onTrayChange?:(ids:string[])=>void|Promise<void>;embedded?:boolean;reduceMotion?:boolean;selectedChannel?:number}){
 const C=useGameTheme(),s=useMemo(()=>makeStyles(C),[C]),{width,fontScale}=useWindowDimensions(),stackCompose=width<360||fontScale>=1.25;
 const [open,setOpen]=useState(embedded),[localChannel,setChannel]=useState(0),[text,setText]=useState(''),[rows,setRows]=useState<WorldMessage[]>([]),[selected,setSelected]=useState<WorldMessage|null>(null),[busy,setBusy]=useState(false),[error,setError]=useState('');
 ...
 const content=<View style={s.root}>
  ...
  <ChatLog channelKey={WORLD_CHANNELS[channel].id} items={rows} emptyText={ot(language,'chat.none')} renderItem={row=><View style={s.messageRow}><Pressable accessibilityRole="button" accessibilityLabel={`Open ${row.sender_name}'s player profile`} onPress={()=>setSelected(row)} style={s.nameButton}><GuildTaggedPlayerName numberOfLines={1} style={s.name} name={row.sender_name} guildTag={row.guild_tag} tagColorId={row.guild_tag_color_id} nameStyle={row.player_name_style??undefined} reduceMotion={reduceMotion}/><Text style={s.profileMark}>›</Text></Pressable><Text style={s.time}>{new Date(row.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</Text><ChatMessageText body={row.body} mentionName={playerName}/></View>}/>
  ...
  <View style={[s.compose,stackCompose&&s.composeStack]}><TextInput accessibilityLabel="World chat message" value={text} onChangeText={setText} onSubmitEditing={()=>void send()} placeholder={ot(language,'chat.placeholder')} maxLength={300} style={s.input}/><View style={[s.composeActions,stackCompose&&s.composeActionsStack]}><ChatEmotePicker unlockedIds={unlockedEmoteIds} trayIds={trayIds} bodyPresentation={bodyPresentation} usedCount={chatEmoteCount(text)} onTrayChange={onTrayChange} onPick={token=>setText(value=>(value+token).slice(0,300))}/><Pressable accessibilityRole="button" accessibilityLabel="Send message" accessibilityState={{disabled:busy||!text.trim()}} disabled={busy||!text.trim()} onPress={()=>void send()} style={({pressed})=>[s.send,(pressed||busy||!text.trim())&&s.sendDim]}><UiIcon name="next" size={24}/></Pressable></View></View>
