...
export function EquipmentPreview({state,itemId,onClose}:{state:GameState;itemId:string|null;onClose:()=>void}){
  ...
  const item=itemDef(itemId),before=effectiveStats(state),after=effectiveStats(projected),rarityId=itemRarity(item),rarity=rarityMeta(rarityId),nameColor=rarityNameColor(rarityId,true,C.text),enhancement=gearEnhancement(state,itemId),capacity=gemSocketCapacity(itemId);
  ...
  return <Modal visible animationType={state.settings.reduceMotion?'none':'slide'} onRequestClose={onClose}><SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.root}>
    ...
    <View style={[s.hero,{borderColor:rarity.color,backgroundColor:rarity.surface}]}>{hasEquipmentArtwork(item)&&<EquipmentArtwork item={item}/>}<View style={s.flex}><Text style={[s.rarity,{color:rarity.color}]}>{rarity.symbol} {rarity.label.toUpperCase()} · {item.slot?.toUpperCase()}</Text><Text style={[s.title,{color:nameColor}]}>{item.name}{enhancement.rank?` +${enhancement.rank}`:''}</Text><Text style={s.meta}>{capacity?`◆ ${enhancement.gemIds.length}/${capacity} gems`:'No gem sockets'}</Text></View></View>
