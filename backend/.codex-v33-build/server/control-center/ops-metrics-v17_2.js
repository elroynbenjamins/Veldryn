...
class OpsMetrics {
    ...
    itemDestroyed(quantity, itemId, sink) { return this.record('economy.items.destroyed', quantity, { dimensionKey: `sink:${sink}`, dimensions: { sink, itemId } }); }
    currencyCreated(currencyId, amount, source) { return this.record('economy.currency.created', amount, { dimensionKey: `currency:${currencyId}:source:${source}`, dimensions: { currencyId, source } }); }
    currencyDestroyed(currencyId, amount, sink) { return this.record('economy.currency.destroyed', amount, { dimensionKey: `currency:${currencyId}:sink:${sink}`, dimensions: { currencyId, sink } }); }
    rewardBundleGranted(bundleId, count = 1) { return this.record('economy.reward_bundle.grants', count, { dimensionKey: `bundle:${bundleId}`, dimensions: { bundleId } }); }
