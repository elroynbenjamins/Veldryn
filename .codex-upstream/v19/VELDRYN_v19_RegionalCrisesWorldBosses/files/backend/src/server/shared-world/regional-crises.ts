...
import type { LiveOpsActivityKind } from '../liveops/event-definitions';
import type { SocialContributionEvent } from '../liveops/contribution';
import { contributionEligibleForRules } from '../liveops/contribution';
import type { EventContributionRules } from '../liveops/event-definitions';
import { scoreContribution } from '../party/party-contracts';

export type RegionalCrisisFocus = 'combat' | 'skilling' | 'mixed';
export type RegionalCrisisState = 'scheduled' | 'active' | 'secured' | 'failed' | 'cancelled' | 'finalized';

export interface RegionalCrisisScaleInput {
  eligibleActiveAccounts7d: number;
  expectedParticipationFraction?: number;
  expectedMinutesPerParticipantPerDay?: number;
  durationHours: number;
  minimumTargetPoints?: number;
  maximumTargetPoints?: number;
}

export interface RegionalCrisisScaleSnapshot {
  eligibleActiveAccounts7d: number;
  expectedParticipationFraction: number;
  expectedMinutesPerParticipantPerDay: number;
  durationHours: number;
...
export interface RegionalCrisisStageDefinition {
  id: string;
  thresholdFraction: number;
  name: string;
  description: string;
  rewardBundleId?: string;
  unlockWorldBossTemplateId?: string;
}

export interface RegionalCrisisDefinition {
  id: string;
  version: number;
  name: string;
  regionId: string;
  description: string;
  focus: RegionalCrisisFocus;
  durationHours: number;
  contributionRules: EventContributionRules;
  stages: readonly RegionalCrisisStageDefinition[];
  personalMilestones: readonly number[];
  personalRewardEligibilityPoints: number;
  successRewardBundleId: string;
  participationRewardBundleId: string;
  successRegionModifier?: { modifierKey:string; durationHours:number; description:string };
  eventTags: readonly string[];
}

export interface RegionalCrisisContributionScore {
  eligible: boolean;
  reason?: string;
  rawPoints: number;
...
export interface RegionalCrisisProgress {
  targetPoints: number;
  creditedPoints: number;
  combatPoints: number;
...
export interface RegionalCrisisEvaluation {
...
export function deriveRegionalCrisisScale(input:RegionalCrisisScaleInput):RegionalCrisisScaleSnapshot {
...
export function validateRegionalCrisisDefinition(definition:RegionalCrisisDefinition):string[] {
  const errors:string[]=[];
  ...
  if(definition.durationHours<24||definition.durationHours>96) errors.push('duration_out_of_range');
  if(definition.personalRewardEligibilityPoints<=0) errors.push('eligibility_points_invalid');
  if(definition.stages.length<2) errors.push('stages_required');
  let previous=0;
  for(const stage of definition.stages){
    if(stage.thresholdFraction<=previous||stage.thresholdFraction>1) errors.push('stage_thresholds_invalid');
    if(!stage.id.trim()||!stage.name.trim()) errors.push('stage_identity_required');
    previous=stage.thresholdFraction;
  }
  if(Math.abs((definition.stages[definition.stages.length-1]?.thresholdFraction ?? 0)-1)>0.000001) errors.push('final_stage_must_be_100_percent');
  let milestone=0;
  for(const points of definition.personalMilestones){ if(points<=milestone) errors.push('personal_milestones_not_increasing'); milestone=points; }
  const ruleCategories = definition.contributionRules.allowedCategories;
  if(definition.focus==='combat' && (ruleCategories.length!==1||ruleCategories[0]!=='combat')) errors.push('combat_focus_rules_mismatch');
  if(definition.focus==='skilling' && (ruleCategories.length!==1||ruleCategories[0]!=='skilling')) errors.push('skilling_focus_rules_mismatch');
  if(definition.focus==='mixed' && (!ruleCategories.includes('combat')||!ruleCategories.includes('skilling'))) errors.push('mixed_focus_rules_mismatch');
  return [...new Set(errors)];
}

export function scoreRegionalCrisisContribution(
  event:SocialContributionEvent,
  definition:RegionalCrisisDefinition,
  pointsCreditedToday:number,
):RegionalCrisisContributionScore {
  const eligible = contributionEligibleForRules(event,definition.contributionRules);
  const rawPoints=Math.max(0,Math.floor(scoreContribution({profile:event.profile,units:event.units})));
  if(!eligible.eligible||rawPoints<=0) return {eligible:false,reason:eligible.reason??'no_points',rawPoints,creditedPoints:0,category:event.profile.category};
...
export function evaluateRegionalCrisis(definition:RegionalCrisisDefinition,progress:RegionalCrisisProgress):RegionalCrisisEvaluation {
  ...
  if(currentStageIndex<0) currentStageIndex=definition.stages.length-1;
  const missing:string[]=[];
  if(progress.creditedPoints<target) missing.push('global_points');
  if(definition.focus==='mixed'){
    const required=definition.contributionRules.minimumCategoryFraction??{combat:0.25,skilling:0.25};
    if(progress.combatPoints<Math.ceil(target*(required.combat??0))) missing.push('combat_share');
    if(progress.skillingPoints<Math.ceil(target*(required.skilling??0))) missing.push('skilling_share');
  }
  return {progressFraction:fraction,secured:missing.length===0,currentStageIndex,reachedStageIds:reached,missing};
}

export function reachedCrisisPersonalMilestones(points:number,definition:RegionalCrisisDefinition):number[]{
  return definition.personalMilestones.filter(threshold=>points>=threshold);
}

const sharedStages=(bossTemplateId?:string):readonly RegionalCrisisStageDefinition[]=>[
...
export const REGIONAL_CRISIS_TEMPLATE_POOL:readonly RegionalCrisisDefinition[]=[
  {
...
for(const definition of REGIONAL_CRISIS_TEMPLATE_POOL){ const errors=validateRegionalCrisisDefinition(definition); if(errors.length) throw new Error(`invalid_regional_crisis:${definition.id}:${errors.join(',')}`); }
