...
function ContractCard({ contract, nowMs }: { contract: PartyContractView; nowMs: number }) {
  ...
  return <View style={styles.contractCard}>
    <View style={styles.between}><Text style={styles.bodyStrong}>{contract.name}</Text><Text style={styles.badge}>{contract.category.toUpperCase()}</Text></View>
    ...
    <Text style={eligible ? styles.good : styles.warning}>Your contribution: {contract.personalPoints} / {contract.minimumPersonalPoints} minimum {eligible ? '✓' : ''}</Text>
