...
export function GuildCrest({size=48,bannerId}:{size?:number;bannerId?:GuildBannerId|string}){const banner=GUILD_BANNERS.find(entry=>entry.id===normalizeGuildBannerId(bannerId))??GUILD_BANNERS[0];return <View accessibilityLabel={`${banner.name} guild crest`} style={[s.guildCrest,{width:size,height:size,borderColor:banner.secondary,backgroundColor:banner.primary}]}><Text style={[s.guildEmblem,{fontSize:Math.max(18,Math.round(size*.48)),color:banner.secondary}]}>{banner.emblem}</Text></View>;}
