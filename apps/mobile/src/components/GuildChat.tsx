...
export function GuildChat({language,currentPlayerName,unlockedEmoteIds=[],trayIds=[],bodyPresentation='male',onTrayChange,firstUnreadMessageId,onRead,reduceMotion=false}:{language:Language;currentPlayerName?:string;unlockedEmoteIds?:readonly string[];trayIds?:readonly string[];bodyPresentation?:'male'|'female';onTrayChange?:(ids:string[])=>void|Promise<void>;firstUnreadMessageId?:string;onRead?:()=>void;reduceMotion?:boolean}){
 ...
 return <View style={s.root}>
  ...
  <ChatLog channelKey={guild?.id??'guild:none'} items={messages} firstUnreadMessageId={firstUnreadMessageId} emptyText="No Guild messages yet. Start the conversation." onCaughtUp={()=>void markSocialChatRead('guild').then(()=>onReadRef.current?.()).catch(()=>{})} renderItem={message=><View style={s.message}>
    <View style={s.messageHead}><Pressable accessibilityRole="button" accessibilityLabel={'Open '+message.sender_name+"'s player profile"} onPress={()=>setSelected(message)} style={s.nameButton}><GuildTaggedPlayerName style={s.name} name={message.sender_name} guildTag={message.guild_tag} tagColorId={message.guild_tag_color_id} nameStyle={message.name_style} reduceMotion={reduceMotion}/><Text style={s.profileMark}>›</Text></Pressable>{message.guild_role?<View style={[s.rolePill,message.guild_role==='leader'?s.roleLeader:message.guild_role==='officer'?s.roleOfficer:s.roleMember]}><Text style={[s.role,message.guild_role==='leader'?s.roleTextLeader:message.guild_role==='officer'?s.roleTextOfficer:s.roleTextMember]}>{message.guild_role.toUpperCase()}</Text></View>:null}<Text style={s.time}>{new Date(message.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</Text></View>
  ...
  <ChatPlayerSheet message={selected?{...selected,message_id:selected.id}:null} onClose={()=>setSelected(null)} onBlocked={blockedId=>setSnapshot(current=>current?{...current,messages:current.messages.filter(message=>message.account_id!==blockedId)}:current)} reduceMotion={reduceMotion}/>
 </View>;
}
