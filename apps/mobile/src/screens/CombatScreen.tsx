...
export function CombatScreen({state,onChangeRegion,onStart,onBoss}:{state:GameState;onChangeRegion:()=>void;onStart:(id:string)=>void;onBoss:()=>void}){
  ...
  return <ScrollView contentContainerStyle={s.root}>
    <Text style={s.kicker}>COMBAT</Text><Text accessibilityRole="header" style={s.h}>Combat Level {character.level}</Text><StatBar label="Combat experience" current={progress.current} max={progress.need}/>
    <View style={[s.region,{borderColor:zone.accent}]}><View style={s.flex}><Text style={s.regionLabel}>CURRENT REGION</Text><Text style={s.regionName}>{zone.symbol} {zone.name}</Text><Text style={s.sub}>{zone.subtitle}</Text></View><View style={s.change}><GameButton title="Change" tone="secondary" onPress={onChangeRegion}/></View></View>
...
const s=StyleSheet.create({root:{padding:spacing.lg,gap:spacing.md},kicker:{...typography.caption,color:equipmentColors.gold,fontWeight:'900',letterSpacing:1.2},h:{...typography.hero,color:C.text},region:{minHeight:92,flexDirection:'row',alignItems:'center',gap:spacing.md,padding:spacing.md,backgroundColor:equipmentColors.panel,borderWidth:1,borderRadius:radii.lg},flex:{flex:1,minWidth:0},regionLabel:{...typography.caption,color:C.muted,fontWeight:'900',letterSpacing:.8},regionName:{...typography.title,color:equipmentColors.goldSoft},sub:{...typography.body,color:C.muted},change:{width:92},section:{...typography.caption,color:equipmentColors.goldSoft,fontWeight:'900',letterSpacing:1}});
