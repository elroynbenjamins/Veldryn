...
type Row={recipe:(typeof RECIPES)[number];status:ReturnType<typeof recipeAvailability>};

export function CraftingRecipeBrowser({state,skillId,initialQuery='',onCraft,onNavigate,onCraftPrerequisites}:{state:GameState;skillId:CraftingSkillId;initialQuery?:string;onCraft:(id:string)=>void;onNavigate?:(destination:WorkingTowardDestination)=>void;onCraftPrerequisites?:(recipeId:string)=>void}){
 ...
 const all:Row[]=RECIPES.filter(recipe=>recipe.skillId===skillId&&!recipe.noviceSetId&&(!recipe.classId||recipe.classId===state.character?.classId)).sort((a,b)=>a.level-b.level||a.name.localeCompare(b.name)).map(recipe=>({recipe,status:recipeAvailability(state,recipe.id)}));
