...
export function RecruitmentBoardPanel(props: RecruitmentBoardPanelProps) {
  ...
  return <View style={styles.panel}>
    ...
    <View style={styles.filters}>{(['all', 'combat', 'skilling', 'mixed'] as const).map((value) => <Pressable key={value} style={[styles.filter, focus === value && styles.filterActive]} onPress={() => setFocus(value)}><Text style={styles.filterText}>{value === 'all' ? 'All' : activityPreferenceLabel[value]}</Text></Pressable>)}</View>
...
const styles = StyleSheet.create({
  ...
  filters: { flexDirection: 'row', gap: 6, flexWrap: 'wrap' }, filter: { paddingHorizontal: 9, paddingVertical: 6, borderRadius: 7, borderWidth: 1, borderColor: '#354858' }, filterActive: { borderColor: '#6fa2c2', backgroundColor: '#1c3444' }, filterText: { color: '#c6d2dc', fontWeight: '700', fontSize: 11 },
