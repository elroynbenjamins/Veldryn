...
export function gatheringBalanceProjection(state:GameState,activity:GatherDef,offlineHours:number):GatheringBalanceProjection{
  ...
  const essence=activity.skillId==='herbalism'?HERBALISM_ESSENCE_BY_ZONE[activity.zoneId]:undefined,rareItemsPerHour=essence?actionsPerHour*Math.min(1,essence.baseChance*(method?.rareFindMultiplier??1)*effect.dropChanceMultiplier*permanent.dropChanceMultiplier):undefined;
...
export function activeGatheringRuntimeProjection(state:GameState){
  ...
  const herbLevel=state.skills.find(row=>row.skillId==='herbalism')?.level??1,method=definition.skillId==='herbalism'?herbalismMethod(state.character?.herbalismMethodId,herbLevel):undefined;
  ...
  return {
    ...
    herbalismMethod:method,rareItemId:essence?.itemId,rareItemsPerHour:essence?actionsPerHour*Math.min(1,essence.baseChance*(method?.rareFindMultiplier??1)*effect.dropChanceMultiplier*permanent.dropChanceMultiplier):undefined,
  };
}
