...
export const COMPANION_HOUSING_SLOTS_BASE=3;
export const COMPANION_HOUSING_SLOTS_MAX=8;
...
export function companionHousingLevelCap(housingTier:number){
 const tier=Math.max(0,Math.min(3,Math.floor(housingTier)));
 return COMPANION_HOUSING_MILESTONES[tier].levelCap;
}
export function companionHousingRequiredTier(progress:OwnedCompanionProgress){
 if(progress.level<10)return 0;if(progress.level<20)return 1;if(progress.level<25)return 2;return 3;
}
