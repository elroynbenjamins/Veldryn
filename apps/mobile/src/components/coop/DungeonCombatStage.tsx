...
export function DungeonCombatStage({run,enemyLabel,boss=false}:{run:CoopRunView;enemyLabel?:string;boss?:boolean}){
 ...
 const stepReplay=(delta:-1|1)=>{setPaused(true);setCueIndex(value=>Math.max(0,Math.min(cues.length-1,value+delta)));};
 ...
 return <FantasyPanel variant={boss?'danger':'selected'}>
  ...
  <View style={[s.arena,{padding:layout.arenaPadding}]}>
   <View style={[s.enemyField,{width:`${boss?layout.bossWidthPctFinal:layout.bossWidthPct}%` as `${number}%`}]}><EnemyCombatProfileCard name={shownEnemy} boss={boss} active={enemyActive} targeted={enemyTargeted} currentCue={currentCue} bossPhaseLabel={boss?bossPhaseLabel:undefined} bossPhases={boss?run.bossMechanic?.telegraph?.phases:undefined} bossCast={boss?bossCast:undefined} currentHp={enemyState?.hp} maximumHp={enemyState?.maxHp} combatShield={enemyState?.shield??0} statuses={enemyStatuses} animateHealth={!reduceMotion} onPress={()=>toggleInspect('enemy')} selectedForInspect={inspectKey==='enemy'} feedbackStyle={feedbackStyle} motionStyle={enemyActive?actorStyle:enemyTargeted?targetStyle:undefined}/></View>
  ...
  {replay?<View style={s.replayPanel}>
   ...
   {(!complete||paused)&&!reduceMotion&&cues.length>1?<View style={[s.playbackControls,layout.controlsWrap&&s.playbackControlsWrap]}><View style={s.speedGroup}>{DUNGEON_PLAYBACK_SPEEDS.map(speed=><Pressable key={speed} accessibilityRole="button" accessibilityLabel={`Combat replay speed ${speed} times`} accessibilityState={{selected:playbackSpeed===speed}} onPress={()=>setPlaybackSpeed(speed)} style={({pressed})=>[s.speedButton,playbackSpeed===speed&&s.speedButtonActive,pressed&&s.controlPressed]}><Text style={[s.speedButtonText,playbackSpeed===speed&&s.speedButtonTextActive]}>{speed}×</Text></Pressable>)}</View><View style={s.controlActions}>{paused?<Pressable accessibilityRole="button" accessibilityLabel="Previous combat event" disabled={cueIndex<=0} onPress={()=>stepReplay(-1)} style={({pressed})=>[s.stepButton,cueIndex<=0&&s.stepButtonDisabled,pressed&&cueIndex>0&&s.controlPressed]}><Text style={s.stepButtonText}>‹</Text></Pressable>:null}<Pressable accessibilityRole="button" accessibilityLabel={paused?'Resume combat replay':'Pause combat replay'} accessibilityState={{selected:paused}} onPress={()=>setPaused(value=>!value)} style={({pressed})=>[s.pauseButton,paused&&s.pauseButtonActive,pressed&&s.controlPressed]}><Text style={[s.pauseButtonText,paused&&s.pauseButtonTextActive]}>{paused?'▶ Resume':'Ⅱ Pause'}</Text></Pressable>{paused?<Pressable accessibilityRole="button" accessibilityLabel="Next combat event" disabled={cueIndex>=cues.length-1} onPress={()=>stepReplay(1)} style={({pressed})=>[s.stepButton,cueIndex>=cues.length-1&&s.stepButtonDisabled,pressed&&cueIndex<cues.length-1&&s.controlPressed]}><Text style={s.stepButtonText}>›</Text></Pressable>:null}<Pressable accessibilityRole="button" accessibilityLabel="Skip combat replay to result" onPress={()=>{setPaused(false);setShowLog(false);setCueIndex(Math.max(0,cues.length-1));}} style={({pressed})=>[s.skipButton,pressed&&s.controlPressed]}><Text style={s.skipButtonText}>Skip →</Text></Pressable></View></View>:null}
...
const s=StyleSheet.create({
 ...
 enemyField:{alignSelf:'center',alignItems:'stretch',justifyContent:'center',zIndex:2},
 partyField:{width:'100%',flexDirection:'row',alignItems:'flex-end',justifyContent:'space-between',zIndex:2},
