...
export function MonsterPortraitFrame({monster,size=96,active=false,framed=true}:{monster:MonsterDef;size?:number;active?:boolean;framed?:boolean}){
  const accent=monster.boss?'#e2ad52':monster.level>=20?'#a783d8':monster.level>=10?'#5ea3c7':C.good;
  return <View accessibilityLabel={`${monster.name} pixel portrait`} style={[s.frame,!framed&&s.unframed,{width:size,height:size,borderColor:active?C.accent:accent},active&&framed&&s.active]}>
