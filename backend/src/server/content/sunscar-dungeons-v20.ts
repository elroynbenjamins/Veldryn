...
export type DungeonNodeTypeV20='combat'|'elite'|'event'|'rest'|'treasure'|'hazard'|'puzzle'|'boss';
export interface DungeonContentQueueRulesV20 {contentId:string;minLevel:number;maxLevelDeltaPreferred:number;startPowerWindowFraction:number;expandPowerWindowFractionPerMinute:number;maxPowerWindowFraction:number;minimumTankScore:number;minimumSupportScore:number;readyCheckSeconds:number;routeVoteSeconds:number;reconnectGraceSeconds:number;safetyAiAfterSeconds:number;hardDisconnectSeconds:number;}

export interface SunscarDungeonNodeV20 {
  id:string;
  dungeonId:string;
  depth:number;
...
export interface SunscarDungeonBossCheckV20 {
...
export interface SunscarDungeonDefinitionV20 {
  id:'COP_004'|'COP_005'|'COP_006';
  name:string;
  regionId:'REG_002';
  minLevel:number;
  ...
  bossChecks:readonly SunscarDungeonBossCheckV20[];
  rewardHooks:readonly string[];
  equipmentRewardsFinalized:false;
}

const queue=(contentId:string,minLevel:number):DungeonContentQueueRulesV20=>({
...
export const SUNSCAR_DUNGEONS_V20:readonly SunscarDungeonDefinitionV20[]=[
  {
...
export const SUNSCAR_DUNGEON_NODES_V20:readonly SunscarDungeonNodeV20[]=[
...
export function validateSunscarDungeonDefinitionsV20():string[]{
  const errors:string[]=[];
  for(const dungeon of SUNSCAR_DUNGEONS_V20){
    if(dungeon.routeDepthMin<4||dungeon.routeDepthMax>7||dungeon.routeDepthMin>dungeon.routeDepthMax) errors.push(`${dungeon.id}:route_depth_invalid`);
    if(dungeon.branchOptionsMin<3) errors.push(`${dungeon.id}:needs_at_least_three_branch_options`);
    if(dungeon.queueRules.contentId!==dungeon.id) errors.push(`${dungeon.id}:queue_content_mismatch`);
    if(dungeon.queueRules.minimumTankScore<.72||dungeon.queueRules.minimumTankScore>1) errors.push(`${dungeon.id}:tank_score_invalid`);
    if(dungeon.queueRules.minimumSupportScore<.72||dungeon.queueRules.minimumSupportScore>1) errors.push(`${dungeon.id}:support_score_invalid`);
    if(dungeon.bossChecks.filter(c=>c.role==='tank').length!==1) errors.push(`${dungeon.id}:tank_check_missing`);
    if(dungeon.bossChecks.filter(c=>c.role==='support').length!==1) errors.push(`${dungeon.id}:support_check_missing`);
    if(dungeon.bossChecks.filter(c=>c.role==='damage').length!==1) errors.push(`${dungeon.id}:damage_check_missing`);
  }
  return errors;
}
