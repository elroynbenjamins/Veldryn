...
export function InventoryScreen({state,onEquip,onFood,onEat,onSell,onSalvage,onDeposit,onDepositMaterials,onUpgradeStorage,onWithdraw,onOverflow,onToggleFavorite,onAcknowledgeItem,onAcknowledgeAll,onBulkAction,onNavigateInspect}:{state:GameState;onEquip:(id:string)=>void;onFood:(id:string)=>void;onEat:(id:string)=>void;onSell:(id:string)=>void;onSalvage:(id:string)=>void;onDeposit:(id:string,quantity:number)=>void;onDepositMaterials:()=>void;onUpgradeStorage:(location:StorageLocation)=>void;onWithdraw:(id:string,quantity:number)=>void;onOverflow:()=>void;onToggleFavorite:(id:string)=>void;onAcknowledgeItem:(id:string)=>void;onAcknowledgeAll:()=>void;onBulkAction:(kind:BulkAction,location:BulkStorageLocation,ids:string[])=>void;onNavigateInspect:(destination:WorkingTowardDestination)=>void}){
  ...
  const message=pending?.kind==='sell'?`Sell 1 for ${pending.item.value} gold. This cannot be undone.${foodWarning}${goalWarningText}`:pending?.kind==='deposit'?`Move ${pending.quantity}× ${pending.item.name} to Bank?${foodWarning}`:pending?.item.salvage?`Destroy 1 item for ${pending.item.salvage.quantity}× ${itemDef(pending.item.salvage.itemId).name}.${goalWarningText}`:'';
  const bulkMessage=bulkPending?.kind==='transfer'
    ...
    :bulkPending?.kind==='sell'
      ?`Sell ${selectionSummary.sellableStackCount} eligible stacks (${selectionSummary.sellableUnitCount} items) for ${formatGameNumber(selectionSummary.sellGold,state.settings.numberMode)} gold?${selectionSummary.sellProtectedCount?` ${selectionSummary.sellProtectedCount} selected stacks are excluded by protection or sell rules.`:''}${selectionSummary.goalProtectedCount?` ${selectionSummary.goalProtectedCount} Working Toward stack${selectionSummary.goalProtectedCount===1?' stays':'s stay'} protected.`:''} This cannot be undone.`
      :bulkPending?.kind==='salvage'
        ?`Salvage ${selectionSummary.salvageableStackCount} eligible equipment stacks (${selectionSummary.salvageableUnitCount} items)?${selectionSummary.salvageProtectedCount?` ${selectionSummary.salvageProtectedCount} selected stacks are excluded because they are protected or not salvageable.`:''}${selectionSummary.goalProtectedCount?` ${selectionSummary.goalProtectedCount} Working Toward stack${selectionSummary.goalProtectedCount===1?' stays':'s stay'} protected.`:''} This cannot be undone.`
        :'';
  ...
  return <><EquipmentPreview state={state} itemId={previewId} onClose={()=>setPreviewId(null)}/><ItemQuickInspect state={state} itemId={inspectId} onClose={closeInspect} onNavigate={destination=>{closeInspect();onNavigateInspect(destination)}}/><ScrollView contentContainerStyle={s.root} keyboardShouldPersistTaps="handled">
    ...
    {showStorage&&<Panel><Text style={s.sub}>Inventory travels with this character. Bank storage is shared by every character on the account.</Text>{location==='inventory'&&<GameButton title="Deposit all materials" tone="secondary" disabled={!state.inventory.stacks.some(entry=>itemDef(entry.itemId).type==='material')} onPress={()=>run(onDepositMaterials)}/>}<View style={s.row}><View style={s.flex}><Text style={s.upgradeLabel}>INVENTORY · {effectiveInventoryCapacity} SLOTS{effectiveInventoryCapacity>state.inventory.capacity?` · ${state.inventory.capacity} base`:''}</Text><GameButton title={inventoryUpgrade?`Upgrade to ${inventoryUpgrade.capacity} · ${formatGameNumber(inventoryUpgrade.cost,state.settings.numberMode)}g`:'Inventory maxed'} disabled={!inventoryUpgrade} tone="secondary" onPress={()=>run(()=>onUpgradeStorage('inventory'))}/></View><View style={s.flex}><Text style={s.upgradeLabel}>BANK · {effectiveBankCapacity} SLOTS{effectiveBankCapacity>state.bank.capacity?` · ${state.bank.capacity} base`:''}</Text><GameButton title={bankUpgrade?`Upgrade to ${bankUpgrade.capacity} · ${formatGameNumber(bankUpgrade.cost,state.settings.numberMode)}g`:'Bank maxed'} disabled={!bankUpgrade} tone="secondary" onPress={()=>run(()=>onUpgradeStorage('bank'))}/></View></View></Panel>}
