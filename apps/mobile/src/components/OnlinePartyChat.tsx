...
export function OnlinePartyChat(){
 ...
 return <PartyChatGate party={party} accountId={accountId}><View style={s.root}><ScrollView style={s.messages}>{messages.map(message=><View style={s.message} key={message.id}><Pressable accessibilityRole="button" accessibilityLabel={`Open ${message.sender_name}'s player profile`} onPress={()=>setSelected(message)}><GuildTaggedPlayerName style={s.name} name={message.sender_name} guildTag={message.guild_tag} tagColorId={message.guild_tag_color_id}/></Pressable><ChatMessageText body={message.body}/></View>)}</ScrollView>
...
const createStyles=(C:any,equipmentColors:any)=>StyleSheet.create({root:{gap:spacing.sm},messages:{maxHeight:260,minHeight:120},message:{paddingVertical:7,borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:C.line},text:{color:C.text,marginTop:2},name:{color:'#8dcdf0',fontWeight:'800'},errorCard:{gap:4,padding:spacing.sm,borderWidth:1,borderColor:C.bad,borderRadius:8,backgroundColor:'#2a1b20'},errorLabel:{...typography.caption,color:C.bad,fontWeight:'900',letterSpacing:1},error:{color:C.text},compose:{flexDirection:'row',alignItems:'center',gap:spacing.sm},input:{flex:1},send:{minWidth:72}});
