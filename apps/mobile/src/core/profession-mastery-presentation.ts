...
export function professionMasteryActionsForSkill(state:GameState,skillId:SkillId):ProfessionMasteryActionView[]{
 ...
 for(const node of [...GATHERING,...HERB_NODES].filter(row=>row.skillId===skillId)){
  ...
  rows.push({...progress,id:node.id,name:node.name,skillId,kind:'gathering',level:node.unlockLevel,nextBonus:nextProfessionMasteryBonus(progress.rank),destination:{kind:'skills',skillId,mode:'gathering',actionId:node.id,regionId:node.zoneId,button:'Open '+node.name,detail:'Repeat '+node.name+' to build action mastery.'}});
 }
 for(const recipe of RECIPES.filter(row=>row.skillId===skillId&&relevantRecipe(state,row))){
  ...
  rows.push({...progress,id:recipe.id,name:recipe.name,skillId,kind:'crafting',level:recipe.level,nextBonus:nextProfessionMasteryBonus(progress.rank),destination:{kind:'skills',skillId,mode:'crafting',recipeId:recipe.id,button:'Open '+recipe.name,detail:'Craft '+recipe.name+' to build recipe mastery.'}});
 }
...
export function professionMasteryActiveBonusText(state:GameState,actionId:string){
 const view=professionMasteryView(actionId,state.account.professionMasteryByAction?.[actionId]),parts:string[]=[];
 if(view.xpBonusBps)parts.push('+'+(view.xpBonusBps/100).toFixed(0)+'% XP');
 if(view.yieldBonusBps)parts.push('+'+(view.yieldBonusBps/100).toFixed(0)+'% yield');
 if(view.speedBonusBps)parts.push('+'+(view.speedBonusBps/100).toFixed(0)+'% speed');
 return parts.length?parts.join(' · '):'No mastery bonus yet';
}