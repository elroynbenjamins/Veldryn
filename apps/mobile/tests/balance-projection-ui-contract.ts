...
ok(prepareRoute.includes('PREPARE MATERIALS')&&prepareRoute.includes('route.chainLabel')&&prepareRoute.includes('numberOfLines={1}'),'Prepare Materials must keep the compact source chain visible while the disclosure stays one-line bounded');
ok(prepareRoute.includes('recipePreparationRouteLabel(route)')&&acquisitionPlan.includes('preparationEtaSeconds')&&acquisitionPlan.includes('preparationBottleneck'),'Collapsed preparation metadata must surface authoritative prep time and bottleneck without expanding the card');
...
ok(prepareRoute.includes('bottleneck={step.id===route.bottleneck?.stepId}')&&prepareRoute.includes('BOTTLENECK'),'Expanded route rows must identify the same bottleneck surfaced in the collapsed summary');
