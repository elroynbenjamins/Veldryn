...
async function main(){
 const state=createCharacter(newGame(1000),'IRONWARDEN','Entry Test');
 ...
 const handler=coopEntryHandler({authenticate:async token=>token==='valid'?'owned-account':null,rpc:async<T>(name:string,args:Record<string,unknown>):Promise<T>=>{
  ...
  if(name==='load_online_game_server_v1')return {state,version:7,serverNow:1000} as T;
 ...
 assert.equal(projection.dungeons[0].available,false);assert.equal(projection.dungeons[4].available,false);
