...
export interface CoopRunView {runId:string;mode:CoopMode;phase:string;syncedLevel:number;roleSlots:Array<{role:'tank'|'damage'|'support';name:string;echo:boolean;ready?:boolean}>;options:CoopRouteOptionView[];rewardText?:string;}
export function validateCoopRunView(view:CoopRunView):void{
 ...
 if(view.options.length>0&&view.options.length<3)throw new Error('insufficient_route_options');
