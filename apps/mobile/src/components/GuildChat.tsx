...
export function GuildChat({language,currentPlayerName,firstUnreadMessageId,onRead}:{language:Language;currentPlayerName?:string;firstUnreadMessageId?:string;onRead?:()=>void}){
 ...
 return <View style={s.root}>
  ...
  <View style={s.compose}><TextInput accessibilityLabel="Guild message" value={body} onChangeText={setBody} onSubmitEditing={()=>void send()} maxLength={300} placeholder={ot(language,'chat.placeholder')} style={s.input}/><ChatEmotePicker onPick={token=>setBody(value=>(value+token).slice(0,300))}/><View style={s.send}><GameButton compact title={busy?'…':ot(language,'chat.send')} disabled={busy||!body.trim()||!guild} onPress={()=>void send()}/></View></View>
