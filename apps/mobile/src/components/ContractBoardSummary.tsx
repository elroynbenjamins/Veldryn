...
export function ContractBoardSummary({state,nowMs,onOpen,onContinue}:{state:GameState;nowMs:number;onOpen:()=>void;onContinue?:(order:NonNullable<ReturnType<typeof contractBoardSummary>['nextOrder']>)=>void}){
 ...
 return <View style={[s.card,summary.pendingRewards>0&&s.ready]}>
  ...
  {next&&onContinue?<View style={s.actions}><View style={s.primary}><GameButton compact title="Continue job" onPress={()=>onContinue(next)}/></View><View style={s.secondary}><GameButton compact title="Board" tone="secondary" onPress={onOpen}/></View></View>:<GameButton compact title={summary.pendingRewards>0?'Open Contract Board rewards':'Open Contract Board'} tone={summary.pendingRewards>0?'primary':'secondary'} onPress={onOpen}/>}
 </View>;
}
