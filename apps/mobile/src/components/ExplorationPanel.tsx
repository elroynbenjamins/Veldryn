...
export function ExplorationPanel({state,onCommand}:{state:GameState;onCommand:(command:GameCommand)=>Promise<void>}){
 ...
 const regionId=currentRegionId(state),region=WORLD_ZONES.find(item=>item.id===regionId),exploration=state.skills.find(skill=>skill.skillId==='exploration')?.level??1;
 ...
 return <Panel>
  <View style={s.heading}><View style={s.flex}><Text style={s.eyebrow}>SCOUTING ROUTES</Text><Text style={s.title}>Explore the world</Text><Text style={s.body}>Scouting is the fastest way to train Exploration, reveal encounters, and discover routes into later regions. Combat also grants a smaller amount of passive Exploration XP while you learn the area.</Text></View><View style={s.regionBadge}><Text style={s.regionBadgeLabel}>CURRENT</Text><Text numberOfLines={2} style={s.regionBadgeValue}>{region?.name??regionId}</Text></View></View>
  {routes.map(route=>{const zone=WORLD_ZONES.find(item=>item.id===route.zoneId),nextZone=route.unlockZoneId?WORLD_ZONES.find(item=>item.id===route.unlockZoneId):undefined,here=regionId===route.zoneId,characterReady=c.level>=route.requiredLevel,explorationReady=exploration>=route.requiredExplorationLevel,ready=here&&characterReady&&explorationReady,completed=(state.exploredRouteIds??[]).includes(route.id),found=!!route.unlockMonsterId&&state.unlockedMonsterIds.includes(route.unlockMonsterId);const stateLabel=completed?'MAPPED':!characterReady?`CHAR LV ${route.requiredLevel}`:!explorationReady?`EXP LV ${route.requiredExplorationLevel}`:!here?'TRAVEL':'READY';return <View key={route.id} style={[s.route,ready&&s.routeReady]}>
   <View style={s.routeTop}><View style={s.flex}><View style={s.inline}><Text style={s.routeName}>{route.name}</Text><Text style={[s.tag,ready?s.tagReady:completed?s.tagFound:s.tagLocked]}>{stateLabel}</Text></View><Text style={s.routeMeta}>{zone?.name??route.zoneId} · {route.seconds}s · {route.xp} Exploration XP · Exp Lv {route.requiredExplorationLevel}</Text>{route.unlockMonsterId?<Text style={s.reward}>{found?'Encounter revealed':'Reveals a regional encounter'}</Text>:null}{nextZone?<Text style={s.reward}>{completed?`Route to ${nextZone.name} discovered`:`Discovers the route to ${nextZone.name}`}</Text>:null}</View></View>
   <GameButton compact title={completed&&ready?'Scout again':ready?'Scout':!characterReady?`Requires character level ${route.requiredLevel}`:!explorationReady?`Requires Exploration level ${route.requiredExplorationLevel}`:`Travel to ${zone?.name??route.zoneId}`} disabled={!ready} tone={ready?'primary':'secondary'} onPress={()=>void onCommand({type:'explore',args:{id:route.id}})}/>
  </View>})}
 </Panel>;
}
