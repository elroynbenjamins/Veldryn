...
export function WorkingTowardSummary({state,onOpen,onNavigate}:{state:GameState;onOpen:()=>void;onNavigate:(destination:WorkingTowardDestination)=>void}){
 ...
 const context=progressionGoalContext(state),views=goals.map(goal=>{if(goal.kind==='recipe_preparation'){const tracked=recipePreparationTrackingView(state,goal);return {goal,view:tracked,destination:tracked.destination,nextLabel:tracked.nextLabel};}return {goal,view:progressionGoalView(goal,context),destination:progressionGoalDestination(state,goal),nextLabel:undefined};});
