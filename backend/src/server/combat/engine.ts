...
export function simulateCombat(input: CombatInput): CombatResult {
  ...
  const applyDamage=(now:number, source:CombatantState, target:CombatantState, effect:AbilityEffect, abilityId:string, eventType:'damage'|'dot_tick'='damage')=>{
    ...
    const mit=effect.damageType==='true'?0:defenseMitigation(target.definition.stats.defense,mitigationConstant);
  ...
  const applyEffect=(now:number, source:CombatantState,target:CombatantState,effect:AbilityEffect,abilityId:string)=>{
    ...
    if(effect.kind==='buff'||effect.kind==='debuff'){target.modifiers.push({sourceId:source.definition.id,tag:effect.tag??'generic',value:effect.value??0,expiresAt:now+(effect.durationMs??5000)});return;}
  };
  ...
  for(let now=0; now<=maxMs; now+=tick){
    ...
    for(const actor of all){ if(!actor.alive)continue; const allies=actor.definition.team==='players'?players:enemies, foes=actor.definition.team==='players'?enemies:players;
      ...
      const ab=ready[0];
      if(ab){ const ts=targetsFor(ab.target,actor,allies,foes,rng,`${now}:${ab.id}:select`); const t=ts[0]; if(t){ actor.cooldownReadyAt[ab.id]=now+Math.round(ab.cooldownMs/Math.max(.25,1+actor.definition.stats.haste)); if(ab.castTimeMs>0){actor.casting={abilityId:ab.id,completesAt:now+ab.castTimeMs,targetId:t.definition.id};events.push({atMs:now,type:'cast_start',actorId:actor.definition.id,targetId:t.definition.id,abilityId:ab.id});}else{for(const fx of ab.effects){for(const x of targetsFor(ab.target,actor,allies,foes,rng,`${now}:${ab.id}:instant`))applyEffect(now,actor,x,fx,ab.id);}} continue; } }
      if(actor.nextBasicAt<=now){ const t=chooseEnemy(actor,foes,rng,`${now}:${actor.definition.id}:basic`); if(t){applyDamage(now,actor,t,{kind:'damage',coeff:actor.definition.basicAttackCoeff,damageType:'physical',threatMultiplier:actor.definition.role==='tank'?2.5:1},'BASIC');actor.nextBasicAt=now+Math.round(actor.definition.basicAttackMs/Math.max(.25,1+actor.definition.stats.haste));} }
    }
  }
