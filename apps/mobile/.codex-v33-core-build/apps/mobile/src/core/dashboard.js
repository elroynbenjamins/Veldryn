...
function activityCycleSeconds(state) {
    ...
    const monster = monsters_1.MONSTERS.find(m => m.id === target), gathering = [...skills_1.GATHERING, ...herbalism_1.HERB_NODES].find(g => g.id === target);
    const brew = state.activity?.kind === 'alchemy' ? (0, alchemy_1.alchemyRecipeDef)(target ?? '') : undefined;
    const route = state.activity?.kind === 'exploration' ? (0, exploration_1.explorationRoute)(target ?? '') : undefined;
    if (brew)
        return brew.seconds;
    if (route)
    ...
    return monster.secondsPerKill * COMBAT_TIME_SCALE * environmentMultiplier / (speed * (0, combat_companions_1.companionCombatContribution)(state).outputMultiplier * (1 + (0, monster_mastery_1.monsterMastery)(state, monster.id).damageBonus));
}
