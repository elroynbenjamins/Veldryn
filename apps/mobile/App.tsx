...
const next=discoverCharacterSkins(candidate);stateRef.current=next;setState(next);queueCustomizationUnlocks(current,next);try{await repo.save(next)}catch{Alert.alert('Local save failed','Progress is still in memory. Keep the app open and try another action to save again.')}}
  ...
  <ChatOverlay state={state} visible={showChatOverlay} onOpen={()=>setShowChatOverlay(true)} onClose={()=>setShowChatOverlay(false)} guildUnread={notificationCounts.guildChatUnread} guildMentions={notificationCounts.guildChatMentions} partyUnread={notificationCounts.partyChatUnread} partyMentions={notificationCounts.partyChatMentions} onChatRead={()=>void refreshSocialNotifications()}/>
