...
export function itemInspectModel(state:GameState,itemId:string){
  ...
  if(item.type==='gear'){
    ...
    if(state.character&&item.slot){
      ...
      const gems=enhancement.gemIds.map(id=>{const gem=itemDef(id),kind=gemSocketKind(id);return {id,name:gem.name,kind,detail:kind==='stat'?`+${Math.round((gem.gemPercent??0)*100)}% ${title(gem.gemStat??'stat')}`:gemEffectDescription(id)};});
