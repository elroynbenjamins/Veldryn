...
export function HomeScreen({state,preview,onClaim,onStop,onNavigate,onOpenCombat,onOpenSkill}:{state:GameState;preview:RewardBundle;onClaim:()=>void;onStop:()=>void;onNavigate:(tab:DashboardDestination|'Events',zoneId?:string)=>void;onOpenCombat:()=>void;onOpenSkill:(skillId:SkillId)=>void}){
 ...
 return <ScrollView contentContainerStyle={s.root}>
  ...
  {state.activity&&activityName?<ActivityCard title={activityName} kind={state.activity.kind==='combat'?'combat':'gathering'} cycleSeconds={cycle} capHours={afk.hours} preview={preview} rates={rate} reduceMotion={state.settings.reduceMotion} numberMode={state.settings.numberMode} onClaim={onClaim} onStop={onStop}/>:<Panel><Text style={s.title}>Your next adventure</Text><Text style={s.small}>Choose a hunt or gathering activity to start earning.</Text><GameButton title="Explore activities" onPress={onOpenCombat}/></Panel>}
  {completed>0&&<GameButton compact title={`Claim ${completed} completed quest${completed===1?'':'s'}`} onPress={()=>onNavigate('Quests')}/>}
  <View style={[s.guide,guide.priority==='urgent'&&s.urgent]}><Text style={s.kicker}>{guide.priority==='urgent'?'ATTENTION':'NEXT STEP'}</Text><Text style={s.title}>{guide.title}</Text><Text style={s.small}>{guide.detail}</Text><GameButton compact title={guide.button} tone={guide.priority==='urgent'?'primary':'secondary'} onPress={()=>onNavigate(guide.destination,guide.zoneId)}/></View>
  <View style={s.campaignStrip}><View style={s.flex}><Text style={s.kicker}>ASTERFALL CAMPAIGN · CHAPTER {campaign.chapter}/{campaign.total}</Text><Text numberOfLines={1} style={s.campaignTitle}>{campaign.currentTitle}</Text><View style={s.campaignTrack}><View style={[s.campaignFill,{width:`${Math.max(2,campaign.campaignPct)}%`}]}/></View><Text style={s.campaignMeta}>{campaign.claimed}/{campaign.total} claimed{campaign.ready?` · ${campaign.ready} reward${campaign.ready===1?'':'s'} ready`:''} · Fallen Knight {campaign.bossDefeated?'defeated':`Lv. ${campaign.level}/25`}</Text></View><GameButton compact title="Journal" tone="secondary" onPress={()=>onNavigate('Quests')}/></View>
  ...
  <View style={s.row}><View style={s.flex}><GameButton compact title="Explore world" tone="secondary" onPress={()=>onNavigate('World')}/></View><View style={s.flex}><GameButton compact title="Equipment & food" tone="secondary" onPress={()=>onNavigate('Inventory')}/></View></View>
...
const s=StyleSheet.create({root:{padding:spacing.md,gap:10,paddingBottom:spacing.lg},identity:{flexDirection:'row',alignItems:'center',gap:10,padding:10,borderWidth:1,borderColor:C.line,borderRadius:radii.md,backgroundColor:C.panel},portrait:{width:56,height:70},flex:{flex:1,minWidth:0},name:{...typography.hero,color:C.text,fontSize:23,lineHeight:30},small:{...typography.body,color:C.muted},gold:{...typography.bodyStrong,color:C.accent},title:{...typography.title,color:C.text},kicker:{...typography.caption,color:C.accent,letterSpacing:.8,fontWeight:'600'},guide:{gap:6,padding:10,borderWidth:1,borderColor:C.line,borderRadius:radii.md,backgroundColor:C.panel},urgent:{borderLeftWidth:4,borderLeftColor:C.warning,paddingLeft:spacing.md},row:{flexDirection:'row',flexWrap:'wrap',alignItems:'center',gap:12},disclosure:{minHeight:48,flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:12,paddingVertical:8},link:{...typography.bodyStrong,color:C.info,flex:1},expanded:{gap:12},event:{padding:12,gap:7,backgroundColor:C.panel,borderRadius:radii.md,borderLeftWidth:3},campaignStrip:{flexDirection:'row',alignItems:'center',gap:10,padding:spacing.sm,borderWidth:1,borderColor:C.line,borderRadius:radii.md,backgroundColor:C.panel},campaignTitle:{...typography.bodyStrong,color:C.text},campaignTrack:{height:7,marginVertical:5,borderRadius:4,overflow:'hidden',backgroundColor:C.bg},campaignFill:{height:'100%',backgroundColor:C.accent},campaignMeta:{fontSize:10,lineHeight:14,color:C.muted},earned:{...typography.body,color:C.good}});
