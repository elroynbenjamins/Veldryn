...
export function MoreScreen({language,onNavigate,onOpenChatPilot,onOpenAdminQa,companionAttention=false,workingTowardAttention=false,dailySuppliesAttention=false,eventAttention=false,friendRequestCount=0,guildAttentionCount=0,socialAttentionCount=0,profileAttention=false,companionsUnlocked=true,petsUnlocked=true}:{language:GameState['settings']['language'];onNavigate:(destination:MoreDestination)=>void;onOpenChatPilot?:()=>void;onOpenAdminQa?:()=>void;companionAttention?:boolean;workingTowardAttention?:boolean;dailySuppliesAttention?:boolean;eventAttention?:boolean;friendRequestCount?:number;guildAttentionCount?:number;socialAttentionCount?:number;profileAttention?:boolean;companionsUnlocked?:boolean;petsUnlocked?:boolean}){
  ...
  const attentionLabel=(id:MoreDestination)=>{
    ...
    if(id==='Companions'&&companionsUnlocked&&companionAttention)return 'companion actions ready';
  ...
  return <ScrollView contentContainerStyle={s.root}>
    ...
    {sections.map(section=><View key={section.label} style={s.section}>
      ...
      <View style={s.grid}>{section.items.map(id=>{const baseMeta=itemMeta(language,id),meta=id==='Companions'&&!companionsUnlocked?{...baseMeta,description:'Unlocks after Into Ironwood · Level 10'}:id==='Collections'&&!petsUnlocked?{...baseMeta,description:'Profile collectibles available · Pets unlock after First Blood, First Skill'}:baseMeta,alert=attentionLabel(id),featureLocked=id==='Companions'&&!companionsUnlocked,inDevelopment=id==='Arena',disabled=inDevelopment||featureLocked;return <Pressable key={id} accessibilityRole="button" accessibilityState={{disabled}} accessibilityLabel={inDevelopment?`${meta.title}, In Development`:featureLocked?`${meta.title}, Locked. ${meta.description}`:alert?`${meta.title}, ${alert}`:meta.title} accessibilityHint={meta.description} disabled={disabled} onPress={()=>onNavigate(id)} style={({pressed})=>[s.tile,alert&&s.tileAttention,disabled&&s.tileDisabled,singleColumn&&s.tileWide,pressed&&!disabled&&s.pressed]}>
        ...
        <View style={s.titleRow}><Text numberOfLines={singleColumn?2:1} style={s.title}>{meta.title}</Text>{inDevelopment?<View style={s.developmentPill}><Text style={s.developmentPillText}>IN DEVELOPMENT</Text></View>:featureLocked?<View style={s.developmentPill}><Text style={s.developmentPillText}>🔒 LOCKED</Text></View>:null}</View>
