...
equal(rows[0]?.months.join(','),'12,1','Turning maps to the December / January crossover');
equal(rows[1]?.eventId,'EVT_ANNUAL_002_2026','Heartbond follows Turning');
equal(rows.find(row=>row.eventId==='EVT_ANNUAL_003_2026')?.months.join(','),'3,4','Bloomwake spans March and April');
equal(rows.find(row=>row.eventId==='EVT_ANNUAL_006_2026')?.months.join(','),'6,7','Suncrest spans June and July');
