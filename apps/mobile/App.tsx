...
import {ThemeProvider,THEMES,normalizeThemeId} from './src/theme/app-theme';
...
function tabLabel(language:Language,tab:Tab):string{
  switch(tab){
    case 'Home':return t(language,'nav.home');case 'World':return t(language,'nav.world');case 'Character':return t(language,'nav.character');case 'Inventory':return t(language,'nav.inventory');case 'More':return 'Account';case 'Activity':return 'Character Activity';
...
function VeldrynApp(){
  ...
  const themeId=normalizeThemeId(state?.settings.themeId),appTheme=THEMES[themeId];
...
const next=discoverCharacterSkins(candidate),newSkins=newlyUnlockedCharacterSkins(state,next);stateRef.current=next;setState(next);try{await repo.save(next)}catch{Alert.alert('Local save failed','Progress is still in memory. Keep the app open and try another action to save again.')}if(newSkins.length)Alert.alert(ot(next.settings.language,'skin.unlockTitle'),ot(next.settings.language,'skin.unlockBody',{names:newSkins.map(skin=>skin.name).join(', ')}))}
  ...
  if(loadError||!state)return <ThemeProvider themeId={themeId}><SafeAreaView style={[s.safe,{backgroundColor:appTheme.bg}]}><StatusBar style={appTheme.dark?'light':'dark'}/><SaveRecoveryScreen language={recoveryLanguage} message={loadError||'No readable save state was returned.'} onRetry={()=>void loadGame()} onStartFresh={()=>Alert.alert('Delete unreadable local save?','This permanently removes the existing local data and starts a new game.',[{text:'Cancel'},{text:'Start fresh',style:'destructive',onPress:async()=>{await repo.reset();setState(newGame(Date.now()));setLoadError('');setCurrentTab('Home');setTabHistory([])}}])}/></SafeAreaView></ThemeProvider>;
  if(!state.character)return <ThemeProvider themeId={themeId}><SafeAreaView style={[s.safe,{backgroundColor:appTheme.bg}]}><StatusBar style={appTheme.dark?'light':'dark'}/><ClassSelectScreen language={state.settings.language} onLanguage={language=>commit({...state,settings:{...state.settings,language}})} onSelect={async(id,name,body)=>{if(serverGameplayEnabled){await perform({type:'create',args:{classId:id,name,body}});return;}const next=createCharacter(state,id,name,body);await repo.save(next);setState(next)}}/></SafeAreaView></ThemeProvider>;
  if(creatingRoster)return <ThemeProvider themeId={themeId}><SafeAreaView style={[s.safe,{backgroundColor:appTheme.bg}]}><StatusBar style={appTheme.dark?'light':'dark'}/><ClassSelectScreen language={state.settings.language} cancelLabel={t(state.settings.language,'roster.cancel')} onCancel={()=>setCreatingRoster(false)} onSelect={async(id,name,body)=>{if(serverGameplayEnabled){const result=await perform({type:'roster_create',args:{classId:id,name,body}});if(result)setCreatingRoster(false);return;}const result=executeGameCommand(state,{type:'roster_create',args:{classId:id,name,body}},Date.now());await commit(result.state);setCreatingRoster(false)}}/></SafeAreaView></ThemeProvider>;
  ...
  const primaryBadges:Partial<Record<PrimaryTab,number|'dot'>>={
    ...
    Skills:navigationBadgeModel.skills.dot?'dot':0,
  ...
  return <ThemeProvider themeId={themeId}><SafeAreaView style={[s.safe,{backgroundColor:appTheme.bg}]}><StatusBar style={appTheme.dark?'light':'dark'}/><GameTopBar state={state} nowMs={now} labelForDestination={destination=>tabLabel(state.settings.language,destination)} onNavigate={setTab} onChangeDestinations={destinations=>commit({...state,settings:{...state.settings,quickNavDestinations:destinations}})} onOpenActivity={openActiveActivity}/>{secondary&&tab!=='Coop'&&<View style={s.backBar}><Pressable accessibilityRole="button" accessibilityLabel={t(state.settings.language,'common.back')} onPress={goBack} hitSlop={8} style={({pressed})=>[s.backButton,pressed&&s.backPressed]}><Image source={require('./assets/ui-icons-v2/small/back.png')} resizeMode="contain" style={s.backIcon}/><Text style={s.backText}>{t(state.settings.language,'common.back')}</Text></Pressable><Text numberOfLines={1} style={s.backTitle}>{tabLabel(state.settings.language,tab)}</Text><View style={s.backSpacer}/></View>}{serverGameplayEnabled&&<View style={{paddingHorizontal:12}}>{!!online.error&&<Text accessibilityRole="alert" style={s.txt}>{online.error}</Text>}{online.pending?<GameButton title="Retry pending action" disabled={online.busy} onPress={()=>void perform()}/>:<Text style={s.txt}>{online.busy?'Saving online…':'Online · progress saved on server'}</Text>}</View>}<View pointerEvents={serverGameplayEnabled&&online.busy?'none':'auto'} style={s.body} {...backSwipe.panHandlers}>
    ...
    {tab==='Skills'&&<SkillsScreen key={`${skillsMode}:${selectedSkill??'all'}`} state={state} initialMode={skillsMode} initialSkill={selectedSkill} onSelectSkill={id=>{if(id==='companion'||id.startsWith('class:')){setTab('Companions');return;}setSelectedSkill(id as any);setSkillsMode(id==='faith'?'faith':['mining','woodcutting','fishing','herbalism'].includes(id)?'gathering':'crafting')}} onCommand={runCompanionCommand} onCharacter={()=>setTab('Character')} onInventory={()=>setTab('Inventory')} onViewToolRecipes={()=>{setSelectedSkill('smithing');setSkillsMode('crafting')}} onGather={id=>changeActivity({kind:'gathering',id})} onQueueGather={id=>queueActivity({kind:'gathering',id})} onQueueRemove={index=>mutateActionQueue({type:'queue_remove',args:{index}})} onQueueClear={()=>mutateActionQueue({type:'queue_clear'})} onQueueStart={()=>mutateActionQueue({type:'queue_start'},true)} onEquipTool={id=>{if(serverGameplayEnabled){void perform({type:'equip_tool',args:{id}});return;}try{const toolKind=itemDef(id).toolSkillId;const settled=state.activity?.kind===toolKind?claimActivity(state,Date.now()):{state,reward:null};commit(equipGatheringTool(settled.state,id));if(settled.reward)presentCollected(settled.reward,state.activity)}catch(error){Alert.alert('Cannot equip tool',error instanceof Error?error.message:'Please try again.')}}} onCraft={id=>{if(serverGameplayEnabled){void perform({type:'craft',args:{id}}).then(result=>{if(result)Alert.alert('Craft complete','Your crafted items have been saved online.');});return;}try{const next=craftRecipe(state,id);const completed=!noviceSetProgress(state).unlocked&&noviceSetProgress(next).unlocked;commit(next);Alert.alert(ot(state.settings.language,completed?'craft.setComplete':'craft.complete'),completed?`${noviceSetProgress(next).set.name} is fully crafted and ready to equip. Equipment changes stats; skins are chosen separately on Character.`:'Your crafted items were added to Inventory, or Bank if Inventory was full. Equip them from Inventory or use Equip owned novice set on Character.',[{text:'Continue'},{text:'View character',onPress:()=>setTab('Character')}])}catch(error){Alert.alert(ot(state.settings.language,'craft.cannot'),error instanceof Error?error.message:'Please try again.')}}}/>}
    {tab==='Inventory'&&<InventoryScreen state={state} onEquip={id=>serverGameplayEnabled?void perform({type:'equip',args:{id}}):void commit(equipItem(state,id))} onFood={id=>serverGameplayEnabled?void perform({type:'food',args:{id}}):void commit(equipFood(state,id))} onEat={id=>serverGameplayEnabled?void perform({type:'eat',args:{id}}):void commit(eatFood(state,id))} onSell={id=>serverGameplayEnabled?void perform({type:'sell',args:{id}}):void commit(sellItem(state,id))} onSalvage={id=>serverGameplayEnabled?void perform({type:'salvage',args:{id}}):void commit(salvageItem(state,id))} onDeposit={(id,quantity)=>serverGameplayEnabled?void perform({type:'deposit',args:{id,quantity}}):void commit(depositToBank(state,id,quantity))} onDepositMaterials={()=>serverGameplayEnabled?void perform({type:'deposit_materials'}):void commit(depositAllMaterials(state))} onUpgradeStorage={location=>serverGameplayEnabled?void perform({type:'storage',args:{location}}):void commit(upgradeStorage(state,location))} onWithdraw={(id,quantity)=>serverGameplayEnabled?void perform({type:'withdraw',args:{id,quantity}}):void commit(withdrawFromBank(state,id,quantity))} onOverflow={()=>serverGameplayEnabled?void perform({type:'overflow'}):void commit(claimOverflowToBank(state))}/>}
    {tab==='Companions'&&<CompanionsScreen state={state} language={state.settings.language} now={now} onCommand={runCompanionCommand} onCreate={()=>setCreatingRoster(true)} onNavigateSource={source=>{if(source.kind==='combat'){const currentZone=WORLD_ZONES.find(zone=>zone.id===state.currentRegionId)?.name;setTab(currentZone===source.zoneName?'Combat':'World');return;}if(source.kind==='crafting'){setSelectedSkill(source.skillId as SkillId);setSkillsMode('crafting');setTab('Skills');return;}if(source.kind==='gathering'){setSelectedSkill(source.skillId as SkillId);setSkillsMode('gathering');setTab('Skills');return;}if(source.kind==='events')setTab('Events')}}/>}
