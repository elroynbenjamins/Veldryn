...
export function RecipePreparationRoute({
  ...
  return <View style={s.box}>
    <Pressable accessibilityRole="button" accessibilityState={{expanded}} accessibilityLabel={(expanded?'Hide':'Show')+' prepare materials route for '+recipe.name} onPress={()=>setExpanded(value=>!value)} style={({pressed})=>[s.head,expanded&&s.headOpen,pressed&&s.pressed]}>
      <View style={s.copy}><Text style={s.eyebrow}>PREPARE MATERIALS</Text><Text numberOfLines={1} ellipsizeMode="tail" style={s.title}>{route.chainLabel}</Text><Text numberOfLines={1} ellipsizeMode="tail" style={s.sub}>{recipePreparationRouteLabel(route)}</Text></View>
      <Text style={s.mark}>{expanded?'−':'+'}</Text>
    </Pressable>
    {expanded?<View style={s.steps}>
      {route.steps.map((step,index)=><PreparationStepRow key={step.id} step={step} index={index} total={route.steps.length} bottleneck={step.id===route.bottleneck?.stepId} onNavigate={onNavigate}/>)}
...
function PreparationStepRow({step,index,total,bottleneck,onNavigate}:{step:RecipePreparationStep;index:number;total:number;bottleneck:boolean;onNavigate?:(destination:WorkingTowardDestination)=>void}){
  ...
  const content=<>
    ...
    <View style={s.copy}><View style={s.labelRow}><Text style={s.stepLabel}>{step.label}</Text><View style={s.tags}><Text style={[s.state,{color:tone}]}>{step.stateLabel}</Text>{bottleneck?<Text style={s.bottleneck}>BOTTLENECK</Text>:null}</View></View><Text style={s.stepDetail}>{step.detail}</Text></View>
...
function makeStyles(C:ThemeColors){return StyleSheet.create({
  ...
  numberText:{fontSize:9,lineHeight:11,fontWeight:'900'},labelRow:{flexDirection:'row',alignItems:'center',gap:6},tags:{flexDirection:'row',alignItems:'center',gap:5},
  stepLabel:{...typography.caption,color:C.text,fontWeight:'900',flex:1},state:{fontSize:7.5,lineHeight:10,fontWeight:'900',letterSpacing:.5},bottleneck:{fontSize:7,lineHeight:9,color:C.warning,fontWeight:'900',letterSpacing:.45},
