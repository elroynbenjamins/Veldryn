...
export function OnlineProfileExtensionPanel({state,onSaved}:{state:GameState;onSaved?:()=>void|Promise<void>}){
 ...
 return <><Panel>
  <View style={s.heading}><View style={s.flex}><Text style={s.eyebrow}>ONLINE PROFILE</Text><Text style={s.title}>Identity, Privacy & Showcase</Text></View><Text style={s.revision}>r{value.revision}</Text></View>
  ...
  <GameButton title={busy?'Saving…':'Save Profile Settings'} disabled={guest||busy} onPress={()=>void save()}/>{notice?<Text accessibilityLiveRegion="polite" style={s.notice}>{notice}</Text>:null}{error?<Text accessibilityRole="alert" style={s.error}>{error}</Text>:null}
 </Panel>
