...
exports.OFFLINE_CAP_SECONDS = exports.MAX_OFFLINE_CAP_HOURS = exports.BASE_OFFLINE_CAP_HOURS = void 0;
...
function createCharacter(state, classId, name = 'Adventurer', bodyPresentation = 'male', customization) {
...
function updateCharacterCustomization(state, customization) {
    if (!state.character)
        throw new Error('Create a character first');
    return { ...state, character: { ...state.character, customization: (0, customization_1.normalizeCustomization)(customization) } };
}
...
function itemStackCap(itemId) { const d = (0, items_1.itemDef)(itemId); return d.type === 'gear' ? 1 : 9999; }
...
function previewActivityReward(state, nowMs) {
    ...
    for (const drop of m.drops) {
        ...
        for (let i = 0; i < sim.kills; i++)
            if ((0, rng_1.random01)(seed, i) < drop.chance)
...
function sellItem(state, itemId, quantity = 1) { if (!state.character || quantity <= 0)
    return state; const d = (0, items_1.itemDef)(itemId); return { ...state, inventory: { ...state.inventory, stacks: consume(state.inventory.stacks, itemId, quantity) }, character: { ...state.character, gold: state.character.gold + d.value * quantity } }; }
function salvageItem(state, itemId) { const d = (0, items_1.itemDef)(itemId); if (d.type !== 'gear' || !d.salvage)
    throw new Error('Cannot salvage'); return { ...state, inventory: { ...state.inventory, stacks: stackItems(consume(state.inventory.stacks, itemId, 1), [d.salvage]) } }; }
...
function startGathering(state, targetId, nowMs) { const g = skills_1.GATHERING.find(x => x.id === targetId); if (!g)
    ...
    throw new Error('Skill level too low'); return { ...state, activity: { kind: g.skillId, targetId, startedAtMs: nowMs, lastClaimAtMs: nowMs } }; }
