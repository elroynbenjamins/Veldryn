...
const fields = {
    ...
    create: ['classId', 'name', 'body'], claim: [], start: ['kind', 'id'], stop: [], travel: ['id'], boss: [], craft: ['id'],
    ...
    switch (command.type) {
        ...
        case 'companion_upgrade': {
            ...
            if (id === 'trainingGround')
                state.account.companionSanctuary.lastTrainingClaimAtMs = now;
            if (id === 'essenceBasin')
                state.account.companionSanctuary.lastEssenceClaimAtMs = now;
            break;
        }
        case 'companion_training':
            state = companions.claimSanctuaryTraining(state, now);
            break;
