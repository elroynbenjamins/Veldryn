...
export function companionTrialResetInfo(serverNowMs:number){const seasonKey=companionTrialSeasonKey(serverNowMs),def=companionTrialSeasonDefinition(seasonKey),endMs=Date.parse(def.endsAt),remainingMs=Math.max(0,endMs-serverNowMs);return {seasonKey,serverNow:new Date(serverNowMs).toISOString(),startsAt:def.startsAt,endsAt:def.endsAt,remainingMs,timezone:COMPANION_TRIAL_TIMEZONE,rotationId:def.rotationId,rotationName:def.rotationName,title:`Companion Trials — ${def.rotationName} · ${new Intl.DateTimeFormat('en',{month:'long',year:'numeric',timeZone:'UTC'}).format(new Date(serverNowMs))}`,notice:'One global Trial rotation for all players. Trial progress resets each month; Companion progression does not.'};}
