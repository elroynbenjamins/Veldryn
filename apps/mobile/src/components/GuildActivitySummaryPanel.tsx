...
export function GuildActivitySummaryPanel({compact=false}:{compact?:boolean}){
 ...
 return <Panel>
  ...
  <View accessibilityRole="progressbar" accessibilityValue={{min:0,max:100,now:state.activityPercent}} style={s.track}><View style={[s.fill,{width:(Math.min(100,state.activityPercent)+'%') as any}]}/>{reserve>0?<View style={[s.reserve,{width:(reserve*10+'%') as any}]}/>:null}</View>
...
function makeStyles(C:ThemeColors){return StyleSheet.create({
 ...
 track:{height:9,borderRadius:5,overflow:'hidden',backgroundColor:C.panel2,marginVertical:8,position:'relative'},fill:{height:'100%',backgroundColor:C.accent},reserve:{position:'absolute',right:0,top:0,bottom:0,backgroundColor:C.info},reserveText:{fontSize:8.5,color:C.info,fontWeight:'900',marginTop:3},
