   ...
   <Modal visible={visible} transparent animationType="slide" onRequestClose={()=>setVisible(false)}>
     <View style={s.backdrop}><View style={s.sheet}>
       ...
       <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
         {section('postType','Post Type',value.postTypes.map(labelForType).join(', '),<View style={s.options}>{(['looking_for_party','party_recruiting','looking_for_guild','guild_recruiting'] as RecruitmentPostType[]).map(type=><GameButton key={type} title={`${value.postTypes.includes(type)?'✓ ':''}${labelForType(type)}`} tone="secondary" onPress={()=>update({postTypes:toggle(value.postTypes,type)})}/>)}</View>)}
