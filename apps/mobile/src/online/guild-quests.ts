...
export interface OnlineGuildQuest{
 ...
 category:GuildQuestCategory;description:string;progress:number;target:number;activityReward:number;completed:boolean;contributorCount:number;
}
...
export async function loadOnlineGuildQuests():Promise<OnlineGuildQuest[]>{
 ...
 return ((data??[]) as Record<string,unknown>[]).map(row=>({
  ...
  description:String(row.description??''),progress:num(row.progress),target:Math.max(1,num(row.target)),activityReward:num(row.activity_reward),
