...
export function WorkingTowardSummary({state,onOpen,onNavigate}:{state:GameState;onOpen:()=>void;onNavigate:(destination:WorkingTowardDestination)=>void}){
 ...
 const context=progressionGoalContext(state),views=goals.map(goal=>{if(goal.kind==='recipe_preparation'){const tracked=recipePreparationTrackingView(state,goal);return {goal,view:tracked,destination:tracked.destination,nextLabel:tracked.nextLabel,progressLabel:tracked.status==='complete'?tracked.stepLabel+' · crafted':tracked.stepLabel+' · '+Math.round(tracked.progress*100)+'% prepared'};}const view=progressionGoalView(goal,context);return {goal,view,destination:progressionGoalDestination(state,goal),nextLabel:undefined,progressLabel:Math.floor(view.current).toLocaleString()+' / '+Math.floor(view.target).toLocaleString()+' · '+Math.round(view.progress*100)+'%'};});
 ...
 return <View style={s.card}>
  ...
  {views.map(({goal,view,destination,nextLabel,progressLabel})=><View key={goal.id} style={[s.goal,view.status==='complete'&&s.goalDone,view.status==='blocked'&&s.goalBlocked]}><View style={s.goalTop}><View style={s.flex}><Text numberOfLines={1} style={s.goalName}>{goal.title}</Text><Text style={s.meta}>{progressLabel}</Text></View><Text style={view.status==='complete'?s.done:view.status==='blocked'?s.blocked:s.active}>{view.status==='complete'?'DONE':view.status==='blocked'?'BLOCKED':'ACTIVE'}</Text></View><View style={s.track}><View style={[s.fill,view.status==='complete'&&s.fillDone,{width:(Math.max(2,view.progress*100)+'%') as any}]}/></View>{view.status!=='complete'&&destination.kind!=='info'?<Pressable accessibilityRole="button" onPress={()=>onNavigate(destination)} style={s.continue}><View style={s.flex}><Text numberOfLines={1} style={s.continueText}>{destination.button}</Text>{nextLabel?<Text numberOfLines={1} style={s.nextMeta}>{'Next · '+nextLabel}</Text>:null}</View><Text style={s.arrow}>→</Text></Pressable>:view.status==='complete'?<Text style={s.reward}>Goal reached · clear or replace it in Working Toward.</Text>:<Text style={s.reward}>{view.blocker??destination.detail}</Text>}</View>)}
 </View>;
}
