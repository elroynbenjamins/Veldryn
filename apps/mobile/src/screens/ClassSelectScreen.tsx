 ...
 return <KeyboardAvoidingView style={s.screen} behavior={Platform.OS==='ios'?'padding':'height'}>
  <ScrollView ref={scroll} style={s.scroll} contentContainerStyle={s.root} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
   <View pointerEvents={saving?'none':'auto'}>
    ...
    {!onCancel&&onSignInExisting?<Pressable accessibilityRole="button" disabled={saving} onPress={()=>void onSignInExisting()} style={s.existingAccount}><Text style={s.existingAccountText}>Already played VELDRYN? Sign in</Text></Pressable>:null}
...
function makeStyles(C:ThemeColors){const E=equipmentTheme(C);return StyleSheet.create({
 ...
 existingAccount:{minHeight:40,alignSelf:'center',justifyContent:'center',paddingHorizontal:12,marginTop:-4,marginBottom:4},existingAccountText:{fontSize:12,lineHeight:18,color:C.info,fontWeight:'700'},
