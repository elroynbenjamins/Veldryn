...
ok(qty(copperBlade,'COPPER_INGOT')<=12&&qty(copperBlade,'GREENWOOD_LOG')<=24,'Starter crafted gear must stay session-friendly');
ok(qty(asterChest,'ASTER_IRON_INGOT')<=55&&qty(asterChest,'IRONWOOD_LOG')<=80&&qty(asterChest,'REINFORCED_FITTING')<=5,'Mid-tier chest material costs must stay progression-scale rather than chase-scale');
ok(qty(oathWard,'OATHSTONE_INGOT')<=28&&qty(oathWard,'CROWNWOOD_LOG')<=48&&qty(oathWard,'OATHGLASS_SHARD')<=8,'Oathstone progression gear must remain demanding but session-scale');
