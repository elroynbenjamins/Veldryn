...
import {CharacterPortrait,CharacterVisual} from '../components/CharacterVisual';
...
export function HomeScreen({state,preview,onClaim,onStop,onNavigate}:{state:GameState;preview:RewardBundle;onClaim:()=>void;onStop:()=>void;onNavigate:(tab:DashboardDestination|'Events',zoneId?:string)=>void}){
  ...
  return <ScrollView contentContainerStyle={s.root}>
    <Text style={s.kicker}>ASTERFALL · LOCAL SAVE</Text><View style={s.hero}><CharacterPortrait state={state} compact/><View style={s.flex}><Text style={s.name}>{c.name}</Text><Text style={s.small}>{c.classId.replace('_',' ')} · Level {c.level}</Text><Text style={s.gold}>◈ {formatGameNumber(c.gold,state.settings.numberMode)}</Text></View></View><StatBar label="Experience" current={p.current} max={p.need}/>
