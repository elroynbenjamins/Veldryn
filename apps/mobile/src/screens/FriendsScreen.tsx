...
function Avatar({name}:{name:string}){const C=useGameTheme(),s=useMemo(()=>makeStyles(C),[C]);return <View style={s.avatar}><Text style={s.avatarText}>{name.trim().slice(0,1).toUpperCase()||'?'}</Text></View>}
