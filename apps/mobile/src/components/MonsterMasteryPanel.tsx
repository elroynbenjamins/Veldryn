...
export function MonsterMasteryPanel({state}:{state:GameState}){
 ...
 return <Panel>
  ...
  <View style={s.summary}><Summary label="TRACKED" value={summary.species}/><Summary label="RANKS" value={summary.totalRanks}/><Summary label="RANK 20" value={summary.rank20}/><Summary label="CHALLENGES" value={summary.challengeTiers}/></View>
  ...
  {open&&<><View style={s.ladder}><Text style={s.ladderTitle}>MASTERY LADDER</Text>{MONSTER_MASTERY_MILESTONES.map(row=><Text key={row.rank+row.label} style={s.ladderRow}>Rank {row.rank} · {row.label}</Text>)}</View>
   {available.map(m=>{const p=monsterMasteryGuidance(state,m.id),rankProgress=p.rank>=30?25:p.points%25;return <View key={m.id} style={s.entry}>
    <View style={s.head}><View style={s.flex}><Text style={s.name}>{p.badgeUnlocked?'◆ ':''}{m.name}</Text><Text style={s.rank}>Rank {p.rank}/30 · {p.points} mastery points</Text></View>{p.rank>=30?<Text style={s.max}>MAX</Text>:null}</View>
    ...
    <View style={s.challengeRow}>{p.challengeUnlocks.map(challenge=><View key={challenge.id} style={[s.challenge,challenge.unlocked?s.challengeOn:s.challengeOff]}><Text style={[s.challengeText,challenge.unlocked?s.challengeTextOn:s.challengeTextOff]}>{challenge.unlocked?'✓':'○'} {challenge.def.shortName} · R{challenge.def.masteryRank}</Text></View>)}</View>
...
const s=StyleSheet.create({
 ...
 entry:{gap:8,paddingVertical:12,borderBottomWidth:1,borderBottomColor:C.line},head:{flexDirection:'row',alignItems:'center',gap:8},flex:{flex:1,minWidth:0},max:{...typography.caption,color:C.good,fontWeight:'900'},
 ...
 challengeRow:{flexDirection:'row',flexWrap:'wrap',gap:5},challenge:{paddingHorizontal:7,paddingVertical:4,borderWidth:1,borderRadius:99},challengeOn:{borderColor:C.good,backgroundColor:'#14251f'},challengeOff:{borderColor:C.line,backgroundColor:C.bg},challengeText:{fontSize:9,fontWeight:'900'},challengeTextOn:{color:C.good},challengeTextOff:{color:C.muted},
