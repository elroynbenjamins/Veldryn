...
function effectAnchor(actorIsParty:boolean|undefined,targetIsParty:boolean|undefined):'28%'|'50%'|'72%'{
 if(actorIsParty===true&&targetIsParty===true)return '72%';
 if(actorIsParty===false&&targetIsParty===false)return '28%';
 return '50%';
}
...
export function DungeonCombatStage({run,enemyLabel,boss=false}:{run:CoopRunView;enemyLabel?:string;boss?:boolean}){
 ...
 const actorStyle=!reduceMotion&&fx?{transform:[
  ...
  {translateX:actorAnim.interpolate({inputRange:[0,.44,1],outputRange:[0,fx.actorMotion==='smash'?2:0,0]})},
  {scale:actorAnim.interpolate({inputRange:[0,.44,1],outputRange:[1,motionScale(fx),1]})},
 ]}:undefined;
 const targetStyle=!reduceMotion&&fx?{transform:[
  {translateX:targetAnim.interpolate({inputRange:[0,.28,.45,.62,1],outputRange:[0,0,fx.targetMotion==='shake'?fx.targetShakePx:0,fx.targetMotion==='shake'?-fx.targetShakePx:0,0]})},
  {translateY:targetAnim.interpolate({inputRange:[0,.48,1],outputRange:[0,fx.targetMotion==='brace'?2*targetDirection:0,0]})},
 ...
 const effectStyle=fx?{top:effectAnchor(actorIsParty,targetIsParty),opacity:reduceMotion?1:fxAnim,transform:[
 ...
 return <FantasyPanel variant={boss?'danger':'selected'}>
  ...
  <View style={s.arena}>
   <View style={[s.enemyZone,boss&&s.bossZone]}>
    <EnemyCombatProfileCard name={shownEnemy} boss={boss} active={enemyActive} targeted={enemyTargeted} currentCue={currentCue} motionStyle={enemyActive?actorStyle:enemyTargeted?targetStyle:undefined}/>
   </View>
   <View style={s.battleLane}><View style={s.laneLine}/><Text style={s.vs}>{currentCue?.type==='cast'?'CASTING':currentCue?.type==='phase'?'PHASE':currentCue?.type==='interrupt'?'INTERRUPT':'VS'}</Text><View style={s.laneLine}/></View>
   <View style={s.partyField}>{ordered.map((slot,index)=>{const active=Boolean(currentCue?.actorId&&slot.memberId===currentCue.actorId),isTarget=Boolean(currentCue?.targetId&&slot.memberId===currentCue.targetId),targeted=!active&&isTarget,assistProc=active&&currentCue?.type==='assist';return <View key={slot.memberId??`${slot.name}-${index}`} style={s.formationSlot}><CombatantProfileCard compact slot={slot} active={active} targeted={targeted} assistProc={assistProc} currentCue={currentCue} motionStyle={active?actorStyle:isTarget?targetStyle:undefined}/></View>;})}</View>
  ...
  </View>:<Text style={s.note}>Bosses and enemies occupy the upper battle card; the four fixed class profile cards stay anchored along the bottom with owner-bound companion assists.</Text>}
 </FantasyPanel>;
}
...
const s=StyleSheet.create({
 ...
 arena:{position:'relative',minHeight:430,gap:coopSpacing.xs,padding:coopSpacing.sm,borderWidth:1,borderColor:coopColors.goldDim,borderRadius:coopRadii.tile,backgroundColor:'#04111E',overflow:'hidden'},
 enemyZone:{width:'58%',maxWidth:248,alignSelf:'center',zIndex:2},bossZone:{width:'68%',maxWidth:286},
 battleLane:{height:24,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:6,zIndex:2},laneLine:{flex:1,height:StyleSheet.hairlineWidth,backgroundColor:coopColors.goldDim},vs:{fontSize:8,lineHeight:10,color:coopColors.gold,fontWeight:'900',letterSpacing:.8},
 partyField:{flexDirection:'row',alignItems:'flex-end',justifyContent:'space-between',gap:3,zIndex:2},
 formationSlot:{flex:1,minWidth:0},
 fxLayer:{...StyleSheet.absoluteFillObject,zIndex:4},
 fxMark:{position:'absolute',left:'50%',width:94,minHeight:48,marginLeft:-47,marginTop:-24,alignItems:'center',justifyContent:'center',paddingHorizontal:4,borderWidth:1,borderRadius:coopRadii.tile,backgroundColor:'rgba(4,17,30,.88)',shadowOpacity:.65,shadowRadius:8,elevation:5},
 ...
 castHalo:{position:'absolute',left:'50%',top:44,width:132,height:132,marginLeft:-66,borderWidth:2,borderRadius:999,backgroundColor:'transparent'},
