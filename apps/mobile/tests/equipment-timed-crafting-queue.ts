...
ok(tailorClaimed.state.skills.find(row=>row.skillId==='tailoring')!.xp===tailorBefore+tailoringRecipe.xp,'Tailoring-class V33 craft must award Tailoring XP on completion');
ok(tailorClaimed.state.skills.find(row=>row.skillId==='smithing')!.xp===smithBeforeTailor,'Tailoring-class V33 craft must not award Smithing XP');
ok(tailorClaimed.instance.ownerCharacterId===tailor.character!.id,'Tailoring-class crafted gear must remain owned by its qualifying crafter');
