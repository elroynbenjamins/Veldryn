...
function trialBossProfile(floor:number,theme:CompanionTrialEncounterTheme):TrialEnemyProfile{
 ...
 const abilities:CompanionCombatantDefinition['abilities'][]=[
  [
 ...
 const selected=abilities[Math.min(abilities.length-1,bossIndex-1)];
 return {name:theme.bossName,hp:1,attack:1,defense:1,haste:bossIndex>=5?.015:0,basicMs:2200,basicCoeff:.82,tags:[`trial_boss_theme:${theme.id}`],abilities:selected};
}
