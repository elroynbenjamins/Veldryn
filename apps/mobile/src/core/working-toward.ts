...
export function workingTowardSourceAvailability(state:GameState,source:WorkingTowardDestination):GoalSource|undefined{
 if(source.kind==='combat'){
  ...
  const available=!!monster&&state.unlockedMonsterIds.includes(monster.id);
  const levelReady=!!monster&&(state.character?.level??1)>=monster.unlockLevel;
  return {kind:'monster',id:source.monsterId,label:monster?.name??source.monsterId,available,reason:available?undefined:levelReady?'Discover this encounter through regional Exploration.':`Requires Level ${monster?.unlockLevel??'?'}.`};
 }
 ...
 if(source.kind==='world'){const region=WORLD_ZONES.find(row=>row.id===source.regionId),available=!!region&&regionTravelAvailability(state,region)==='available';return {kind:'region',id:source.regionId,label:region?.name??source.regionId,available,reason:available?undefined:region?regionTravelLockReason(state,region):'Unknown region.'};}
