...
export function WorldScreen({state,onTravel,onOpenCombat,onOpenSkills,onCoop,onRegionalRewardsChanged,onOpenWeeklyOrder,onOpenContracts,goalRegionId}:Props){
  ...
  const next=nextRegionUnlock(state);
  ...
  return <ScrollView contentContainerStyle={s.root}>
    ...
    <View style={s.unlockCard}><View style={s.unlockHead}><View style={s.flex}><Text style={s.unlockLabel}>{next?'NEXT REGION UNLOCK':'REGION PROGRESSION'}</Text><Text style={s.unlockTitle}>{next?next.name:'All authored regions unlocked'}</Text></View>{next?<Text style={s.unlockLevel}>Lv {level}/{next.minLevel}</Text>:<Text style={s.unlockDone}>COMPLETE</Text>}</View>{next?<><View style={s.unlockTrack}><View style={[s.unlockFill,{width:(nextUnlockProgress+'%') as any}]}/></View><Text style={s.unlockMeta}>{level<next.minLevel?`${Math.max(0,next.minLevel-level)} level${next.minLevel-level===1?'':'s'} until the level gate.`:nextLockReason||'Ready to travel.'}</Text></>:<Text style={s.unlockMeta}>Every currently authored region can be travelled to.</Text>}</View>
    {travelRegions.map(zone=>{
      ...
      return <View key={zone.id} style={[s.destination,goalTarget&&s.goalDestination,inDevelopment&&s.developmentDestination]}>
        <View style={s.thumbnail}><ZoneSceneArtwork regionId={zone.id} muted={!unlocked}/>{!unlocked&&!inDevelopment&&<View style={s.lockedTag}><Text numberOfLines={2} style={s.lockedText}>{level<zone.minLevel?`Lv. ${zone.minLevel}`:'Explore'}</Text></View>}</View>
        <View style={s.flex}>
