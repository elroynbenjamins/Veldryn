...
begin
  ...
  if p_board='profession_total' then
    ...
    select coalesce(jsonb_agg(jsonb_build_object('rank',row_number,'entityType','account','displayName',display_name,'value',value,'isSelf',account_id=p_requester) order by value desc,display_name), '[]'::jsonb) into v_rows
    from (select row_number() over(order by t.value desc,coalesce(nullif(pp.display_name,''),'Adventurer')) row_number, t.account_id,t.value,coalesce(nullif(pp.display_name,''),'Adventurer') display_name from totals t join public.player_profiles pp on pp.account_id=t.account_id where pp.profile_visibility='public' and not exists(select 1 from public.player_blocks b where (b.blocker_id=p_requester and b.blocked_id=t.account_id) or (b.blocker_id=t.account_id and b.blocked_id=p_requester)) order by t.value desc,display_name limit p_limit offset p_offset) q;
    ...
    if v_season<>'' then
      execute format('select coalesce(jsonb_agg(jsonb_build_object(''rank'',row_number,''entityType'',''account'',''displayName'',display_name,''value'',value,''isSelf'',account_id=$1) order by value desc,display_name),''[]''::jsonb) from (select row_number() over(order by a.%I desc,coalesce(nullif(pp.display_name,''''),''Adventurer'')) row_number,a.account_id,a.%I value,coalesce(nullif(pp.display_name,''''),''Adventurer'') display_name from public.squad_arena_season_accounts a join public.player_profiles pp on pp.account_id=a.account_id where a.season_id=$2 and pp.profile_visibility=''public'' and not exists(select 1 from public.player_blocks b where (b.blocker_id=$1 and b.blocked_id=a.account_id) or (b.blocked_id=$1 and b.blocker_id=a.account_id)) order by a.%I desc,display_name limit $3 offset $4) q',case when p_board='arena_wins' then 'wins' else 'rating' end,case when p_board='arena_wins' then 'wins' else 'rating' end,case when p_board='arena_wins' then 'wins' else 'rating' end) using p_requester,v_season,p_limit,p_offset into v_rows;
    end if;
  elsif p_board='guild' then
    select coalesce(jsonb_agg(jsonb_build_object('rank',row_number,'entityType','guild','displayName',name,'value',level,'secondaryValue',xp,'isSelf',false) order by level desc,xp desc,name), '[]'::jsonb) into v_rows from (select row_number() over(order by g.level desc,g.xp desc,g.name) row_number,g.name,g.level,g.xp from public.guilds g order by g.level desc,g.xp desc,g.name limit p_limit offset p_offset) q;
  end if;
