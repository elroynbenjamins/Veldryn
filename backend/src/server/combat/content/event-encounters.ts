...
const encounter=(id:string,name:string,level:number,scale:number,damageType:'fire'|'arcane'):CombatantDefinition[]=>[
 {id:`${id}_A`,name,team:'enemies',role:'enemy',level,stats:stats(6_500*scale,2_800*scale,850*scale,level),basicAttackMs:2750,basicAttackCoeff:.74,abilities:[{id:`${id}_A_HIT`,name:'Event Strike',cooldownMs:6800,castTimeMs:700,target:'current_target',priority:70,effects:[{kind:'damage',coeff:1.02,damageType}]}]},
