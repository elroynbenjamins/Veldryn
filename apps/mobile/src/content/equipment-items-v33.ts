...
const salvageMaterialByTier:Record<string,string>={T1:'MOSS_FIBER',T2:'WOLF_PELT',T3:'TROLL_HIDE',T4:'OATHGLASS_SHARD',T5:'SUNSTONE_ORE',T6:'AMBERGLASS',T7:'FROSTIRON',T8:'RIMEGLASS',T9:'CHOIR_BLOOM'};
const salvageQuantityBySlot:Record<GearSlot,number>={helmet:2,chest:3,gloves:1,legs:2,boots:1,weapon:3,offhand:2,cape:2,amulet:1,ring:1};
...
export const EQUIPMENT_ITEMS_V33:ItemDef[]=(catalog.pieces as Array<Record<string,string|number>>).map(piece=>{
  ...
  return {
    ...
    salvage:{itemId:salvageMaterialByTier[tier]??'MOSS_FIBER',quantity:salvageQuantityBySlot[slot]},
  };
});
