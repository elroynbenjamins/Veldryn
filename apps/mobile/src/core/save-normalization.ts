...
import {normalizeGearInstances} from './crafted-gear-instances';
...
export function normalizeSave(input:any):GameState{
  ...
  const character=input.character?(()=>{
    ...
    equippedGearInstanceIds:Object.fromEntries(Object.entries(input.character.equippedGearInstanceIds&&typeof input.character.equippedGearInstanceIds==='object'?input.character.equippedGearInstanceIds:{}).filter(([slot,id])=>['weapon','offhand','helmet','chest','legs','boots','gloves','cape','amulet','ring'].includes(slot)&&typeof id==='string')),
