...
export function EnemyCombatProfileCard({name,boss=false,active=false,targeted=false,motionStyle,currentCue}:{name:string;boss?:boolean;active?:boolean;targeted?:boolean;motionStyle?:any;currentCue?:CoopCombatReplayCueView}){
 const C=useGameTheme(),s=useMemo(()=>makeStyles(C),[C]),enemyArt=dungeonEnemyPortraitSource(name),float=currentCue?.type==='action'&&currentCue.targetName===name&&currentCue.amount!==undefined?{label:`${currentCue.actionKind==='heal'?'+':'-'}${Math.round(currentCue.amount)}`,heal:currentCue.actionKind==='heal'}:undefined;
