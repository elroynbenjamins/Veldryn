...
validateCoopEventExpeditionPreview({id:'EVENT_TEST',eventName:'Suncrest Games',name:'The Shattered Isles',description:'A seasonal route.',routeHighlights:['Coastal Ruins','Sun Shrine','Pirate Camp'],enemyNames:['Suncrest Corsair','Shoreline Colossus','Solar Reef Warden'],finalBoss:'Aureon, First Champion',status:'preview'});
let eventFailure='';try{validateCoopEventExpeditionPreview({id:'EVENT_BAD',eventName:'Starfall',name:'Rift',description:'Bad duplicate route.',routeHighlights:['Rift Gate','Rift Gate','Boss Nexus'],enemyNames:['Astral Marauder','Meteoric Sentinel','Riftbound Herald'],finalBoss:'The Constellation Eater',status:'preview'})}catch(error){eventFailure=error instanceof Error?error.message:String(error)}equal(eventFailure,'invalid_event_route_highlights','event previews require three distinct route highlights');
