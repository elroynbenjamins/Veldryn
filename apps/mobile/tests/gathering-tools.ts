...
import {createCharacter,equipGatheringTool,newGame,previewActivityReward,startGathering} from '../src/core/game';
...
ok(Math.abs(gatheringPacing(state,oathstone).timeMultiplier-2.3)<.001,'Gathering without a tool should retain the 15% inefficiency penalty');
...
ok(Math.abs(gatheringPacing(state,oathstone).timeMultiplier-1)<.001,'Tier 3 tool should normalize a two-times late-resource penalty');

state=startGathering(state,'OATHSTONE_SEAM',0);
ok(Math.abs(activityCycleSeconds(state)-oathstone.seconds*1.45)<.001,'Dashboard cycle must use the same equipped-tool pacing as reward settlement');
const reward=previewActivityReward(state,Math.ceil(oathstone.seconds*1.45)*1000);
