...
export function OnlineGuildManagement({onApplicationsChanged}:{onApplicationsChanged?:()=>void}={}){
 ...
 return <View style={s.stack}>{invitePanel}<Panel>
  ...
  {(role==='leader'||role==='officer')?<><View style={s.sectionHead}><Text style={s.section}>PENDING APPLICATIONS</Text><Text style={s.sectionMeta}>{applications.length} waiting</Text></View>{applications.length?applications.map(app=><SocialInvitationCard key={app.id} name={'Applicant '+app.account_id.slice(0,8)} status="GUILD APPLICATION" statusTone="info" detail="Awaiting Guild review. Display identity is not included in the application payload." actions={<><GameButton compact title="Accept" disabled={busy} onPress={()=>void review(app.id,true)}/><GameButton compact title="Decline" tone="secondary" disabled={busy} onPress={()=>void review(app.id,false)}/></>}/>):<Text style={s.empty}>No pending applications.</Text>}</>:null}
...
function makeStyles(C:ThemeColors){return StyleSheet.create({
 ...
 leadership:{padding:9,borderWidth:1,borderColor:C.line,borderRadius:8,backgroundColor:C.panel2},leadershipWarn:{borderColor:C.warning,backgroundColor:C.warningSurface},warningText:{fontSize:10,lineHeight:14,color:C.warning,marginTop:3},departure:{gap:6,marginTop:spacing.sm},departureNote:{fontSize:10,lineHeight:14,color:C.muted},
