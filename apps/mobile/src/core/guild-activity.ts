...
export function guildActivityTargetForMembers(memberCount:number){
 const members=Math.max(1,Math.floor(Number.isFinite(memberCount)?memberCount:1));
 // Target scales to the roster but assumes roughly 60% meaningful participation.
...
export function guildActivityAfterDecay(percent:number,days:number){
 ...
 return Math.max(0,safe-elapsed*GUILD_ACTIVITY_DAILY_DECAY_PERCENT);
}
