...
import {materializeGearInstances,normalizeGearInstances} from './equipment-instances';
...
export function normalizeSave(input:any):GameState{
  ...
  return materializeGearInstances(discoverCharacterSkins(reconcileCombatCompanionUnlocks(normalized,normalized.createdAtMs)));
}
