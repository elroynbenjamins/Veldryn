...
exports.PARTY_SOCIAL_TUTORIAL_STEPS = exports.playStyleLabel = exports.activityPreferenceLabel = exports.EMPTY_RECRUITMENT_FILTERS = exports.eventActivityLabel = void 0;
exports.nextUnreachedMilestone = nextUnreachedMilestone;
exports.milestoneProgressPercent = milestoneProgressPercent;
...
exports.contractProgressPercent = contractProgressPercent;
exports.contractEligibilityLabel = contractEligibilityLabel;
exports.filterRecruitmentCards = filterRecruitmentCards;
exports.eventActivityLabel = { combat: 'Combat', gathering: 'Gathering', processing: 'Processing', crafting: 'Crafting', fishing: 'Fishing', hunting: 'Hunting', alchemy: 'Alchemy', delivery: 'Deliveries' };
function nextUnreachedMilestone(current, milestones) { return milestones.find(m => !m.reached && m.points > current) ?? milestones.find(m => !m.reached); }
function milestoneProgressPercent(current, target) { return target <= 0 ? 100 : Math.max(0, Math.min(100, Math.round((current / target) * 100))); }
...
exports.activityPreferenceLabel = { combat: 'Combat', skilling: 'Skilling', mixed: 'Mixed' };
exports.playStyleLabel = { casual: 'Casual', balanced: 'Balanced', active: 'Active', competitive: 'Competitive' };
function contractProgressPercent(contract) { return Math.max(0, Math.min(100, Math.round(contractProgressRatio(contract) * 100))); }
function contractEligibilityLabel(contract) { if (contract.eligible)
    return 'Reward eligible'; if (contract.complete)
    return 'Contribute more to claim'; return 'Keep contributing'; }
