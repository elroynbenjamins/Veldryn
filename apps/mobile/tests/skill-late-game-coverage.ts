...
ok(EXPLORATION_ROUTES.some(x=>x.zoneId==='ASHLANDS'&&x.requiredLevel>=50),'Exploration must retain a meaningful Ashlands discovery gate');
...
ok(MONSTERS.some(x=>x.zone==='Ashlands'&&x.level>=80),'Combat must retain late-game Ashlands targets');
