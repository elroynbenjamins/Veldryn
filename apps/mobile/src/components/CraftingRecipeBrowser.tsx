...
import {equipmentCraftAvailability,timedEquipmentRecipe} from '../core/equipment-crafting-queue';
...
export function CraftingRecipeBrowser({state,skillId,initialQuery='',preferredRecipeId,onCraft,onAlchemyStart,onProcessingStart,onNavigate,onCraftPrerequisites,onTrackPreparation}:{state:GameState;skillId:CraftingSkillId;initialQuery?:string;preferredRecipeId?:string;onCraft:(id:string)=>void;onAlchemyStart?:(id:string,batches:number)=>Promise<void>|void;onProcessingStart?:(id:string,batches:number)=>Promise<void>|void;onNavigate?:(destination:WorkingTowardDestination)=>void;onCraftPrerequisites?:(recipeId:string)=>void;onTrackPreparation?:(recipe:(typeof RECIPES)[number],batches:number,initialStepCount:number)=>Promise<void>|void}){
