...
import {forgeAnticipation} from '../core/progress-anticipation';
...
export function EquipmentCraftQueuePanel({state,onClaim,onClaimAll,onCancel,onMoveWaiting,forgeResults,onDismissForgeResults}:{state:GameState;onClaim:(jobId:string)=>void;onClaimAll:()=>void;onCancel:(jobId:string)=>void;onMoveWaiting:(jobId:string,direction:'up'|'down')=>void;forgeResults?:readonly ForgeCraftResult[]|null;onDismissForgeResults?:()=>void}){
  ...
  return <View style={s.card}>
    ...
    {activeJobs.length>0&&<View style={s.group}><Text style={s.groupLabel}>CRAFTING NOW</Text>{activeJobs.map(job=>{const progress=Math.max(2,Math.min(100,100-(job.remainingSeconds/job.durationSeconds)*100)),anticipation=forgeAnticipation(job.remainingSeconds,job.durationSeconds),near=anticipation.band!=='none';return <View key={job.id} style={[s.job,near&&s.jobNear]}>{job.outputItemId?<ItemArtwork itemId={job.outputItemId} size={44}/>:null}<View style={s.flex}><Text style={s.jobName}>{job.name}</Text><Text style={[s.meta,near&&s.nearText]}>{near?(anticipation.band==='urgent'?'ALMOST DONE · ':'FINISHING SOON · '):''}{duration(job.remainingSeconds)} remaining · {duration(job.durationSeconds)} craft</Text><View style={s.track}><View style={[s.fill,near&&s.fillNear,{width:(progress+'%') as `${number}%`}]}/></View></View><View style={s.jobActions}><Text style={[s.running,near&&s.runningNear]}>{near?'SOON':'CRAFTING'}</Text><GameButton compact title="Cancel" tone="secondary" onPress={()=>onCancel(job.id)}/></View></View>})}</View>}
...
function makeStyles(C:ThemeColors){const E=equipmentTheme(C);return StyleSheet.create({
  ...
  job:{minHeight:64,flexDirection:'row',alignItems:'center',gap:spacing.sm,padding:spacing.sm,borderWidth:1,borderColor:C.line,borderRadius:radii.sm,backgroundColor:C.panel2},jobReady:{borderColor:C.good,backgroundColor:C.goodSurface},jobWaiting:{borderColor:C.info,backgroundColor:C.infoSurface},jobNear:{borderColor:C.good,backgroundColor:C.goodSurface},jobName:{...typography.bodyStrong,color:C.text},jobActions:{alignItems:'flex-end',gap:4},running:{...typography.caption,color:E.goldSoft,fontWeight:'900'},runningNear:{color:C.good},nearText:{color:C.good,fontWeight:'900'},
  track:{height:5,marginTop:5,borderRadius:99,backgroundColor:C.bg,overflow:'hidden'},fill:{height:'100%',backgroundColor:E.selectedLine},fillNear:{backgroundColor:C.good},empty:{...typography.body,color:C.muted,paddingVertical:4},cancelNote:{fontSize:9,lineHeight:13,color:C.muted},
