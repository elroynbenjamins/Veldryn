...
const legacy={...base,version:6 as const,character:{...base.character!,equipment:{...base.character!.equipment,ring:'STONEHEART_RING'},gearEnhancements:{STONEHEART_RING:{rank:4,failures:2,gemIds:['WARD_SHARD','WARD_SHARD','EMBER_SHARD']}}}};
const migrated=parseSaveBackup(JSON.stringify(legacy));
const legacyEnhancement=gearEnhancement(migrated,'STONEHEART_RING');
...
const typed={...migrated,character:{...migrated.character!,gearEnhancements:{...migrated.character!.gearEnhancements,STONEHEART_RING:{rank:4,failures:2,statGemId:'WARD_SHARD',effectGemId:'GEFF_001',legacyGemIds:['EMBER_SHARD'],gemIds:['WARD_SHARD','GEFF_001','EMBER_SHARD']}}}};
const roundTrip=parseSaveBackup(createSaveBackup(typed,new Date('2026-09-21T00:00:00Z')));
const typedEnhancement=gearEnhancement(roundTrip,'STONEHEART_RING');
