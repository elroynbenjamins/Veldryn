 ...
 return <KeyboardAvoidingView style={s.screen} behavior={Platform.OS==='ios'?'padding':'height'}>
  <ScrollView ref={scroll} style={s.scroll} contentContainerStyle={s.root} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
   <View pointerEvents={saving?'none':'auto'}>
    ...
    {!onCancel&&onSignInExisting?<Pressable accessibilityRole="button" disabled={saving} onPress={onSignInExisting} style={s.existingAccount}><Text style={[s.existingAccountText,{color:C.info}]}>Already played VELDRYN? Sign in to your existing account</Text></Pressable>:null}
...
function makeStyles(C:ThemeColors){const E=equipmentTheme(C);return StyleSheet.create({
 screen:{flex:1,backgroundColor:C.bg},scroll:{flex:1},root:{width:'100%',maxWidth:520,alignSelf:'center',padding:16,paddingTop:12,paddingBottom:20},existingAccount:{alignSelf:'center',minHeight:40,justifyContent:'center',paddingHorizontal:12,marginTop:4},existingAccountText:{fontSize:12,fontWeight:'700',textAlign:'center',textDecorationLine:'underline'},brandRow:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:8},wordmark:{flex:1,maxWidth:184,height:62},kicker:{fontSize:10,lineHeight:14,color:C.muted,fontWeight:'800',letterSpacing:2,textAlign:'center',marginVertical:8},languageToggle:{minHeight:44,paddingHorizontal:8,flexDirection:'row',alignItems:'center',gap:8},languageCurrent:{fontSize:12,color:C.text,fontWeight:'700',flexShrink:1},languageMark:{fontSize:22,color:C.accent},languageGrid:{flexDirection:'row',flexWrap:'wrap',gap:8,marginVertical:8},languageChip:{minWidth:96,minHeight:44,justifyContent:'center',padding:10,borderWidth:1,borderColor:C.line,borderRadius:16},
