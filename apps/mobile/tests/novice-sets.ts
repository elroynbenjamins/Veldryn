...
import {noviceSetProgress,resolveCharacterAppearance} from '../src/core/character-appearance';
...
ok(NOVICE_SETS.length===9&&NOVICE_ITEMS.length===60&&NOVICE_RECIPES.length===60,'Nine complete sets and sixty unique piece recipes');
...
for(const set of NOVICE_SETS){
  for(const body of ['male','female'] as const){
    ...
    ok(resolveCharacterAppearance(initial)==='starting','New character has starting appearance');
    ...
    let state:GameState={...initial,character:{...initial.character!,level:4,gold:1000},bank:{...initial.bank,stacks:[{itemId:'COPPER_ORE',quantity:200},{itemId:'GREENWOOD_LOG',quantity:200},{itemId:'MOSS_FIBER',quantity:100}]}};
    ...
    const other=NOVICE_SETS.find(s=>s.classId!==set.classId)!;
    ...
    for(const slot of set.slots){
      ...
      if(slot==='chest'){
        const partial=equipItem(state,id);
        ok(resolveCharacterAppearance(partial)==='mixed','Single armor piece never shows full set');
      }
    }
    const cost=NOVICE_RECIPES.filter(r=>r.classId===set.classId).reduce((sum,r)=>sum+r.gold,0);
    ...
    ok(resolveCharacterAppearance(state)==='starting','Crafting alone does not equip');
    const equipped=equipNoviceSet(state);
    ok(resolveCharacterAppearance(equipped)==='first-crafted','Equipped set selects completed artwork');
    ...
    ok(count(equipped,CLASSES.find(c=>c.id===set.classId)!.starterEquipment.weapon)===1,'Starting weapon preserved in storage');
    ...
    ok(resolveCharacterAppearance(loaded)==='first-crafted'&&noviceSetProgress(loaded).unlocked,'Save/load preserves equipment and milestone');
    ok(loaded.character!.bodyPresentation===body,'Gender survives save/load');
    const removed=unequipItem(equipped,'helmet');
    ok(resolveCharacterAppearance(removed)==='mixed','Removing a piece removes full-set visual');
    ...
    ok(resolveCharacterAppearance(incompatible)==='mixed','Additional cape cannot silently use complete outfit');
    const restored=equipNoviceSet(incompatible);
    ok(resolveCharacterAppearance(restored)==='first-crafted'&&count(restored,'OATHGLASS_CAPE')===1,'Set equip stores incompatible cape');
    ...
    const missing={...state,inventory:{...state.inventory,stacks:state.inventory.stacks.filter(s=>s.itemId!==noviceItemId(set.classId,'helmet'))}};
    ...
    const bankOnly={...state,bank:{...state.bank,stacks:[...state.bank.stacks,...state.inventory.stacks.filter(s=>itemDef(s.itemId).noviceSetId)]},inventory:{...state.inventory,stacks:state.inventory.stacks.filter(s=>!itemDef(s.itemId).noviceSetId)}};
    ok(resolveCharacterAppearance(equipNoviceSet(bankOnly))==='first-crafted','Equip-all consumes owned Bank pieces');
    const cramped:GameState={...equipped,character:{...equipped.character!,equipment:{...equipped.character!.equipment,weapon:CLASSES.find(c=>c.id===set.classId)!.starterEquipment.weapon,cape:'OATHGLASS_CAPE'}},inventory:{stacks:[],capacity:0},bank:{stacks:[{itemId:noviceItemId(set.classId,'weapon'),quantity:1}],capacity:1}};
