...
export function executeGameCommand(previous:GameState,value:unknown,now:number,options:{characterId?:string;randomRoll?:number;accountId?:string;eventId?:string}={}):GameCommandResult{
 ...
 switch(command.type){
  ...
  case 'equip':state=game.equipItem(state,text(a,'id',180));break;
  ...
  case 'sell':state=game.sellItem(state,text(a,'id',180),a.quantity===undefined?1:integer(a,'quantity'));break;
  case 'salvage':state=game.salvageItem(state,text(a,'id',180));break;
  ...
  case 'upgrade':{if(options.randomRoll===undefined)throw new Error('trusted_random_required');const result=attemptEquipmentUpgrade(state,text(a,'id',180),options.randomRoll);state=result.state;upgrade=result.result;break;}
  case 'socket':state=socketGem(state,text(a,'id',180),text(a,'gemId'));break;
  case 'unsocket':state=unsocketGem(state,text(a,'id',180),integer(a,'index',0,1));break;
