...
type Props={
  ...
  lockedDestinations?:Partial<Record<QuickNavDestination,string>>;
};

export function GameTopBar({state,nowMs,labelForDestination,onNavigate,onChangeDestinations,onOpenActivity,attention={},lockedDestinations={}}:Props){
    ...
    <GameModalSurface visible={open} reduceMotion={state.settings.reduceMotion} onClose={close} backdropLabel="Close quick navigation" surfaceStyle={styles.sheet}>
          ...
          {customizing?<>
            ...
            <ScrollView style={styles.choiceScroll} contentContainerStyle={styles.choiceList}>
              {orderedChoices.map(destination=>{const isSelected=selected.has(destination),lockedReason=lockedDestinations[destination],blocked=!isSelected&&(draft.length>=5||!!lockedReason);return <Pressable key={destination} accessibilityRole="checkbox" accessibilityState={{checked:isSelected,disabled:blocked}} accessibilityLabel={lockedReason?labelForDestination(destination)+', locked, '+lockedReason:labelForDestination(destination)} onPress={()=>toggle(destination)} disabled={blocked} style={({pressed})=>[styles.choice,isSelected&&styles.choiceSelected,blocked&&styles.choiceBlocked,pressed&&styles.pressed]}>
                <Image source={destinationIcons[destination]} style={[styles.choiceIcon,!isSelected&&styles.choiceIconDim]} resizeMode="contain"/><Text style={styles.choiceText}>{labelForDestination(destination)}</Text>{lockedReason?<Text style={styles.lockedTag}>LOCKED</Text>:null}<View style={[styles.check,isSelected&&styles.checkSelected]}><Text style={styles.checkText}>{isSelected?'✓':''}</Text></View>
              </Pressable>})}
            </ScrollView>
            ...
            <View style={styles.shortcutList}>{active.map((destination,index)=>{const badge=attention[destination],lockedReason=lockedDestinations[destination];return <Pressable key={destination} accessibilityRole="button" accessibilityLabel={lockedReason?labelForDestination(destination)+', locked, '+lockedReason:badge?.dot?labelForDestination(destination)+', needs attention':labelForDestination(destination)} onPress={()=>{close();onNavigate(destination)}} style={({pressed})=>[styles.shortcut,lockedReason&&styles.shortcutLocked,pressed&&styles.pressed]}><View style={styles.shortcutNumber}><Text style={styles.shortcutNumberText}>{index+1}</Text></View><Image source={destinationIcons[destination]} style={styles.shortcutIcon} resizeMode="contain"/><Text style={styles.shortcutText}>{labelForDestination(destination)}</Text>{lockedReason?<Text style={styles.lockedTag}>LOCKED</Text>:null}{badge?.dot?<View style={styles.shortcutBadge}><Text style={styles.shortcutBadgeText}>{badge.display??'•'}</Text></View>:null}<Text style={styles.chevron}>›</Text></Pressable>})}</View>
...
function makeStyles(C:ThemeColors){const equipmentColors=equipmentTheme(C);return StyleSheet.create({
  ...
  shortcutList:{gap:7},shortcut:{minHeight:52,flexDirection:'row',alignItems:'center',gap:10,paddingHorizontal:10,backgroundColor:equipmentColors.panel,borderWidth:1,borderColor:equipmentColors.line,borderRadius:8},shortcutLocked:{opacity:.62,borderStyle:'dashed'},shortcutNumber:{width:26,height:26,alignItems:'center',justifyContent:'center',backgroundColor:equipmentColors.selected,borderWidth:1,borderColor:equipmentColors.selectedLine,borderRadius:6},shortcutNumberText:{color:C.text,fontWeight:'900'},shortcutIcon:{width:28,height:28},shortcutText:{flex:1,color:C.text,fontSize:15,fontWeight:'800'},shortcutBadge:{minWidth:20,height:20,paddingHorizontal:5,alignItems:'center',justifyContent:'center',borderRadius:10,backgroundColor:C.notification},shortcutBadgeText:{fontSize:8,lineHeight:10,color:C.notificationText,fontWeight:'900'},chevron:{color:equipmentColors.goldSoft,fontSize:26},attentionSection:{gap:6,marginTop:12,paddingTop:10,borderTopWidth:1,borderTopColor:C.line},attentionLabel:{fontSize:9,color:C.warning,fontWeight:'900',letterSpacing:.8},attentionRow:{minHeight:46,flexDirection:'row',alignItems:'center',gap:9,paddingHorizontal:10,borderWidth:1,borderColor:C.warning,borderRadius:8,backgroundColor:C.warningSurface},attentionIcon:{width:24,height:24},attentionText:{flex:1,color:C.text,fontSize:14,fontWeight:'800'},attentionMore:{fontSize:9,lineHeight:13,color:C.muted,fontWeight:'700'},customizeButton:{marginTop:12},
  choiceScroll:{maxHeight:410},choiceList:{gap:6,paddingBottom:8},choice:{minHeight:48,flexDirection:'row',alignItems:'center',gap:10,paddingHorizontal:11,backgroundColor:equipmentColors.panel,borderWidth:1,borderColor:C.line,borderRadius:7},choiceSelected:{backgroundColor:equipmentColors.selected,borderColor:equipmentColors.selectedLine},choiceBlocked:{opacity:.38},choiceIcon:{width:25,height:25},choiceIconDim:{opacity:.65},choiceText:{flex:1,color:C.text,fontSize:14,fontWeight:'800'},lockedTag:{fontSize:7,color:C.muted,fontWeight:'900',letterSpacing:.5,paddingHorizontal:5,paddingVertical:2,borderWidth:1,borderColor:C.line,borderRadius:99},check:{width:24,height:24,alignItems:'center',justifyContent:'center',borderWidth:1,borderColor:C.muted,borderRadius:5},checkSelected:{backgroundColor:equipmentColors.selectedLine,borderColor:equipmentColors.selectedLine},checkText:{color:C.dark?C.bg:C.primaryButtonText,fontWeight:'900'},
