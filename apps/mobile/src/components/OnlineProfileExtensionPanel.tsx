...
export function OnlineProfileExtensionPanel({state,onSaved,onDirtyChange,onDraftChange}:{state:GameState;onSaved?:()=>void|Promise<void>;onDirtyChange?:(dirty:boolean)=>void;onDraftChange?:(draft:ProfileExtensionSelfV43|null)=>void}){
 ...
 const masteries=professionMasteryMasteredRecords(state).map(row=>({id:row.actionId,label:row.name,meta:skillIdentity(row.skillId).label+' · R50'}));
 ...
 return <><Panel>
  ...
  <View style={s.settings}>
   <GameButton title={'Showcase character: '+(selectedCharacter?.name??'Current character')+' ▾'} tone="secondary" disabled={guest||busy||characters.length<2} onPress={()=>openPicker('character')}/>
   <GameButton title={'Favorite skill: '+(selectedSkill?.label??'None')+' ▾'} tone="secondary" disabled={guest||busy} onPress={()=>openPicker('skill')}/>
   <GameButton title={'Favorite companion: '+(selectedCompanion?.label??'None')+' ▾'} tone="secondary" disabled={guest||busy} onPress={()=>openPicker('companion')}/>
  </View>
  ...
  <View style={s.settings}>
   <GameButton title={'Achievements '+value.achievementShowcaseIds.length+'/3 ▾'} tone="secondary" disabled={guest||busy} onPress={()=>openPicker('achievements')}/>
   <GameButton title={'Personal records '+value.recordShowcaseIds.length+'/3 ▾'} tone="secondary" disabled={guest||busy} onPress={()=>openPicker('records')}/>
   <GameButton title={'Collection '+value.collectionShowcase.length+'/3 ▾'} tone="secondary" disabled={guest||busy} onPress={()=>openPicker('collections')}/>
   <GameButton title={'Mastery '+value.masteryShowcaseActionIds.length+'/3 ▾'} tone="secondary" disabled={guest||busy||!masteries.length} onPress={()=>openPicker('mastery')}/>
  </View>
  ...
  <View style={s.settings}><GameButton title={'Visibility: '+visibilityLabel[value.visibility]+' ▾'} tone="secondary" disabled={guest||busy} onPress={()=>openPicker('visibility')}/></View>
 ...
 <GameModalSurface visible={picker!==null} reduceMotion={state.settings.reduceMotion} onClose={()=>setPicker(null)} backdropLabel="Close profile picker" surfaceStyle={s.pickerSheet}><GameModalHeader eyebrow="SOCIAL PROFILE" title={pickerTitle} onClose={()=>setPicker(null)}/><Text style={s.copy}>{multiPicker?pickerCount+'/3 selected · tap a selected item to remove it.':'Choose one option.'}</Text><ScrollView style={s.pickerList} contentContainerStyle={s.pickerContent} keyboardShouldPersistTaps="handled">
