...
export interface PublicQModeRunProjection {
  ...
  options:CoopRouteNode[];visitedNodeIds:string[];resources:number;boons:string[];artifacts:string[];curses:string[];
...
export function projectQModeRun(run:QModeRun):PublicQModeRunProjection{
  ...
  const options=current.nextNodeIds.filter(id=>visible.has(id)).map(id=>graph.nodes.find(node=>node.nodeId===id)!).filter(Boolean);
