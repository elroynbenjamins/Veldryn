...
function tabLabel(language:Language,tab:Tab):string{
  switch(tab){
    case 'Home':return t(language,'nav.home');case 'World':return t(language,'nav.world');case 'Character':return t(language,'nav.character');case 'Inventory':return t(language,'nav.inventory');case 'More':return 'Account';case 'Activity':return 'Character Activity';case 'Progression':return 'Working Toward';case 'DailySupplies':return 'Daily Supplies';case 'AccountBonuses':return 'Account Bonuses';
...
function VeldrynApp(){
  ...
  const [rerollingRoster,setRerollingRoster]=useState<{id:string;confirmation:string}|null>(null);
  ...
  async function runRosterManagement(command:GameCommand){
    if(serverGameplayEnabled)return Boolean(await perform(command));
    try{
      const current=stateRef.current;if(!current)throw new Error('Load your character first.');
      const result=executeGameCommand(current,command,Date.now());
      await commit(result.state);return true;
    }catch(error){Alert.alert('Character management',error instanceof Error?error.message:'The character change could not be completed.');return false;}
  }
...
const next=discoverCharacterSkins(candidate);stateRef.current=next;setState(next);queueCustomizationUnlocks(current,next);try{await repo.save(next)}catch{Alert.alert('Local save failed','Progress is still in memory. Keep the app open and try another action to save again.')}}
  ...
  if(rerollingRoster){const theme=resolveTheme(state.settings.uiTheme);return <GameThemeProvider themeId={state.settings.uiTheme}><SafeAreaView style={[s.safe,{backgroundColor:theme.bg}]}><StatusBar style={theme.dark?'light':'dark'}/><ClassSelectScreen mode="reroll" language={state.settings.language} cancelLabel="Cancel reroll" onCancel={()=>setRerollingRoster(null)} onSelect={async(id,name,body)=>{const ok=await runRosterManagement({type:'roster_reroll',args:{id:rerollingRoster.id,classId:id,name,body,confirmation:rerollingRoster.confirmation}});if(!ok)throw new Error('Reroll was not confirmed.');setRerollingRoster(null);setCurrentTab('Activity');}}/></SafeAreaView></GameThemeProvider>;}
  ...
  return <GameThemeProvider themeId={state.settings.uiTheme}><SafeAreaView style={[s.safe,{backgroundColor:theme.bg}]}><StatusBar style={theme.dark?'light':'dark'}/><GameTopBar state={state} nowMs={now} labelForDestination={destination=>tabLabel(state.settings.language,destination)} onNavigate={setTab} onChangeDestinations={destinations=>commit({...state,settings:{...state.settings,quickNavDestinations:destinations}})} onOpenActivity={openActiveActivity}/>{secondary&&tab!=='Coop'&&<View style={[s.backBar,{backgroundColor:theme.panel,borderColor:theme.line}]}><Pressable accessibilityRole="button" accessibilityLabel={t(state.settings.language,'common.back')} onPress={goBack} hitSlop={8} style={({pressed})=>[s.backButton,pressed&&s.backPressed]}><Image source={require('./assets/ui-icons-v2/small/back.png')} resizeMode="contain" style={s.backIcon}/><Text style={[s.backText,{color:theme.accent}]}>{t(state.settings.language,'common.back')}</Text></Pressable><Text numberOfLines={1} style={[s.backTitle,{color:theme.text}]}>{tabLabel(state.settings.language,tab)}</Text><View style={s.backSpacer}/></View>}{serverGameplayEnabled&&<View style={{paddingHorizontal:12}}>{!!online.error&&<Text accessibilityRole="alert" style={s.txt}>{online.error}</Text>}{online.pending?<GameButton title="Retry pending action" disabled={online.busy} onPress={()=>void perform()}/>:<Text style={s.txt}>{online.busy?'Saving online…':'Online · progress saved on server'}</Text>}</View>}<View pointerEvents={serverGameplayEnabled&&online.busy?'none':'auto'} style={s.body} {...backSwipe.panHandlers}>
    ...
    {tab==='Activity'&&<ActivityOverviewScreen state={state} now={now} onSwitch={id=>{if(serverGameplayEnabled){void perform({type:'roster_switch',args:{id}});return;}void runCompanionCommand({type:'roster_switch',args:{id}})}} onCreate={()=>setCreatingRoster(true)} onReroll={(id,confirmation)=>setRerollingRoster({id,confirmation})} onDelete={(id,confirmation)=>{const target=state.character?.id===id?state.character:state.otherCharacters?.find(entry=>entry.character.id===id)?.character;if(!target)return;Alert.alert(`Delete ${target.name}?`,'Shared Bank and account-wide progress stay intact. This character\'s personal progress, carried items, equipment and upgrades are permanently removed.',[{text:'Cancel',style:'cancel'},{text:'Delete character',style:'destructive',onPress:()=>void runRosterManagement({type:'roster_delete',args:{id,confirmation}})}]);}}/>}
