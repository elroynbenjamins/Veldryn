...
import {earlyFeatureUnlocked,earlyFeatureUnlockProgress} from '../core/feature-unlocks';
...
export function QuestScreen({state,mode,onModeChange,focusedWeeklyOrderId,onClaim,onClaimContract,onNavigate,onOpenWeeklyBoss,onOpenWeeklyOrder,onPinWeeklyOrder,onQueueWeeklyOrder,onStopWeeklyOrder}:{state:GameState;mode:QuestMode;onModeChange:(mode:QuestMode)=>void;focusedWeeklyOrderId?:string;onClaim:(id:string)=>void;onClaimContract:(period:SeasonalPeriod,id:string)=>void;onNavigate:(destination:QuestDestination)=>void;onOpenWeeklyBoss:()=>void;onOpenWeeklyOrder:(order:WeeklyOrder)=>void;onPinWeeklyOrder:(order:WeeklyOrder)=>void;onQueueWeeklyOrder:(order:WeeklyOrder)=>void;onStopWeeklyOrder:(order:WeeklyOrder)=>void}){
  ...
  const contractsUnlocked=earlyFeatureUnlocked(state,'contracts'),challengesUnlocked=earlyFeatureUnlocked(state,'classChallenges'),contractsUnlock=earlyFeatureUnlockProgress(state,'contracts'),challengesUnlock=earlyFeatureUnlockProgress(state,'classChallenges');
  ...
  return <ScrollView contentContainerStyle={s.root} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
    ...
    <QuestModeSwitch mode={mode} onChange={onModeChange} storyMeta={ready?ready+' ready':claimed+'/'+QUESTS.length+' claimed'} contractMeta={boardComplete+'/'+contractBoard.orders.length+' complete'} challengeMeta={challengeReadyCount?challengeReadyCount+' ready':challengeClaimedCount+'/'+allChallenges.length+' claimed'} storyAttention={ready} contractAttention={contractsUnlocked?pendingBoardRewards.length:0} challengeAttention={challengesUnlocked?challengeReadyCount:0} lockedModes={{...(contractsUnlocked?{}:{contracts:'Locked · '+contractsUnlock.requirement}),...(challengesUnlocked?{}:{challenges:'Locked · '+challengesUnlock.requirement})}}/>
    ...
    {mode==='contracts'&&!contractsUnlocked?<FeatureLockedPanel state={state} featureId="contracts"/>:mode==='contracts'?<>
      <Panel accentColor={C.info}>
        <View style={s.modeHeroHead}><View style={s.flex}><Text style={s.modeEyebrow}>WEEKLY BOARD · {contractBoard.weekKey}</Text><Text style={s.title}>{focusedOrder?'Focused · '+focusedOrder.title:nextBoardOrder?'Next · '+nextBoardOrder.title:'All posted jobs complete'}</Text></View>{focusedOrder?<StatusPill label="FOCUSED" tone="info"/>:pendingBoardRewards.length>0?<StatusPill label={pendingBoardRewards.length+' REWARD'+(pendingBoardRewards.length===1?'':'S')} tone="good"/>:<StatusPill label="WEEKLY" tone="info"/>}</View>
      ...
      <Panel>
        {focusedOrder?<Text style={s.focusContext}>Opened from another gameplay screen · focused contract is promoted first.</Text>:null}
    ...
    {mode==='challenges'&&!challengesUnlocked?<FeatureLockedPanel state={state} featureId="classChallenges"/>:mode==='challenges'?<>
      <Panel accentColor={C.special}>
        ...
        <Text style={s.sub}>{challengeReadyCount>0?'Claim completed objectives, then continue the next cadence target.':nextChallenge?(Math.max(0,nextChallenge.required-nextChallenge.progress)+' progress remaining · '+QUEST_RARITIES[nextChallenge.rarity].cache):'Class-aligned challenges refresh on daily, weekly and monthly cadences and award progressively stronger rarity caches.'}</Text>
      </Panel>
