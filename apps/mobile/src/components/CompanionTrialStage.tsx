...
export function CompanionTrialStage({state,now,floor,teamIds}:{state:GameState;now:number;floor:number;teamIds:string[]}){
 ...
 if(playback&&fresh){
  ...
  const unitCard=(unit:PlaybackUnit)=>{
   ...
   const impactLabel=current?.targetId===unit.id?(current.type==='damage'||current.type==='dot_tick'?'-'+Math.round(current.amount??0):current.type==='heal'||current.type==='hot_tick'?'+HP '+Math.round(current.amount??0):current.type==='shield'?'+SHIELD '+Math.round(current.amount??0):undefined):undefined;
   return <View key={unit.id} style={[s.liveUnit,!unit.alive&&s.downedUnit,current?.targetId===unit.id&&s.targetUnit]}><View style={s.liveUnitRow}><View style={s.unitIcon}>{art?<Image source={art} resizeMode="contain" style={s.unitArt}/>:<Text style={s.unitGlyph}>{unit.boss?'♛':def?roleGlyph[def.role]:'◇'}</Text>}</View><View style={s.liveUnitBody}>
    ...
    {unit.casting?<Text style={s.castText}>CASTING · {unit.casting}</Text>:null}</View>{impactLabel?<Text style={[s.impactNumber,(current?.type==='heal'||current?.type==='hot_tick'||current?.type==='shield')&&s.impactPositive]}>{impactLabel}</Text>:null}</View></View>;
  };
 ...
 return <View style={s.stage}>
  ...
  <View style={s.arena}>
   <View style={s.teamColumn}><Text style={s.sideLabel}>YOUR FORMATION</Text>{team.length?team.map(id=>{const def=COMBAT_COMPANIONS.find(row=>row.id===id),progress=view.owned[id];if(!def)return null;const affinity=COMPANION_AFFINITIES[companionAffinity(def)];const art=companionArtSource(id);return <View key={id} style={s.unit}><View style={s.previewIcon}>{art?<Image source={art} resizeMode="contain" style={s.previewArt}/>:<Text style={s.unitGlyph}>{roleGlyph[def.role]}</Text>}</View><View style={s.unitCopy}><Text numberOfLines={1} style={s.unitName}>{def.name}</Text><Text style={s.unitMeta}>{def.role.toUpperCase()} · {affinity.glyph} {affinity.label} · LV {progress.level} · B{progress.bondLevel}</Text><Text numberOfLines={1} style={s.ability}>{def.activeAbility.name} · {def.activeAbility.cooldownSeconds}s</Text></View></View>}):<View style={s.empty}><Text style={s.emptyText}>Choose Tank · Damage · Support below to preview the formation.</Text></View>}</View>
