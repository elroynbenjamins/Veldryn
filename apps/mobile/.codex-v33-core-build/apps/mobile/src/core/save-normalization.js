...
const companion_save_1 = require("./companion-save");
...
const character_loadouts_1 = require("./character-loadouts");
const onboarding_1 = require("./onboarding");
function normalizeSave(input) {
    if (!input || ![4, 5, 6, 7, 8, 9, 10, 11].includes(input.version))
    ...
    const character = input.character ? (() => {
        ...
        return {
            ...savedCharacter,
            monsterMasteryPoints: (0, monster_mastery_1.normalizeMonsterMastery)(savedCharacter.monsterMasteryPoints),
            masteryMaterialRemainders: Object.fromEntries(Object.entries(savedCharacter.masteryMaterialRemainders ?? {}).filter(([id, v]) => items_1.ITEMS.some(i => i.id === id && i.type === 'material') && typeof v === 'number' && Number.isFinite(v) && v >= 0 && v < 1)),
            classSkills: (0, class_skills_1.normalizeClassSkills)(savedCharacter.classId, savedCharacter.classSkills),
            faith: (0, faith_1.normalizeFaith)(savedCharacter.faith),
            trainingFocus: (0, class_skills_1.normalizeTrainingFocus)(savedCharacter.trainingFocus),
            classTraining: input.activity ? undefined : (0, class_skills_1.normalizeClassDrills)(savedCharacter.classTraining),
            classSkillRemainders: Object.fromEntries(Object.entries(savedCharacter.classSkillRemainders ?? {}).filter(([id, v]) => (0, class_skills_1.normalizeClassSkills)(savedCharacter.classId, []).some(s => s.skillId === id) && typeof v === 'number' && Number.isFinite(v) && v >= 0 && v < 1)),
            ...
            ownedBoostIds: legacyPetOwnership ? undefined : Array.isArray(input.character.ownedBoostIds) ? [...new Set(input.character.ownedBoostIds.filter((id) => typeof id === 'string'))] : [],
            gearEnhancements,
            ...
            savedLoadouts: (0, character_loadouts_1.normalizeCharacterLoadouts)(input.character.savedLoadouts, input.character.classId)
        };
    })() : null;
    ...
    if (input.activity?.brew !== undefined && input.activity?.kind !== 'alchemy')
        throw new Error('Invalid orphaned alchemy reservation.');
    if (input.activity && input.activity.progressFraction !== undefined && (!Number.isFinite(input.activity.progressFraction) || input.activity.progressFraction < 0 || input.activity.progressFraction >= 1))
        throw new Error('Invalid activity progress.');
    if (input.activity && (!Number.isSafeInteger(input.activity.lastClaimAtMs) || input.activity.lastClaimAtMs < input.activity.startedAtMs))
        throw new Error('Invalid activity timeline.');
    ...
    const activity = input.activity ? { ...input.activity, environment, brew: input.activity.kind === 'alchemy' ? (0, alchemy_1.normalizeAlchemyBatch)(input.activity.brew, input.activity.targetId) : undefined } : null;
    ...
    const skills = skillIds.map(skillId => { const raw = (input.skills ?? []).find((entry) => entry?.skillId === skillId); const xp = Number.isFinite(Number(raw?.xp)) ? Math.max(0, Math.floor(Number(raw.xp))) : 0; return { skillId, xp, level: Math.max(1, Math.min(100, Number.isFinite(Number(raw?.level)) ? Math.floor(Number(raw.level)) : 1)) }; });
    ...
    normalized.account = { ...normalized.account, ...(0, combat_companions_1.migrateLegacyCombatCompanionAccount)(input), ...(0, companion_save_1.normalizeCompanionRuntimeSave)(input.account) };
    normalized.account.collectionPreferences = (0, collection_preferences_1.normalizeCollectionPreferences)(input.account?.collectionPreferences);
    normalized.account.guideState = (0, onboarding_1.normalizeOnboardingGuideState)(input.account?.guideState);
    normalized.account.arenaSquadCharacterIds = Array.isArray(input.account?.arenaSquadCharacterIds)
        ? [...new Set(input.account.arenaSquadCharacterIds.filter((id) => typeof id === 'string'))].slice(0, 3)
        : undefined;
    if (input.otherCharacters !== undefined && !Array.isArray(input.otherCharacters))
        throw new Error('Invalid account roster.');
    if (Array.isArray(input.otherCharacters) && input.otherCharacters.length > 4)
        throw new Error('Account roster exceeds the five-character limit.');
    const rosterIds = new Set();
    const roster = Array.isArray(input.otherCharacters) ? input.otherCharacters.filter((entry) => entry?.character?.id).map((entry) => { const id = String(entry.character.id); if (rosterIds.has(id))
        throw new Error('Duplicate account character.'); rosterIds.add(id); return entry; }) : [];
    if (character?.id && rosterIds.has(character.id))
        throw new Error('Duplicate active account character.');
    normalized.otherCharacters = roster;
    normalized.account.unlockedCharacterSlots = Math.max(1, Math.min(5, Number(input.account?.unlockedCharacterSlots ?? 1)));
    return (0, character_skins_1.discoverCharacterSkins)((0, combat_companions_1.reconcileCombatCompanionUnlocks)(normalized, normalized.createdAtMs));
}
