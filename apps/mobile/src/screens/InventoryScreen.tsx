...
export function InventoryScreen({state,onEquip,onFood,onEat,onSell,onSalvage,onDeposit,onDepositMaterials,onUpgradeStorage,onWithdraw,onOverflow,onCraft}:{state:GameState;onEquip:(id:string)=>void;onFood:(id:string)=>void;onEat:(id:string)=>void;onSell:(id:string)=>void;onSalvage:(id:string)=>void;onDeposit:(id:string,quantity:number)=>void;onDepositMaterials:()=>void;onUpgradeStorage:(location:StorageLocation)=>void;onWithdraw:(id:string,quantity:number)=>void;onOverflow:()=>void;onCraft:(recipeId:string)=>void}){
  ...
  const renderStack=(stack:ItemStack)=>{
    ...
    return <ItemCard expanded={expandedItem===location+':'+item.id} onToggle={()=>setExpandedItem(value=>value===location+':'+item.id?null:location+':'+item.id)} key={`${location}:${item.id}`} item={item} quantity={stack.quantity} equipped={equippedId?itemDef(equippedId):undefined} upgradeRank={enhancement?.rank} socketed={enhancement?.gemIds.length} socketCapacity={item.type==='gear'?gemSocketCapacity(item.id):0} enhancementProtected={enhancementProtected} displayStats={item.type==='gear'?enhancedGearStats(state,item.id):undefined} equippedDisplayStats={equippedId?enhancedGearStats(state,equippedId):undefined} selectedFood={selectedFood} healAmount={recoveryAmount(state,item.id)} transferQuantity={amount} transferIssue={transferError(state,item.id,amount,location)} numberMode={state.settings.numberMode}
      ...
      onSell={carried&&item.value>0&&!enhancementProtected?()=>setPending({kind:'sell',item,quantity:1}):undefined} onSalvage={carried&&item.salvage&&!enhancementProtected?()=>setPending({kind:'salvage',item,quantity:1}):undefined}
  ...
  return <><InventoryFilterSheet visible={showFilters} onClose={()=>setShowFilters(false)} filter={filter} onFilter={setFilter} sort={sort} onSort={setSort} quantity={quantity} onQuantity={setQuantity} resultCount={stacks.length} totalCount={totalStacks}/><EquipmentPreview state={state} itemId={previewId} onClose={()=>setPreviewId(null)}/><ScrollView contentContainerStyle={s.root} keyboardShouldPersistTaps="handled">
    ...
    <View style={s.controlRow}><View style={s.flex}><GameButton title={`Filters${filter==='all'?'': ' · 1'} ▾`} tone={filter==='all'?'secondary':'primary'} onPress={()=>setShowFilters(true)}/></View><View style={s.flex}><GameButton title={`Sort: ${inventorySortLabel(sort)} ▾`} tone="secondary" onPress={()=>setShowFilters(true)}/></View></View>
    <Text style={s.sub}>{inventoryFilterLabel(filter)} · transfer {quantity==='all'?'all':quantity}</Text>
...
const s=StyleSheet.create({recovery:{gap:4,paddingVertical:8,paddingHorizontal:4},root:{padding:spacing.lg,gap:spacing.md},h:{...typography.hero,color:C.text},title:{...typography.title,color:C.text},sub:{...typography.body,color:C.muted},warning:{...typography.body,color:C.warning},label:{...typography.caption,color:C.accent,fontWeight:'700'},upgradeLabel:{...typography.caption,color:C.muted,fontWeight:'700',marginBottom:spacing.xs},disclosure:{minHeight:60,flexDirection:'row',alignItems:'center',gap:spacing.sm,paddingHorizontal:spacing.md,borderWidth:1,borderColor:C.line,borderRadius:10,backgroundColor:C.panel},disclosureTitle:{...typography.caption,color:C.accent,fontWeight:'700',letterSpacing:.8},disclosureMark:{width:28,color:C.accent,fontSize:25,textAlign:'center'},rarityLegend:{gap:spacing.sm,paddingRight:spacing.md},rarityKey:{minHeight:32,justifyContent:'center',borderWidth:1,borderRadius:99,paddingHorizontal:spacing.sm},rarityKeyText:{...typography.caption,fontWeight:'700'},row:{flexDirection:'row',gap:spacing.sm},controlRow:{flexDirection:'row',gap:spacing.sm,alignItems:'center'},flex:{flex:1},input:{minHeight:48,paddingHorizontal:spacing.md,borderRadius:10,borderWidth:1,borderColor:C.line,color:C.text,backgroundColor:C.panel,fontSize:16}});
