...
export function CombatantProfileCard({slot,active=false,targeted=false,assistProc=false,motionStyle,feedbackStyle,currentCue}:{slot:Slot;active?:boolean;targeted?:boolean;assistProc?:boolean;motionStyle?:any;feedbackStyle?:any;currentCue?:CoopCombatReplayCueView}){
...
export function EnemyCombatProfileCard({name,boss=false,active=false,targeted=false,motionStyle,feedbackStyle,currentCue,bossPhaseLabel,bossCast}:{name:string;boss?:boolean;active?:boolean;targeted?:boolean;motionStyle?:any;feedbackStyle?:any;currentCue?:CoopCombatReplayCueView;bossPhaseLabel?:string;bossCast?:BossCombatCastState}){
 const C=useGameTheme(),s=useMemo(()=>makeStyles(C),[C]),enemyArt=dungeonEnemyPortraitSource(name),float=currentCue?.type==='action'&&currentCue.targetName===name?{label:currentCue.outcome==='miss'?'MISS':`${currentCue.actionKind==='heal'?'+':'-'}${Math.round(currentCue.amount??0)}`,heal:currentCue.actionKind==='heal',miss:currentCue.outcome==='miss',outcome:currentCue.outcome==='critical'?'CRIT':currentCue.outcome==='miss'?'DODGE':currentCue.absorbed&&currentCue.absorbed>0?`BARRIER -${Math.round(currentCue.absorbed)}`:currentCue.gemProc?'GEM PROC':undefined}:undefined;
