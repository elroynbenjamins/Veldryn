...
import {effectiveOwnedGearRarity,equippedGearInstanceId,gearInstanceEnhancement,resolveGearSelection,updateGearInstanceEnhancement} from './crafted-gear-instances';
...
function resolveRef(state:GameState,ref:string){
  const resolved=resolveGearSelection(state,ref);
  if(resolved)return {itemId:resolved.itemId,instanceId:resolved.instance?.id};
  return {itemId:ref,instanceId:undefined};
}
export function gearEnhancement(state:GameState,itemRef:string,instanceId?:string):GearEnhancementState{
  const resolved=resolveRef(state,itemRef),itemId=resolved.itemId,id=instanceId??resolved.instanceId??equippedGearInstanceId(state,itemId);
  const instanceRaw=gearInstanceEnhancement(state,itemId,id);
  const raw=instanceRaw??state.character?.gearEnhancements?.[itemId],slots=normalizeEnhancementGemSlots(raw);
...
export function hasEnhancement(state:GameState,itemRef:string,instanceId?:string){const enhancement=gearEnhancement(state,itemRef,instanceId);return enhancement.rank>0||enhancement.gemIds.length>0;}
export function gemSocketCapacity(itemRef:string){const item=itemDef(itemRef);return item.type==='gear'?EQUIPMENT_GEM_SOCKET_COUNT:0;}
export function gemSocketState(state:GameState,itemRef:string,instanceId?:string){
  const {itemId}=resolveRef(state,itemRef),enhancement=gearEnhancement(state,itemId,instanceId??resolveRef(state,itemRef).instanceId);
  return {statGemId:enhancement.statGemId,effectGemId:enhancement.effectGemId,filled:Number(Boolean(enhancement.statGemId))+Number(Boolean(enhancement.effectGemId)),capacity:gemSocketCapacity(itemId)};
}
export function upgradeQuote(state:GameState,itemRef:string,instanceId?:string){
  const resolved=resolveRef(state,itemRef),itemId=resolved.itemId,id=instanceId??resolved.instanceId??equippedGearInstanceId(state,itemId);
  const item=itemDef(itemId);if(item.type!=='gear')throw new Error('Only equipment can be upgraded');
  const current=gearEnhancement(state,itemId,id),targetRank=current.rank+1;
  ...
  const rarity=effectiveOwnedGearRarity(state,itemId,id)??itemRarity(item),pity=Math.min(.10,current.failures*.02);
...
function setEnhancement(state:GameState,itemId:string,value:GearEnhancementState,instanceId?:string){
  const slots=normalizeEnhancementGemSlots(value),normalized={...value,...slots},id=instanceId??equippedGearInstanceId(state,itemId);
  if(id)return updateGearInstanceEnhancement(state,id,normalized);
  return {...state,character:{...state.character!,gearEnhancements:{...(state.character!.gearEnhancements??{}),[itemId]:normalized}}};
}
function requireEquipped(state:GameState,itemRef:string){
  if(!state.character)throw new Error('No character');
  const resolved=resolveRef(state,itemRef),itemId=resolved.itemId,item=itemDef(itemId);
...
export function attemptEquipmentUpgrade(state:GameState,itemRef:string,roll=Math.random()){
  const equipped=requireEquipped(state,itemRef);if(roll<0||roll>=1)throw new Error('Invalid upgrade roll');
  const quote=upgradeQuote(state,equipped.itemId,equipped.instanceId);if(quote.maxed)throw new Error('This item is already +10');
  ...
  const prior=gearEnhancement(state,equipped.itemId,equipped.instanceId),success=roll<quote.successChance;
  const rank=success?quote.targetRank:prior.rank;
  next=setEnhancement(next,equipped.itemId,{...prior,rank,failures:success?0:prior.failures+1},equipped.instanceId);
  return {state:next,result:{success,oldRank:prior.rank,newRank:rank,successChance:quote.successChance,downgraded:!success&&rank<prior.rank,instanceId:equipped.instanceId}};
}
export function socketGem(state:GameState,itemRef:string,gemId:string){
  const equipped=requireEquipped(state,itemRef),gem=safeGem(gemId);if(!gem)throw new Error('That item is not a gem');
  const kind=gemSocketKind(gemId),enhancement=gearEnhancement(state,equipped.itemId,equipped.instanceId);
  ...
  next=setEnhancement(next,equipped.itemId,{...enhancement,statGemId:kind==='stat'?gemId:enhancement.statGemId,effectGemId:kind==='effect'?gemId:enhancement.effectGemId,gemIds:[]},equipped.instanceId);
  return next;
}
...
export function unsocketGem(state:GameState,itemRef:string,index:number){
  const equipped=requireEquipped(state,itemRef);if(index!==0&&index!==1)throw new Error('Unknown gem socket');
  const enhancement=gearEnhancement(state,equipped.itemId,equipped.instanceId),gemId=index===0?enhancement.statGemId:enhancement.effectGemId;
  ...
  next=setEnhancement(next,equipped.itemId,{...enhancement,statGemId:index===0?undefined:enhancement.statGemId,effectGemId:index===1?undefined:enhancement.effectGemId,gemIds:[]},equipped.instanceId);
  return next;
}
...
export function enhancedGearStats(state:GameState,itemRef:string,instanceId?:string){
  const resolved=resolveRef(state,itemRef),itemId=resolved.itemId,id=instanceId??resolved.instanceId??equippedGearInstanceId(state,itemId);
  const stats=gearStatsAtRank(itemId,gearEnhancement(state,itemId,id).rank),rarity=effectiveOwnedGearRarity(state,itemId,id),m=craftedRarityStatMultiplier(itemId,rarity),scale=(value:number)=>value>0?Math.ceil(value*m):Math.round(value*m);
