...
const halloween=eventExpeditionPreviews(Date.UTC(2026,10,31));
...
const newYearsEve=eventExpeditionPreviews(Date.UTC(2026,12,31));
...
const heartbond=eventExpeditionPreviews(Date.UTC(2027,2,14));
assert.equal(heartbond.find(item=>item.id==='EVENT_HEARTBOND_GARDEN_BROKEN_VOWS')?.scheduled,true);
const bloomwake=eventExpeditionPreviews(Date.UTC(2027,3,25));
