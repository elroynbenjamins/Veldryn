...
ok(bosses[0].abilities.length>=2,'Floor 5 boss should combine offense and a defensive phase');
ok(bosses[2].abilities.some(a=>a.interruptible&&a.castTimeMs>0),'Floor 15 boss should expose an interruptible signature');
...
ok(bosses[5].abilities.some(a=>a.interruptible&&a.castTimeMs>0)&&bosses[5].abilities.some(a=>a.target==='all_enemies'),'Floor 30 boss should mix AoE pressure with an interruptible judgment');
