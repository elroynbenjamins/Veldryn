...
import {earlyFeatureUnlocked,earlyFeatureUnlockProgress,newlyUnlockedEarlyFeatures,type EarlyFeatureUnlockMoment} from './src/core/feature-unlocks';
...
function VeldrynApp(){
  ...
  const goBack=useCallback(()=>{
    ...
    return true;
  },[showChatOverlay,showChatPilot,showAdminQa,showCoopUiGallery,tab,tabHistory,selectedSkill,confirmProfileCustomizeExit]);
...
const next=discoverCharacterSkins(candidate);queuePreparationNotices(current,next);stateRef.current=next;setState(next);queueCustomizationUnlocks(current,next);queueFeatureUnlockMoments(current,next);try{await repo.save(next)}catch{Alert.alert('Local save failed','Progress is still in memory. Keep the app open and try another action to save again.')}}
  ...
  const eventUnlocked=earlyFeatureUnlocked(state,'events'),companionUnlocked=earlyFeatureUnlocked(state,'companions'),dailyUnlocked=earlyFeatureUnlocked(state,'dailySupplies'),contractsUnlocked=earlyFeatureUnlocked(state,'contracts'),guildUnlocked=earlyFeatureUnlocked(state,'guild');
  const eventClaims=eventUnlocked?eventReadyClaimCount(state,now):0;
  const companionAttention=companionAttentionSummary(state,now);
  const workingTowardReady=workingTowardReadyCount(state);
  const dailySuppliesReady=dailyUnlocked&&dailySuppliesStatus(state,now).canClaim;
  ...
  const navigationNotifications:NavigationNotification[]=[
    ...
    {key:'companion-attention',kind:'companion_attention',unread:companionUnlocked&&companionAttention.hasAttention},
    ...
    {key:'weekly-order-rewards',kind:'weekly_order_complete',count:contractsUnlocked?contractBoard.pendingRewards:0,unread:contractsUnlocked&&contractBoard.pendingRewards>0},
    ...
    {key:'online-event-attention',kind:'event_reward_ready',count:eventUnlocked?notificationCounts.events:0,unread:eventUnlocked&&notificationCounts.events>0},
  ];
  ...
  const quickNavLocks:Partial<Record<QuickNavDestination,string>>={};
  if(!guildUnlocked)quickNavLocks.Guild=earlyFeatureUnlockProgress(state,'guild').requirement;
  if(!earlyFeatureUnlocked(state,'dungeons'))quickNavLocks.Dungeon=earlyFeatureUnlockProgress(state,'dungeons').requirement;
  if(!companionUnlocked)quickNavLocks.Companions=earlyFeatureUnlockProgress(state,'companions').requirement;
  if(!eventUnlocked)quickNavLocks.Events=earlyFeatureUnlockProgress(state,'events').requirement;
  ...
  return <GameThemeProvider themeId={state.settings.uiTheme}><SafeAreaView edges={['top','left','right']} style={[s.safe,{backgroundColor:theme.bg}]}><StatusBar style={theme.dark?'light':'dark'}/><GameTopBar state={state} nowMs={now} attention={quickNavigationBadges} lockedDestinations={quickNavLocks} labelForDestination={destination=>tabLabel(state.settings.language,destination)} onNavigate={destination=>{if(destination==='Dungeon'){setTab('Coop');return;}if(destination==='Quests'){openQuestMode('story');return;}setTab(destination)}} onChangeDestinations={destinations=>commit({...state,settings:{...state.settings,quickNavDestinations:destinations}})} onOpenActivity={openActiveActivity}/>{secondary&&tab!=='Coop'&&<View style={[s.backBar,{backgroundColor:theme.panel,borderColor:theme.line}]}><Pressable accessibilityRole="button" accessibilityLabel={t(state.settings.language,'common.back')} onPress={goBack} hitSlop={8} style={({pressed})=>[s.backButton,pressed&&s.backPressed]}><Image source={require('./assets/ui-icons-v2/small/back.png')} resizeMode="contain" style={s.backIcon}/><Text style={[s.backText,{color:theme.accent}]}>{t(state.settings.language,'common.back')}</Text></Pressable><Text numberOfLines={1} style={[s.backTitle,{color:theme.text}]}>{tabLabel(state.settings.language,tab)}</Text><View style={s.backSpacer}/></View>}{serverGameplayEnabled&&<View style={{paddingHorizontal:12}}>{!!online.error&&<Text accessibilityRole="alert" style={s.txt}>{online.error}</Text>}{online.pending?<GameButton title="Retry pending action" disabled={online.busy} onPress={()=>void perform()}/>:<Text style={s.txt}>{online.busy?'Saving online…':'Online · progress saved on server'}</Text>}</View>}<View pointerEvents={serverGameplayEnabled&&online.busy?'none':'auto'} style={s.body} {...backSwipe.panHandlers}>
    ...
    {tab==='More'&&<MoreScreen state={state} language={state.settings.language} onNavigate={destination=>{if(destination==='Quests'){openQuestMode('story');return;}if(destination==='Dungeon'){setTab('Coop');return;}setTab(destination)}} companionAttention={companionUnlocked&&companionAttention.hasAttention} workingTowardAttention={workingTowardReady>0} dailySuppliesAttention={dailySuppliesReady} eventAttention={eventUnlocked&&(eventClaims>0||notificationCounts.events>0)} friendRequestCount={notificationCounts.friendRequests} guildAttentionCount={guildUnlocked?notificationCounts.guild:0} socialAttentionCount={notificationCounts.partyInvites+notificationCounts.chatUnread} profileAttention={profileAttentionKeys.length>0} onOpenChatPilot={__DEV__?()=>{setChatPilotInitialPanel('chat');setShowChatPilot(true)}:undefined} onOpenAdminQa={adminQa||(__DEV__&&!serverGameplayEnabled)?()=>setShowAdminQa(true):undefined}/>}
  ...
  <ChatOverlay state={state} visible={showChatOverlay} onOpen={()=>setShowChatOverlay(true)} onClose={()=>setShowChatOverlay(false)} onEmoteTrayChange={ids=>commit({...state,settings:{...state.settings,chatEmoteTrayIds:ids}})} guildUnread={guildUnlocked?notificationCounts.guildChatUnread:0} guildMentions={guildUnlocked?notificationCounts.guildChatMentions:0} guildFirstUnreadMessageId={guildUnlocked?notificationCounts.guildFirstUnreadMessageId:undefined} partyUnread={notificationCounts.partyChatUnread} partyMentions={notificationCounts.partyChatMentions} partyFirstUnreadMessageId={notificationCounts.partyFirstUnreadMessageId} onChatRead={()=>void refreshSocialNotifications()}/>
