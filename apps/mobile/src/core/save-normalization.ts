...
import {normalizeGearEnhancementState} from './equipment-enhancement';
...
export function normalizeSave(input:any):GameState{
  ...
  const character=input.character?(()=>{
    ...
    const gearEnhancements=Object.fromEntries(Object.entries(input.character.gearEnhancements??{}).filter(([id,value])=>gearIds.has(id)&&value&&typeof value==='object').map(([id,value])=>[id,normalizeGearEnhancementState(id,value)]));
    return {
    ...savedCharacter,
  ...
  const roster=Array.isArray(input.otherCharacters)?input.otherCharacters.filter((entry:any)=>entry?.character?.id).map((entry:any)=>{const id=String(entry.character.id);if(rosterIds.has(id))throw new Error('Duplicate account character.');rosterIds.add(id);const rosterGearEnhancements=Object.fromEntries(Object.entries(entry.character.gearEnhancements??{}).filter(([gearId,value])=>ITEMS.some(item=>item.id===gearId&&item.type==='gear')&&value&&typeof value==='object').map(([gearId,value])=>[gearId,normalizeGearEnhancementState(gearId,value)]));return {...entry,character:{...entry.character,gearEnhancements:rosterGearEnhancements,dailySupplyBoostBank:normalizeDailySupplyBank(entry.character.dailySupplyBoostBank),activeDailySupplyBoost:normalizeActiveDailySupplyBoost(entry.character.activeDailySupplyBoost)}};}):[];
