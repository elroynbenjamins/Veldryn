...
export function GuildScreen({state,onChange,onlineDirectory,onlineManagement,onlinePve}:{state:GameState;onChange:(next:GameState)=>void;onlineDirectory?:ReactNode;onlineManagement?:ReactNode;onlinePve?:ReactNode}){
  ...
  return <ScrollView contentContainerStyle={s.root}>
    ...
    {section==='Roster'&&<Panel><Text style={s.title}>Roster · {joined?ROSTER.length:0}</Text>{joined?ROSTER.map(([name,role,level,status])=><View style={s.member} key={name}><View style={s.avatar}><Text style={s.avatarText}>{String(name).slice(0,1)}</Text></View><View style={s.flex}><Text style={s.memberName}>{name}</Text><Text style={s.sub}>{role} · Level {level}</Text></View><Text style={s.status}>{status}</Text></View>):<Text style={s.sub}>Join a guild to view its members.</Text>}</Panel>}
  </ScrollView>;
}
