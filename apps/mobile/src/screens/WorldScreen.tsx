...
export function WorldScreen({state,onStart,onBoss,selectedId,onSelectZone}:{state:GameState;onStart:(id:string)=>void;onBoss:()=>void;selectedId:string|null;onSelectZone:(id:string|null)=>void}){
  ...
  return <ScrollView contentContainerStyle={s.root}>
    ...
    {mode==='map'?<><View style={s.mapTools}><View style={s.flex}><Text style={s.current}>Drag to explore · tap a region</Text><Text style={s.mapHint}>Use −/+ to change the map scale.</Text></View><Pressable accessibilityRole="button" accessibilityLabel="Zoom out map" disabled={zoom<=1} onPress={()=>setZoom(value=>Math.max(1,value-.35))} style={[s.zoomButton,zoom<=1&&s.zoomDisabled]}><Text style={s.zoomText}>−</Text></Pressable><Text style={s.zoomLevel}>{Math.round(zoom*100)}%</Text><Pressable accessibilityRole="button" accessibilityLabel="Zoom in map" disabled={zoom>=2.05} onPress={()=>setZoom(value=>Math.min(2.05,value+.35))} style={[s.zoomButton,zoom>=2.05&&s.zoomDisabled]}><Text style={s.zoomText}>+</Text></Pressable></View><View style={s.viewport}><ScrollView horizontal nestedScrollEnabled directionalLockEnabled={false} showsHorizontalScrollIndicator><ScrollView nestedScrollEnabled directionalLockEnabled={false} showsVerticalScrollIndicator contentContainerStyle={{width:620*zoom,height:620*zoom}}><ImageBackground source={require('../../assets/world/asterfall-map-v1.png')} resizeMode="cover" imageStyle={s.mapArtwork} style={[s.map,{width:620*zoom,height:620*zoom}]} accessibilityLabel="Interactive illustrated map of Asterfall">
      ...
      {WORLD_ZONES.map(zone=>{const unlocked=level>=zone.minLevel,active=activeMonster?.zone===zone.name,environment=environmentForZone(zone.id);return <Pressable key={zone.id} accessibilityRole="button" accessibilityLabel={`${zone.name}, ${environment.weatherName}, ${unlocked?'unlocked':`preview, requires level ${zone.minLevel}`}`} onPress={()=>onSelectZone(zone.id)} style={[s.node,{left:`${zone.x*100}%`,top:`${zone.y*100}%`,borderColor:unlocked?zone.accent:C.disabled},active&&s.nodeActive]}><View style={s.nodeSymbols}><Text style={[s.symbol,{color:unlocked?zone.accent:C.disabled}]}>{zone.symbol}</Text><Text style={[s.weatherSymbol,{color:environment.weatherColor}]}>{environment.weatherSymbol}</Text></View><Text style={s.nodeLabel}>{zone.name}</Text><Text style={s.nodeWeather}>{environment.weatherName}</Text><Text style={s.nodeLevel}>{unlocked?`LV ${zone.minLevel}–${zone.maxLevel}`:`LOCKED · LV ${zone.minLevel}`}</Text>{active&&<Text style={s.activeTag}>ACTIVE</Text>}</Pressable>})}
    </ImageBackground></ScrollView></ScrollView></View></>:WORLD_ZONES.map(zone=>{const environment=environmentForZone(zone.id);return <Panel key={zone.id}><Text style={s.title}>{zone.symbol} {zone.name}</Text><Text style={s.sub}>{zone.subtitle} · Levels {zone.minLevel}–{zone.maxLevel}</Text><Text style={[s.current,{color:environment.weatherColor}]}>{environment.seasonSymbol} {environment.seasonName} · {environment.weatherSymbol} {environment.weatherName}</Text><GameButton title={level>=zone.minLevel?'Explore region':`Preview · unlocks at level ${zone.minLevel}`} tone="secondary" onPress={()=>onSelectZone(zone.id)}/></Panel>})}
...
function ZoneView({zone,state,onBack,onStart,onBoss}:{zone:WorldZoneDef;state:GameState;onBack:()=>void;onStart:(id:string)=>void;onBoss:()=>void}){
  ...
  const environment=environmentForZone(zone.id);
  return <ScrollView contentContainerStyle={s.root} keyboardShouldPersistTaps="handled">
    ...
    <EnvironmentBanner environment={environment} kind="combat"/>
    ...
    {monsters.map(monster=>{
      ...
      return <Panel key={monster.id}>
        ...
        {!monster.boss&&<Text style={s.sub}>Base rate: {formatGameNumber(baseXp,state.settings.numberMode)} XP/hour before combat speed and survival. Not a guaranteed reward.</Text>}
