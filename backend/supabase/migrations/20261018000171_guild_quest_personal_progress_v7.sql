...
-- Guild Quest state v7: add authoritative personal objective progress.\n-- State v5 projects only the curated weekly board.
...
returns table(
 ...
 board_slot integer,featured boolean,personal_progress jsonb
)
 ...
 for b in select * from public.guild_quest_board_v5(gid,wk) order by board_slot loop
   ...
   if b.source_key in ('combat_front','skilling_drive') then
     return query select gid,wk,(wk+7)::timestamp at time zone 'UTC',t.active_members,b.source_key,
       ...
       exists(select 1 from public.guild_quest_completions c where c.guild_id=gid and c.week_key=wk and c.quest_key=b.source_key||':'||rr),p.contributors,b.board_slot,b.featured,'[]'::jsonb;
   else
     select * into s from (
     ...
     return query select gid,wk,(wk+7)::timestamp at time zone 'UTC',t.active_members,b.source_key,s.title,s.category,s.description,s.theme,rr,public.guild_quest_rarity_minutes_v3(rr),
       ...
       exists(select 1 from public.guild_quest_completions c where c.guild_id=gid and c.week_key=wk and c.quest_key=b.source_key||':'||rr),p.contributors,b.board_slot,b.featured,public.guild_quest_personal_objective_progress_v6(gid,uid,wk,prog);
   end if;
 end loop;
end $$;
