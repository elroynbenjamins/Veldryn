...
const enemy=(id:string,name:string,level:number,hp:number,attackPower:number,defense:number,type:DamageType,index:number):CombatantDefinition=>({id,name,team:'enemies',role:'enemy',level,stats:stats(hp,attackPower,defense,level),basicAttackMs:2750,basicAttackCoeff:.74,abilities:[strike(`${id}_STRIKE`,'Regional Strike',.96+index*.025,type)],});
...
for(const [index,id] of ['FROST_CHOIR_BATTLE_01','FROST_CHOIR_BATTLE_02','FROST_CHOIR_BATTLE_03'].entries())FROSTMARCH_ENCOUNTERS[id]=()=>battle(id,70,index,'ice',[['Choir Wisp','Rime Cantor'],['Frozen Pilgrim','Bellbound Shade'],['Choir Sentinel','Glacial Acolyte'][index] as string, ''] as unknown as [string,string],13_900,5_100,1_500);
...
for(const [index,name] of ['Rimehorn Alpha','Choirbreaker','Permafrost Warden'].entries()){FROSTMARCH_ENCOUNTERS[`FROST_LAKE_ELITE_0${index+1}`]=()=>elite(`FROST_LAKE_ELITE_${index+1}`,name,70,'ice',index,35_000,5_600,2_100);FROSTMARCH_ENCOUNTERS[`FROST_CHOIR_ELITE_0${index+1}`]=()=>elite(`FROST_CHOIR_ELITE_${index+1}`,name,70,'ice',index,35_800,5_700,2_150);}
...
for(const [index,name] of ['Fen Pyrecaller','Blackglass Devourer','Crucible Warden'].entries()){ASHLANDS_ENCOUNTERS[`ASH_FEN_ELITE_0${index+1}`]=()=>elite(`ASH_FEN_ELITE_${index+1}`,name,94,'fire',index,39_000,7_000,2_500);ASHLANDS_ENCOUNTERS[`ASH_CRUCIBLE_ELITE_0${index+1}`]=()=>elite(`ASH_CRUCIBLE_ELITE_${index+1}`,name,94,'fire',index,41_000,7_200,2_550);}
