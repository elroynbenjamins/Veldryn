...
const setState={...state,character:{...state.character!,equipment:{...state.character!.equipment,gloves:'STONEHEART_GLOVES'}}};
const stoneheartInspect=itemInspectModel(setState,'STONEHEART_CHEST');
ok(stoneheartInspect.gearDecision?.set?.currentPieces===1&&stoneheartInspect.gearDecision.set.previewPieces===2,'Gear Check previews set-piece progress after equip');
ok(stoneheartInspect.gearDecision?.set?.reached?.pieces===2&&stoneheartInspect.gearDecision.set.next?.pieces===4,'Gear Check exposes reached and next v33 set milestones');
