...
export function SocialScreen({onGuild,onFriends,onAccount}:{onGuild:()=>void;onFriends:()=>void;onAccount:()=>void}){
 ...
 return <SocialHubPanel active={tab} onChange={next=>{setTab(next);setSelected(null);setDraft(null);}}><ScrollView contentContainerStyle={s.content}>
  ...
  {!social.accountId?<Panel><Text style={s.text}>{social.loading?'Loading account…':'Sign in and sync a character to use Parties and Recruitment.'}</Text><GameButton title="Account settings" onPress={onAccount}/></Panel>:<>
   ...
   {selected&&<Panel accentColor={T.info} accentSurface="#102536"><Text style={s.selectedLabel}>SELECTED RECRUITMENT</Text><Text style={s.title}>{selected.title}</Text><Text style={s.text}>{selected.body}</Text><Text style={s.text}>{recruitmentTimeLabel(selected.expiresAtMs,at).text}</Text>
...
function QuickChip({label,selected,onPress}:{label:string;selected:boolean;onPress:()=>void}){
 const T=useGameTheme(),s=useMemo(()=>makeStyles(T),[T]);return <Pressable accessibilityRole="button" accessibilityState={{selected}} onPress={onPress} style={({pressed})=>[s.chip,selected&&s.chipSelected,pressed&&s.pressed]}><Text style={[s.chipText,selected&&s.chipTextSelected]}>{selected?'✓ ':''}{label}</Text></Pressable>}
const makeStyles=(T:ThemePalette)=>StyleSheet.create({content:{padding:spacing.md,gap:spacing.md,paddingBottom:32},row:{flexDirection:'row',flexWrap:'wrap',gap:spacing.sm},chipRow:{flexDirection:'row',flexWrap:'wrap',alignItems:'center',gap:6},chipLabel:{fontSize:10,fontWeight:'900',letterSpacing:1,color:T.accentSoft,marginRight:2},chip:{minHeight:38,paddingHorizontal:12,justifyContent:'center',borderWidth:1,borderColor:T.line,borderRadius:99,backgroundColor:T.bg},chipSelected:{borderColor:T.action,backgroundColor:T.actionSurface},chipText:{fontSize:12,color:T.muted,fontWeight:'800'},chipTextSelected:{color:T.actionText},pressed:{opacity:.76},title:{color:T.accent,fontSize:18,fontWeight:'800'},selectedLabel:{...typography.caption,color:T.info,fontWeight:'900',letterSpacing:1},text:{color:T.text,lineHeight:21},errorCard:{gap:4,padding:spacing.md,borderWidth:1,borderColor:T.bad,borderRadius:10,backgroundColor:T.dark?'#321B22':'#FDECEE'},errorLabel:{...typography.caption,color:T.bad,fontWeight:'900',letterSpacing:1},error:{color:T.text,lineHeight:21},ad:{gap:spacing.sm,paddingVertical:spacing.sm}});
