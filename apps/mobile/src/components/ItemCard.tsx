...
export function ItemCard({item,quantity,equipped,selectedFood=false,onEquip,onSelectFood,onEat,onSell,onSalvage,onDeposit,onWithdraw,transferQuantity=1,transferIssue='',healAmount,onPreview}:{item:ItemDef;quantity:number;equipped?:ItemDef;selectedFood?:boolean;onEquip?:()=>void;onSelectFood?:()=>void;onEat?:()=>void;onSell?:()=>void;onSalvage?:()=>void;onDeposit?:()=>void;onWithdraw?:()=>void;transferQuantity?:number;transferIssue?:string;healAmount?:number;onPreview?:()=>void}){
  const gear=item.type==='gear',rarity=itemRarity(item);
  return <Panel>
    {onPreview&&<GameButton title="Try on / compare" tone="secondary" onPress={onPreview}/>}
    <View style={[s.heading,{borderLeftColor:rarityMeta(rarity).color,borderLeftWidth:gear?3:0,paddingLeft:gear?spacing.xs:0}]}><View style={s.copy}><Text style={s.name}>{item.name}</Text><Text style={[s.rarity,{color:rarityMeta(rarity).color}]}>{rarityLabel(item)} · {item.type.toUpperCase()}{item.slot?` · ${item.slot.toUpperCase()}`:''}</Text></View><Text style={s.qty}>×{quantity}</Text></View>
    {gear&&<><View style={s.stats}>{stats.map(({key,label})=>{const value=item[key]||0,current=equipped?.[key]||0,d=value-current;return <View key={key} style={s.stat}><Text style={s.statLabel}>{label}</Text><Text style={s.statValue}>{value}</Text><Text style={[s.delta,d>0?s.better:d<0?s.worse:s.same]}>{delta(value,current)}</Text></View>})}</View><Text style={s.compare}>{equipped?`Compared with ${equipped.name}`:`Changes versus an empty ${item.slot} slot`}</Text>{(rarity==='legendary'||rarity==='mythic')&&<Text style={s.passive}>◆ Passive: {item.passive||'Class-aligned bonus is active while equipped.'}</Text>}</>}
...
const s=StyleSheet.create({heading:{flexDirection:'row',justifyContent:'space-between',alignItems:'flex-start',gap:spacing.md},copy:{flex:1},name:{...typography.title,color:C.text},rarity:{...typography.caption,fontWeight:'900'},qty:{fontSize:18,color:C.accent,fontWeight:'900'},stats:{flexDirection:'row',gap:spacing.sm},stat:{flex:1,backgroundColor:C.panel2,borderRadius:radii.sm,padding:spacing.sm},statLabel:{...typography.caption,color:C.muted},statValue:{...typography.bodyStrong,color:C.text},delta:{...typography.caption,fontWeight:'900'},better:{color:C.good},worse:{color:C.bad},same:{color:C.muted},compare:{...typography.caption,color:C.info},passive:{...typography.bodyStrong,color:'#f0a24b'},value:{...typography.body,color:C.muted},food:{...typography.bodyStrong,color:C.good},salvage:{...typography.body,color:C.warning}});
