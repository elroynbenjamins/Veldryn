...
import {regionalCombatHandlerV1} from './regional-combat';
...
const services={
 ...
 adminQa:async token=>{const response=await fetch(`${url}/auth/v1/user`,{headers:{apikey:anonKey,Authorization:`Bearer ${token}`}});if(!response.ok)return false;const user=await response.json();const meta=user?.app_metadata??{};return meta.admin_qa===true||(Array.isArray(meta.roles)&&meta.roles.includes('admin_qa'));},
...
const gameplay=gameplayHandler(services),regionalCombat=regionalCombatHandlerV1(services);
