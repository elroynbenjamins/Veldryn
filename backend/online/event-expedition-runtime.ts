...
import {EventExpeditionService,MemoryEventRunRepository,type EventRun} from '../src/server/expeditions/event-service';
...
function project(run:EventRun,version:number,liveEventId:string){
 ...
 const options=run.phase==='awaiting_choice'&&current?current.nextNodeIds.map(id=>run.graph.nodes.find(node=>node.nodeId===id)).filter((node):node is NonNullable<typeof node>=>Boolean(node)).map(node=>({nodeId:node.nodeId,kind:node.kind,risk:node.risk,rewardTag:node.rewardTag})):[];
 return {
  ...
  options,settlement:{status:run.settlement,rewardMarks:run.rewardMarks??definition.rewardMarks},
 };
}
