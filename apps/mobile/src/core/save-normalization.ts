...
import {migrateToPerInstanceGear,normalizeGearInstances} from './gear-instances';
...
export function normalizeSave(input:any):GameState{
  ...
  return discoverCharacterSkins(reconcileCombatCompanionUnlocks(migrateToPerInstanceGear(normalized),normalized.createdAtMs));
}
