...
import {challengeFallenKnight,claimActivity,claimOverflowToBank,claimQuest,claimSeasonalContract,craftRecipe,createCharacter,depositAllMaterials,depositGearInstance,depositToBank,eatFood,equipFood,equipGatheringTool,equipGearInstance,equipItem,newGame,previewActivityReward,salvageGearInstance,salvageItem,sellGearInstance,sellItem,startHerbalism,travelToRegion,unequipItem,upgradeStorage,withdrawFromBank,withdrawGearInstance} from './src/core/game';
...
import {rewardProgressionMoments,type RewardProgressionMoment} from './src/core/reward-game-feel';
...
function VeldrynApp(){
  ...
  async function perform(command?:GameCommand){try{const before=stateRef.current,result=await online.execute(command);stateRef.current=result.state;setState(result.state);queueCustomizationUnlocks(before,result.state);if(result.reward)presentCollected(result.reward,result.activity??null,before,result.state);return result;}catch(error){Alert.alert('Online action',error instanceof Error?error.message:'Please retry.');return null;}}
  async function runCompanionCommand(command:GameCommand){
    ...
    await commit(result.state);if(result.reward)presentCollected(result.reward,result.activity??null,current,result.state);
  }
...
const next=discoverCharacterSkins(candidate);stateRef.current=next;setState(next);queueCustomizationUnlocks(current,next);try{await repo.save(next)}catch{Alert.alert('Local save failed','Progress is still in memory. Keep the app open and try another action to save again.')}}
  ...
  <RewardPopup reward={collected?.reward??null} activity={collected?.activity??null} welcomeBack={!!collected?.welcomeBack} progressionMoments={collected?.progressionMoments??[]} reduceMotion={state.settings.reduceMotion} numberMode={state.settings.numberMode} onClose={()=>setCollected(null)}/>
