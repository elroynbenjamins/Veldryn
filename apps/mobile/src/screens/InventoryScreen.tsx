...
import {ot} from '../i18n';
import {enhancedGearStats,gearEnhancement,gemSocketCapacity} from '../core/equipment-enhancement';
...
import {bestRarityForStack,bestStoredGearInstance,disposableStoredGearCount,rarityBreakdownForStack} from '../core/crafted-gear-instances';
...
export function InventoryScreen({state,onEquip,onFood,onEat,onSell,onSalvage,onDeposit,onDepositMaterials,onUpgradeStorage,onWithdraw,onOverflow,onToggleFavorite,onAcknowledgeItem,onAcknowledgeAll,onBulkAction,onNavigateInspect}:{state:GameState;onEquip:(id:string)=>void;onFood:(id:string)=>void;onEat:(id:string)=>void;onSell:(id:string)=>void;onSalvage:(id:string)=>void;onDeposit:(id:string,quantity:number)=>void;onDepositMaterials:()=>void;onUpgradeStorage:(location:StorageLocation)=>void;onWithdraw:(id:string,quantity:number)=>void;onOverflow:()=>void;onToggleFavorite:(id:string)=>void;onAcknowledgeItem:(id:string)=>void;onAcknowledgeAll:()=>void;onBulkAction:(kind:BulkAction,location:BulkStorageLocation,ids:string[])=>void;onNavigateInspect:(destination:WorkingTowardDestination)=>void}){
  ...
  const renderStack=(stack:ItemStack)=>{
    ...
    const bestInstance=item.type==='gear'?bestStoredGearInstance(state,item.id,location):undefined,disposable=item.type==='gear'?disposableStoredGearCount(state,item.id,location):stack.quantity,protectedCopies=item.type==='gear'?Math.max(0,stack.quantity-disposable):0;
    const breakdown=item.type==='gear'?rarityBreakdownForStack(state,item.id,location,stack.quantity):[],rarityBreakdown=breakdown.length>1?breakdown.map(row=>`${row.count}× ${row.rarity.toUpperCase()}`).join(' · '):undefined;
    const amount=transferAmount(stack.quantity,quantity),selectedFood=state.character?.equippedFoodId===item.id,enhancement=item.type==='gear'?gearEnhancement(state,item.id,bestInstance?.id):undefined,enhancementProtected=protectedCopies>0;
    return <ItemCard favorite={favorite} newItem={newItem} selectionMode={selectMode} selected={selectedSet.has(item.id)} onSelect={()=>toggleSelection(item.id)} onInspect={!selectMode?()=>setInspectId(item.id):undefined} onToggleFavorite={()=>run(()=>onToggleFavorite(item.id))} expanded={!selectMode&&expandedItem===location+':'+item.id} onToggle={()=>toggleItem(item.id,newItem)} key={`${location}:${item.id}`} item={item} quantity={stack.quantity} equipped={equippedId?itemDef(equippedId):undefined} rarityOverride={item.type==='gear'?bestRarityForStack(state,item.id,location,stack.quantity):undefined} rarityBreakdown={rarityBreakdown} upgradeRank={enhancement?.rank} socketed={enhancement?.gemIds.length} socketCapacity={item.type==='gear'?gemSocketCapacity(item.id):0} enhancementProtected={enhancementProtected} displayStats={item.type==='gear'?enhancedGearStats(state,item.id,bestInstance?.id):undefined} equippedDisplayStats={equippedId?enhancedGearStats(state,equippedId):undefined} selectedFood={selectedFood} healAmount={recoveryAmount(state,item.id)} transferQuantity={amount} transferIssue={transferError(state,item.id,amount,location)} numberMode={state.settings.numberMode}
      ...
      onSell={carried&&item.value>0&&disposable>0&&!favorite?()=>setPending({kind:'sell',item,quantity:1}):undefined} onSalvage={carried&&item.salvage&&disposable>0&&!favorite?()=>setPending({kind:'salvage',item,quantity:1}):undefined}
