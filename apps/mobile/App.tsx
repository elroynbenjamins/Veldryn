...
function VeldrynApp(){
  ...
  useEffect(()=>{if(!serverGameplayEnabled)return;const next=online.snapshot?.state??null,before=stateRef.current;queuePreparationNotices(before,next);queueFeatureUnlockMoments(before,next);stateRef.current=next;setState(next);setReady(!online.loading);setLoadError('');},[online.snapshot,online.loading]);
  ...
  async function perform(command?:GameCommand){try{const before=stateRef.current,result=await online.execute(command);queuePreparationNotices(before,result.state);queueFeatureUnlockMoments(before,result.state);stateRef.current=result.state;setState(result.state);queueCustomizationUnlocks(before,result.state);if(result.reward)presentCollected(result.reward,result.activity??null,before,result.state);else queueMasteryNotice(before,result.state);return result;}catch(error){Alert.alert('Online action',error instanceof Error?error.message:'Please retry.');return null;}}
...
const next=discoverCharacterSkins(candidate);queuePreparationNotices(current,next);stateRef.current=next;setState(next);queueCustomizationUnlocks(current,next);queueFeatureUnlockMoments(current,next);try{await repo.save(next)}catch{Alert.alert('Local save failed','Progress is still in memory. Keep the app open and try another action to save again.')}}
  ...
  return <GameThemeProvider themeId={state.settings.uiTheme}><SafeAreaView edges={['top','left','right']} style={[s.safe,{backgroundColor:theme.bg}]}><StatusBar style={theme.dark?'light':'dark'}/><GameTopBar state={state} nowMs={now} attention={quickNavigationBadges} labelForDestination={destination=>tabLabel(state.settings.language,destination)} lockReasonForDestination={destination=>earlyFeatureLockReason(state,destination)} onNavigate={destination=>{if(destination==='Dungeon'){setTab('Coop');return;}if(destination==='Quests'){openQuestMode('story');return;}setTab(destination)}} onChangeDestinations={destinations=>commit({...state,settings:{...state.settings,quickNavDestinations:destinations}})} onOpenActivity={openActiveActivity}/>{secondary&&tab!=='Coop'&&<View style={[s.backBar,{backgroundColor:theme.panel,borderColor:theme.line}]}><Pressable accessibilityRole="button" accessibilityLabel={t(state.settings.language,'common.back')} onPress={goBack} hitSlop={8} style={({pressed})=>[s.backButton,pressed&&s.backPressed]}><Image source={require('./assets/ui-icons-v2/small/back.png')} resizeMode="contain" style={s.backIcon}/><Text style={[s.backText,{color:theme.accent}]}>{t(state.settings.language,'common.back')}</Text></Pressable><Text numberOfLines={1} style={[s.backTitle,{color:theme.text}]}>{tabLabel(state.settings.language,tab)}</Text><View style={s.backSpacer}/></View>}{serverGameplayEnabled&&<View style={{paddingHorizontal:12}}>{!!online.error&&<Text accessibilityRole="alert" style={s.txt}>{online.error}</Text>}{online.pending?<GameButton title="Retry pending action" disabled={online.busy} onPress={()=>void perform()}/>:<Text style={s.txt}>{online.busy?'Saving online…':'Online · progress saved on server'}</Text>}</View>}<View pointerEvents={serverGameplayEnabled&&online.busy?'none':'auto'} style={s.body} {...backSwipe.panHandlers}>
    ...
    {tab==='Events'&&<EventScreen state={state} onChange={commit} onCommand={serverGameplayEnabled?command=>perform(command).then(Boolean):undefined} onOpenSeasonalExpedition={coopOnlineConfigured?liveEventId=>{const reason=earlyFeatureLockReason(state,'Coop');if(reason){Alert.alert('Dungeons locked',reason);return;}setPendingEventLiveId(liveEventId);setTab('Coop')}:undefined}/>}
