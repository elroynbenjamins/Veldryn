...
export function OnlineAccountPanel({state}:{state:GameState}){
 ...
 return <Panel><Text style={s.title}>{auth.recovering?'Reset your password':auth.session?'Your account':creating?'Create your VELDRYN account':'Welcome to VELDRYN'}</Text>
  ...
  {auth.recovering?<>{passwordInput}<GameButton title="Save new password" disabled={busy} onPress={()=>void run(async()=>{await updateAccountPassword(password);auth.clearRecovery();setNotice('Password updated.');})}/></>:!auth.session?<>
   ...
   {!creating&&<><View style={s.divider}><View style={s.dividerLine}/><Text style={s.dividerText}>OR</Text><View style={s.dividerLine}/></View><GameButton title="Continue as Guest" tone="secondary" disabled={busy} onPress={()=>void run(async()=>{await signInAsGuest();setNotice('Guest account created. You can secure it with email later.');})}/><Text style={s.guestHint}>Guest progress is stored on the server for this anonymous account. Secure it with email before signing out or changing devices.</Text></>}
