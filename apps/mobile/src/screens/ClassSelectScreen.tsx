...
import {spacing,equipmentTheme,type ThemeColors} from '../theme/theme';
...
const playStyle:Record<ClassDef['role'],string>={Tank:'Durable · Defensive · Protects allies',Damage:'Offensive · Focused · Defeats threats quickly',Support:'Helpful · Tactical · Strengthens allies'};
...
export function ClassSelectScreen({language='en',onLanguage,onSelect,onCancel,cancelLabel}:{language?:GameState['settings']['language'];onLanguage?:(language:GameState['settings']['language'])=>void;onSelect:(id:ClassId,name:string,body:BodyPresentation)=>Promise<void>|void;onCancel?:()=>void;cancelLabel?:string}){
  ...
  return <KeyboardAvoidingView style={s.screen} behavior={Platform.OS==='ios'?'padding':'height'}><ScrollView ref={scroll} style={s.scroll} contentContainerStyle={s.root} keyboardShouldPersistTaps="handled">
    ...
    {step==='class'&&<View style={s.section}>
      ...
      <View style={s.playStyleCard}><Text style={s.playStyleLabel}>PLAY STYLE</Text><Text style={[s.playStyleText,{color:roleColor[selected.role]}]}>{playStyle[selected.role]}</Text></View><View style={s.loadout}><View style={s.flex}><Text style={s.label}>STARTING WEAPON</Text><Text style={s.gearName}>{starter.name}</Text></View><Text style={s.weaponTag}>LV. 1</Text></View><Text style={s.note}>The class art previews its identity. Your adventure begins in a simple outfit with this weapon.</Text>
    </View>}
    {step==='review'&&<View style={s.section}>
      <Text accessibilityRole="header" style={s.heading}>{t(language,'onboarding.ready')}</Text><Text style={s.description}>One last look before {safeName} enters Asterfall. We will introduce combat, skills and equipment gradually after creation.</Text>
      <View style={s.reviewCard}><FixedCharacterPortrait classId={selected.id} body={body} compact/><View style={s.reviewCopy}><Text style={s.reviewName}>{safeName}</Text><Text style={[s.reviewRole,{color:roleColor[selected.role]}]}>{selected.name} · {selected.role}</Text><Text style={s.reviewLine}>{body==='male'?'Male':'Female'} presentation</Text><Text style={s.reviewLine}>Starting weapon: {starter.name}</Text><Text style={s.reviewLine}>Starting outfit: Simple adventurer gear</Text></View></View>
      <View style={s.firstStep}><Text style={s.firstStepLabel}>WHAT HAPPENS NEXT</Text><Text style={s.firstStepText}>Enter Asterfall, meet your first objective, and learn one part of the game at a time.</Text></View>
