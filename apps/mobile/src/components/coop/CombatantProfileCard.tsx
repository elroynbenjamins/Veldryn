...
export function CombatantProfileCard({slot,active=false,targeted=false,assistProc=false,motionStyle,currentCue}:{slot:Slot;active?:boolean;targeted?:boolean;assistProc?:boolean;motionStyle?:any;currentCue?:CoopCombatReplayCueView}){
 ...
 return <Animated.View accessibilityLabel={`${slot.name}, ${avatar?.label??slot.classId??slot.role}, ${roleLabel(slot.role)}, ${Math.round(pct*100)} percent health`} style={[s.card,{borderColor:accent},targeted&&s.targeted,active&&s.active,slot.ready===false&&s.down,motionStyle]}>
  <View style={s.scene}>
   ...
   {float?<><View style={[s.floatPill,float.kind==='damage'?s.floatDamage:float.kind==='heal'?s.floatHeal:float.kind==='shield'?s.floatShield:s.floatMiss]}><Text style={[s.floatText,float.kind==='damage'?s.floatDamageText:float.kind==='heal'?s.floatHealText:float.kind==='shield'?s.floatShieldText:s.floatMissText]}>{float.label}</Text></View>{float.outcome?<View style={[s.outcomePill,float.outcome==='CRIT'?s.outcomeCrit:float.outcome==='DODGE'?s.outcomeMiss:float.outcome.startsWith('BARRIER')?s.outcomeBarrier:s.outcomeGem]}><Text style={s.outcomeText}>{float.outcome}</Text></View>:null}</>:null}
