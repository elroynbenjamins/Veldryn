...
function emptyEnhancement():GearEnhancementState{return {rank:0,failures:0,gemIds:[]};}
function equippedInstanceIdForItem(state:GameState,itemId:string){
  const projected=migrateToPerInstanceGear(state),item=itemDef(itemId);if(item.type!=='gear'||!item.slot)return undefined;
  return projected.character?.equipment[item.slot]===itemId?projected.character.equipmentInstanceIds?.[item.slot]:undefined;
}
function resolveInstance(state:GameState,itemId:string,instanceId?:string){
  const id=instanceId??equippedInstanceIdForItem(state,itemId);if(!id)return undefined;
  const instance=gearInstanceById(state,id);if(!instance||instance.itemId!==itemId)throw new Error('Equipment copy does not match this item');
  return instance;
}

export function gearEnhancement(state:GameState,itemId:string,instanceId?:string):GearEnhancementState{
  const raw=resolveInstance(state,itemId,instanceId)?.enhancement;if(!raw)return emptyEnhancement();
  const slots=normalizeEnhancementGemSlots(raw);
...
export function hasEnhancement(state:GameState,itemId:string,instanceId?:string){const enhancement=gearEnhancement(state,itemId,instanceId);return enhancement.rank>0||enhancement.gemIds.length>0;}
...
export function gemSocketState(state:GameState,itemId:string,instanceId?:string){const enhancement=gearEnhancement(state,itemId,instanceId);return {statGemId:enhancement.statGemId,effectGemId:enhancement.effectGemId,filled:Number(Boolean(enhancement.statGemId))+Number(Boolean(enhancement.effectGemId)),capacity:gemSocketCapacity(itemId)};}
function instanceRarity(state:GameState,itemId:string,instanceId?:string){return resolveInstance(state,itemId,instanceId)?.rarity??itemRarity(itemDef(itemId));}
export function upgradeQuote(state:GameState,itemId:string,instanceId?:string){
  ...
  const current=gearEnhancement(state,itemId,instanceId),targetRank=current.rank+1;
  ...
  const rarity=instanceRarity(state,itemId,instanceId),pity=Math.min(.10,current.failures*.02);
...
function setEnhancement(state:GameState,instanceId:string,value:GearEnhancementState){const slots=normalizeEnhancementGemSlots(value);return updateGearInstance(state,instanceId,instance=>({...instance,enhancement:{...value,...slots}}));}
function requireEquipped(state:GameState,instanceId:string){
  const projected=migrateToPerInstanceGear(state),instance=gearInstanceById(projected,instanceId);if(!instance)throw new Error('Equipment copy not found');
  if(!projected.character||!Object.values(projected.character.equipmentInstanceIds??{}).includes(instanceId))throw new Error('Equip this exact copy first');
  return {state:projected,instance};
}
export function attemptEquipmentUpgrade(state:GameState,instanceId:string,roll=Math.random()){
  const required=requireEquipped(state,instanceId),itemId=required.instance.itemId;state=required.state;
  if(roll<0||roll>=1)throw new Error('Invalid upgrade roll');
  const quote=upgradeQuote(state,itemId,instanceId);if(quote.maxed)throw new Error('This item is already +10');
  ...
  const prior=gearEnhancement(state,itemId,instanceId),success=roll<quote.successChance,rank=success?quote.targetRank:prior.rank;
  next=setEnhancement(next,instanceId,{...prior,rank,failures:success?0:prior.failures+1});
  return {state:next,result:{success,oldRank:prior.rank,newRank:rank,successChance:quote.successChance,downgraded:false}};
}
export function socketGem(state:GameState,instanceId:string,gemId:string){
  const required=requireEquipped(state,instanceId),itemId=required.instance.itemId;state=required.state;
  const gem=safeGem(gemId);if(!gem)throw new Error('That item is not a gem');
  const kind=gemSocketKind(gemId),enhancement=gearEnhancement(state,itemId,instanceId);
  ...
  next=setEnhancement(next,instanceId,{...enhancement,statGemId:kind==='stat'?gemId:enhancement.statGemId,effectGemId:kind==='effect'?gemId:enhancement.effectGemId,gemIds:[]});
  return next;
}
...
export function unsocketGem(state:GameState,instanceId:string,index:number){
  const required=requireEquipped(state,instanceId),itemId=required.instance.itemId;state=required.state;
  if(index!==0&&index!==1)throw new Error('Unknown gem socket');
  const enhancement=gearEnhancement(state,itemId,instanceId),gemId=index===0?enhancement.statGemId:enhancement.effectGemId;
  ...
  next=setEnhancement(next,instanceId,{...enhancement,statGemId:index===0?undefined:enhancement.statGemId,effectGemId:index===1?undefined:enhancement.effectGemId,gemIds:[]});
  return next;
}
...
export function gemEffectDescription(gemId:string){
  const gem=itemDef(gemId);if(gem.type!=='gem'||!gem.gemEffect)return '';const pct=Math.round((gem.gemEffectValue??0)*100);
  switch(gem.gemEffect){case 'combat_speed':return `+${pct}% combat speed`;case 'boss_power':return `+${pct}% combat power against bosses`;case 'damage_reduction':return `-${pct}% incoming combat damage`;case 'recovery':return `+${pct}% between-kill recovery`;}
}
