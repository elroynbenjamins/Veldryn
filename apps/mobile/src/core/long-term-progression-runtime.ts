...
export function applyTrustedLongTermProgression(input:GameState,events:TrustedProgressionActivity[],reward:RewardBundle|undefined,nowMs:number,options:TrustedProgressionOptions):TrustedProgressionResult{
 const contractsInitiallyUnlocked=earlyFeatureUnlocked(input,'contracts');
 let state=contractsInitiallyUnlocked?reconcileWeeklyOrderRollover(input,nowMs).state:input;const account={...state.account},metrics={...(account.longTermMetrics??{})},mastery={...(account.professionMasteryByAction??{})};state={...state,account:{...account,longTermMetrics:metrics,professionMasteryByAction:mastery}};
 const contractsUnlocked=earlyFeatureUnlocked(state,'contracts'),weekly=contractsUnlocked?ensureWeeklyOrders(state,options.accountId,nowMs):undefined,weeklyCompleted:string[]=[],pending=[...(state.account.weeklyOrderPendingRewards??[])],pendingKeys=new Set(pending.map(row=>row.claimKey));
 ...
 for(const event of events){const units=integerUnits(event.units);if(!units)continue;
  ...
  if(weekly&&event.kind==='combat')applyWeeklyOrderProgress(weekly,{eventId:`${options.eventId}:${event.kind}:${event.contentId}`,characterId:state.character?.id??'unknown',kind:'hunt',targetId:event.contentId,amount:units,completedAtMs:nowMs});
  if(weekly&&event.kind==='boss'&&event.weeklyEligible!==false)applyWeeklyOrderProgress(weekly,{eventId:`${options.eventId}:boss:${event.contentId}`,characterId:state.character?.id??'unknown',kind:'hunt',targetId:event.contentId,amount:units,completedAtMs:nowMs});
  if(weekly&&event.kind==='combat'&&event.challengeId)applyWeeklyOrderProgress(weekly,{eventId:`${options.eventId}:threat:${event.contentId}:${event.challengeId}`,characterId:state.character?.id??'unknown',kind:'threat',targetId:`${event.contentId}:${event.challengeId}`,amount:units,completedAtMs:nowMs});
  if(weekly&&(event.kind==='gathering'||event.kind==='crafting'))applyWeeklyOrderProgress(weekly,{eventId:`${options.eventId}:${event.kind}:${event.contentId}`,characterId:state.character?.id??'unknown',kind:'profession',targetId:event.contentId,amount:units,completedAtMs:nowMs});
  const regionId=trustedEventRegionId(event);if(weekly&&regionId)applyWeeklyOrderProgress(weekly,{eventId:`${options.eventId}:regional:${regionId}:${event.kind}:${event.contentId}`,characterId:state.character?.id??'unknown',kind:'regional',targetId:regionId,amount:units,completedAtMs:nowMs});
 }
 if(weekly)for(const order of weekly.orders){if(order.progress>=order.target&&!wasComplete.has(order.id)){weeklyCompleted.push(order.id);metrics['weekly_orders.completed']=(metrics['weekly_orders.completed']??0)+1}if(order.progress>=order.target&&!order.claimed){const claim=claimWeeklyOrder(weekly,order.id);if(!pendingKeys.has(claim.claimKey)){pending.push({claimKey:claim.claimKey,rewardRef:claim.reward.rewardRef,label:claim.reward.label,weekKey:claim.weekKey,orderId:claim.orderId});pendingKeys.add(claim.claimKey)}}}
 if(weekly&&weekly.orders.length&&weekly.orders.every(row=>row.progress>=row.target)&&!weekly.completionClaimed){const claim=claimWeeklyCompletion(weekly);if(!pendingKeys.has(claim.claimKey)){pending.push({claimKey:claim.claimKey,rewardRef:claim.reward.rewardRef,label:claim.reward.label,weekKey:claim.weekKey});pendingKeys.add(claim.claimKey)}}
 state={...state,account:{...state.account,...(weekly?{weeklyOrders:weekly}:{}),weeklyOrderPendingRewards:pending.slice(-100),longTermMetrics:metrics,professionMasteryByAction:mastery}};
