...
export function ExplorationPanel({state,onCommand}:{state:GameState;onCommand:(command:GameCommand)=>Promise<void>}){
 ...
 const regionId=currentRegionId(state),region=WORLD_ZONES.find(item=>item.id===regionId),explorationLevel=state.skills.find(item=>item.skillId==='exploration')?.level??1;
 ...
 return <Panel>
  <View style={s.heading}><View style={s.flex}><Text style={s.eyebrow}>SCOUTING ROUTES</Text><Text style={s.title}>Explore the world</Text><Text style={s.body}>Scout your current region to earn Exploration XP, reveal encounters, and discover the path into later regions. Combat also grants a small amount of passive Exploration XP.</Text></View><View style={s.regionBadge}><Text style={s.regionBadgeLabel}>EXP LV {explorationLevel}</Text><Text numberOfLines={2} style={s.regionBadgeValue}>{region?.name??regionId}</Text></View></View>
  {routes.map(route=>{const zone=WORLD_ZONES.find(item=>item.id===route.zoneId),here=regionId===route.zoneId,levelReady=explorationLevel>=route.requiredLevel,ready=here&&levelReady,found=!!route.unlockMonsterId&&state.unlockedMonsterIds.includes(route.unlockMonsterId);const stateLabel=found?'DISCOVERED':!levelReady?`EXP LV ${route.requiredLevel}`:!here?'TRAVEL':'READY';return <View key={route.id} style={[s.route,ready&&s.routeReady]}>
   ...
   <GameButton compact title={found&&ready?'Scout again':ready?'Scout':!levelReady?`Requires Exploration ${route.requiredLevel}`:`Travel to ${zone?.name??route.zoneId}`} disabled={!ready} tone={ready?'primary':'secondary'} onPress={()=>void onCommand({type:'explore',args:{id:route.id}})}/>
  </View>})}
 </Panel>;
}
