...
export function SettingsScreen({state,onLanguage,onReset,onChange,onExport,onImport,onOpenChatPilot,onOpenChatEmotes,onOpenCoopUiGallery,online=false}:Props){
  ...
  return <><ScrollView contentContainerStyle={[s.root,{backgroundColor:theme.bg}]} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
    ...
    {section==='gameplay'&&<><Panel>
      ...
      <View style={s.afkBox}><View style={s.afkHead}><Text style={[s.settingLabel,{color:theme.text,marginTop:0}]}>AFK reserve</Text><Text style={[s.afkValue,{color:theme.good}]}>{afk.hours}h</Text></View><Text style={[s.muted,{color:theme.muted}]}>{afk.baseHours}h base · up to {afk.freeMaxHours}h through progression · {afk.maxHours}h maximum</Text><Text style={[s.muted,{color:theme.muted}]}>Earned milestones include class, quest, boss, guild and character-slot progression through slot #5.</Text><Text style={[s.afkPaid,{color:theme.info}]}>Final +6h: VIP +2h · VIP+ +2h extra · Supporter +2h</Text></View>
