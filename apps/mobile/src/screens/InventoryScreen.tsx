...
import {GameState,ItemStack,type GearInstance} from '../core/types';
...
import {gearInstancesForStorage} from '../core/gear-instances';
...
type Pending={kind:'sell'|'salvage'|'deposit';item:ItemDef;quantity:number;instanceId?:string}|null;
type InventoryRow={key:string;stack:ItemStack;instance?:GearInstance;copyLabel?:string};
type InspectTarget={itemId:string;instanceId?:string};
...
export function InventoryScreen({state,onEquip,onFood,onEat,onSell,onSellInstance,onSalvage,onDeposit,onDepositInstance,onDepositMaterials,onUpgradeStorage,onWithdraw,onWithdrawInstance,onOverflow,onToggleFavorite,onAcknowledgeItem,onAcknowledgeAll,onBulkAction,onNavigateInspect}:{state:GameState;onEquip:(instanceId:string)=>void;onFood:(id:string)=>void;onEat:(id:string)=>void;onSell:(id:string)=>void;onSellInstance:(instanceId:string)=>void;onSalvage:(instanceId:string)=>void;onDeposit:(id:string,quantity:number)=>void;onDepositInstance:(instanceId:string)=>void;onDepositMaterials:()=>void;onUpgradeStorage:(location:StorageLocation)=>void;onWithdraw:(id:string,quantity:number)=>void;onWithdrawInstance:(instanceId:string)=>void;onOverflow:()=>void;onToggleFavorite:(id:string)=>void;onAcknowledgeItem:(id:string)=>void;onAcknowledgeAll:()=>void;onBulkAction:(kind:BulkAction,location:BulkStorageLocation,ids:string[])=>void;onNavigateInspect:(destination:WorkingTowardDestination)=>void}){
  ...
  const confirm=()=>{if(!pending)return;run(()=>{if(pending.kind==='deposit')return pending.instanceId?onDepositInstance(pending.instanceId):onDeposit(pending.item.id,pending.quantity);if(pending.kind==='sell')return pending.instanceId?onSellInstance(pending.instanceId):onSell(pending.item.id);if(!pending.instanceId)throw new Error('Select the exact equipment copy to salvage.');return onSalvage(pending.instanceId)});setPending(null)};
  ...
  const rows:InventoryRow[]=stacks.flatMap(stack=>{
    const item=itemDef(stack.itemId);if(item.type!=='gear')return [{key:item.id,stack}];
    const copies=gearInstancesForStorage(state,location,item.id,location==='inventory'?state.character?.id:undefined).slice().sort((a,b)=>a.createdAtMs-b.createdAtMs||a.id.localeCompare(b.id));
    return copies.map((instance,index)=>({key:instance.id,stack:{itemId:item.id,quantity:1},instance,copyLabel:`COPY ${index+1}/${copies.length}`}));
  });
  ...
  const selectShown=()=>{const ids=rows.map(row=>row.key).slice(0,100);setSelectedIds(ids);setError(rows.length>100?'Selected the first 100 matching entries.':'')};
  const beginSelection=()=>{setSelectMode(true);setSelectedIds([]);setExpandedItem(null);setPreviewTarget(null);setInspectTarget(null);setError('')};
  ...
  const closeInspect=()=>{if(inspectTarget&&newItemSet.has(inspectTarget.itemId))run(()=>onAcknowledgeItem(inspectTarget.itemId));setInspectTarget(null)};
  const toggleItem=(rowKey:string,itemId:string,newItem:boolean)=>{const key=location+':'+rowKey,closing=expandedItem===key;if(closing&&newItem)run(()=>onAcknowledgeItem(itemId));setExpandedItem(closing?null:key)};
  const renderRow=(row:InventoryRow)=>{
    const stack=row.stack,item=itemDef(stack.itemId),instanceId=row.instance?.id,equippedId=item.slot?state.character?.equipment[item.slot]:undefined,equippedInstanceId=item.slot?state.character?.equipmentInstanceIds?.[item.slot]:undefined,carried=location==='inventory',favorite=favoriteSet.has(item.id),newItem=newItemSet.has(item.id);
    const amount=item.type==='gear'?1:transferAmount(stack.quantity,quantity),selectedFood=state.character?.equippedFoodId===item.id,enhancement=item.type==='gear'?gearEnhancement(state,item.id,instanceId):undefined,enhancementProtected=item.type==='gear'&&!!instanceId&&hasEnhancement(state,item.id,instanceId);
    return <ItemCard favorite={favorite} newItem={newItem} copyLabel={row.copyLabel} selectionMode={selectMode} selected={selectedSet.has(row.key)} onSelect={()=>toggleSelection(row.key)} onInspect={!selectMode?()=>setInspectTarget({itemId:item.id,instanceId}):undefined} onToggleFavorite={()=>run(()=>onToggleFavorite(item.id))} expanded={!selectMode&&expandedItem===location+':'+row.key} onToggle={()=>toggleItem(row.key,item.id,newItem)} key={`${location}:${row.key}`} item={item} quantity={stack.quantity} equipped={equippedId?itemDef(equippedId):undefined} rarityOverride={row.instance?.rarity} upgradeRank={enhancement?.rank} socketed={enhancement?.gemIds.length} socketCapacity={item.type==='gear'?gemSocketCapacity(item.id):0} enhancementProtected={enhancementProtected} displayStats={item.type==='gear'?enhancedGearStats(state,item.id,instanceId):undefined} equippedDisplayStats={equippedId?enhancedGearStats(state,equippedId,equippedInstanceId):undefined} selectedFood={selectedFood} healAmount={recoveryAmount(state,item.id)} transferQuantity={amount} transferIssue={item.type==='gear'?'':transferError(state,item.id,amount,location)} numberMode={state.settings.numberMode}
      onPreview={item.type==='gear'&&instanceId?()=>run(()=>{previewEquipment(state,item.id,instanceId);setPreviewTarget({itemId:item.id,instanceId})}):undefined}
      onEquip={carried&&item.type==='gear'&&instanceId?()=>run(()=>onEquip(instanceId)):undefined} onSelectFood={carried&&item.type==='food'?()=>run(()=>onFood(item.id)):undefined} onEat={carried&&item.type==='food'?()=>run(()=>onEat(item.id)):undefined}
      onSell={carried&&item.value>0&&!enhancementProtected&&!favorite?()=>setPending({kind:'sell',item,quantity:1,instanceId}):undefined} onSalvage={carried&&item.salvage&&instanceId&&!enhancementProtected&&!favorite?()=>setPending({kind:'salvage',item,quantity:1,instanceId}):undefined}
      onDeposit={carried?(instanceId?()=>run(()=>onDepositInstance(instanceId)):()=>selectedFood?setPending({kind:'deposit',item,quantity:amount}):run(()=>onDeposit(item.id,amount))):undefined} onWithdraw={!carried?(instanceId?()=>run(()=>onWithdrawInstance(instanceId)):()=>run(()=>onWithdraw(item.id,amount))):undefined}/>;
  };
  const foodWarning=pending?.item.id===state.character?.equippedFoodId&&!pending?.instanceId?' This is your selected auto-eat food. Only food carried in Inventory can be consumed in combat.':'';
  ...
  return <><EquipmentPreview state={state} itemId={previewTarget?.itemId??null} instanceId={previewTarget?.instanceId} onClose={()=>setPreviewTarget(null)}/><ItemQuickInspect state={state} itemId={inspectTarget?.itemId??null} instanceId={inspectTarget?.instanceId} onClose={closeInspect} onNavigate={destination=>{closeInspect();onNavigateInspect(destination)}}/><ScrollView contentContainerStyle={s.root} keyboardShouldPersistTaps="handled">
    ...
    <View style={s.resultRow}><Text style={[s.sub,s.resultSummary]}>{selectMode?`${selectedIds.length} selected · ${rows.length} matching entries`:`${rows.length} matching entries · ${activeCapacity.free} free slots${favorites.length?` · ${favorites.length} favorites`:``}${newItemIds.length?` · ${newItemIds.length} new`:``}`}</Text><View style={s.resultActions}>{!selectMode&&newItemIds.length>0&&<Pressable accessibilityRole="button" accessibilityLabel="Mark all new items as seen" onPress={()=>run(onAcknowledgeAll)} style={({pressed})=>[s.markSeen,pressed&&s.pressed]}><Text style={s.markSeenText}>Mark all seen</Text></Pressable>}{rows.length>0&&<Pressable accessibilityRole="button" accessibilityLabel={selectMode?`Finish selecting items`:`Select multiple items`} onPress={selectMode?exitSelection:beginSelection} style={({pressed})=>[s.selectModeButton,selectMode&&s.selectModeButtonActive,pressed&&s.pressed]}><Text style={[s.selectModeText,selectMode&&s.selectModeTextActive]}>{selectMode?`Done`:`Select`}</Text></Pressable>}</View></View>
    ...
    {rows.length?rows.map(renderRow):<><EmptyState title="No items to show" message="Try another storage tab or clear the search and category filter."/><GameButton title="Clear filters" tone="secondary" onPress={()=>{setQuery('');setFilter('all')}}/></>}
