...
type GuildSection='Overview'|'Hall'|'PvE'|'Roster'|'Customize';
...
export function GuildScreen({state,onChange,onlineDirectory,onlineManagement,onlinePve,online=false}:{online?:boolean;state:GameState;onChange:(next:GameState)=>void;onlineDirectory?:ReactNode;onlineManagement?:ReactNode;onlinePve?:ReactNode}){
  ...
  if(online)return <ScrollView contentContainerStyle={s.root}><Text style={s.h}>Guild</Text><View style={s.tabs}>{(['Overview','Hall','PvE','Roster','Customize'] as const).map(value=><View key={value} style={s.flex}><GameButton title={value} tone={section===value?'primary':'secondary'} onPress={()=>setSection(value)}/></View>)}</View>{section==='Overview'&&<>{onlineDirectory}<Panel><Text style={s.title}>Guild overview</Text><Text style={s.sub}>Browse recruitment when you are guildless. Once joined, use Hall, PvE, Roster and Customize for the relevant guild systems.</Text></Panel></>}{section==='Hall'&&<OnlineGuildHallPanel/>}{section==='PvE'&&onlinePve}{section==='Roster'&&onlineManagement}{section==='Customize'&&<OnlineGuildCustomizationPanel/>}</ScrollView>;
  ...
  return <ScrollView contentContainerStyle={s.root}>
    ...
    <View style={s.tabs}>{(['Overview','Hall','PvE','Roster','Customize'] as const).map(value=><View key={value} style={s.flex}><GameButton title={value} tone={section===value?'primary':'secondary'} onPress={()=>setSection(value)}/></View>)}</View>
    {section==='Overview'&&<><Panel><GuildCrest/><Text style={s.title}>{joined?'The Bloomwardens':'Find your guild'}</Text><Text style={s.sub}>{joined?'A friendly expedition guild focused on steady PvE progress.':'Join a guild to unlock shared projects, guild PvE, and the AFK reserve bonus.'}</Text>{joined&&<Text style={s.cap}>Launch progression · Guild Level cap 10 · Member capacity 12 → 20 through Open Halls</Text>}<GameButton title={joined?'Leave guild':'Join The Bloomwardens'} tone={joined?'danger':'primary'} onPress={()=>onChange({...state,account:{...state.account,guildMember:!joined}})}/></Panel>{onlineDirectory}{onlineManagement}</>}
