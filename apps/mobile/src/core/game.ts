...
export function newGame(nowMs:number):GameState{return {
  version:6,createdAtMs:nowMs,character:null,inventory:{stacks:[],capacity:30},bank:{stacks:[],capacity:120},overflow:{stacks:[],expiresAtMs:null},activity:null,currentRegionId:'GREENFIELDS',discoveredRegionIds:['GREENFIELDS'],
...
function previewStandardActivityRewardRaw(state:GameState,effectiveNowMs:number):RewardBundle{
  ...
  const explorationRaw=Math.floor(sim.kills*m.xp*effect.xpMultiplier*multipliers.skillXpMultiplier*challengeReward.xp*.03);
  const explorationXp=Math.min(Math.max(0,totalXpAtLevel(100)-(explorationSkill?.xp??0)),Math.max(0,explorationRaw));
...
export function claimActivity(state:GameState,nowMs:number){
  ...
  if(state.activity.kind!=='combat'){
    ...
    const nextBase={...state,skills,...routed,rewardRemainders:reward.nextRewardRemainders,unlockedMonsterIds:[...new Set([...state.unlockedMonsterIds,...(reward.explorationDiscoveries??[])])],discoveredRegionIds:[...new Set([...(state.discoveredRegionIds??['GREENFIELDS']),...(reward.explorationRegionUnlocks??[])])],activity:idleWindow.shouldStop?null:{...state.activity,lastClaimAtMs:settledAtMs,progressFraction:reward.nextProgressFraction}} as GameState;
