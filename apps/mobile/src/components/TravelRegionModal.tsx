  ...
  return <GameModalSurface visible={visible} presentation="sheet" onClose={onClose} backdropLabel="Close travel destination">
    ...
    <ScrollView bounces={false} showsVerticalScrollIndicator={false} contentContainerStyle={s.content}>
      <View style={[s.hero,{borderColor:zone.accent}]}>
        <ZoneSceneArtwork regionId={zone.id} blurRadius={2}/>
        <View style={s.heroFade}/>
        <View style={s.heroCopy}>
          <Text style={s.level}>LEVELS {zone.minLevel}–{zone.maxLevel}</Text>
      ...
      <Text style={s.hint}>Travelling changes your active region immediately. Available hunts, gathering nodes and regional activities update to {zone.name}.</Text>
    </ScrollView>
    <View style={s.actions}><View style={s.flex}><GameButton title="Cancel" tone="secondary" onPress={onClose}/></View><View style={s.flex}><GameButton title="Travel" onPress={()=>onTravel(zone.id)}/></View></View>
  </GameModalSurface>;
}
...
function makeStyles(C:ThemeColors){const equipmentColors=equipmentTheme(C);return StyleSheet.create({
  ...
  info:{gap:3,padding:spacing.sm,borderLeftWidth:3,borderLeftColor:C.info,backgroundColor:C.infoSurface},
