...
import {touchTargetPreferred} from '../theme/theme';
...
export function GameTopBar({state,nowMs,labelForDestination,onNavigate,onChangeDestinations,onOpenActivity}:Props){
  const C=useGameTheme(),styles=useMemo(()=>makeStyles(C),[C]);
...
const makeStyles=(C:ThemePalette)=>StyleSheet.create({
  shell:{paddingTop:Platform.OS==='android'?(NativeStatusBar.currentHeight??24)+6:6,minHeight:70,flexDirection:'row',alignItems:'center',gap:8,paddingHorizontal:10,paddingBottom:8,backgroundColor:C.bg,borderBottomWidth:1,borderBottomColor:C.lineStrong},
  environmentButton:{width:54,height:44,position:'relative',overflow:'hidden',flexDirection:'row',alignItems:'center',justifyContent:'center',gap:2,borderWidth:1,backgroundColor:C.panel2,borderRadius:8},environmentDivider:{width:1,height:22,backgroundColor:C.line},seasonStrip:{position:'absolute',left:0,right:0,bottom:0,height:2},lockedDot:{position:'absolute',right:3,top:3,width:6,height:6,borderRadius:3,backgroundColor:C.accent},
  hpBlock:{flex:1,minWidth:72,gap:4},hpHeading:{flexDirection:'row',alignItems:'center',justifyContent:'space-between'},hpLabel:{color:C.muted,fontSize:9,fontWeight:'900',letterSpacing:.8},hpTrack:{height:8,overflow:'hidden',backgroundColor:C.stage,borderWidth:1,borderColor:C.line,borderRadius:4},hpFill:{height:'100%',backgroundColor:C.good,borderRadius:3},hpValue:{color:C.text,fontSize:10,fontWeight:'800',fontVariant:['tabular-nums']},
  goldBlock:{width:72,alignItems:'flex-end'},goldLabel:{color:C.muted,fontSize:9,fontWeight:'900',letterSpacing:1},goldValue:{width:'100%',color:C.accentSoft,fontSize:13,fontWeight:'900',textAlign:'right',fontVariant:['tabular-nums']},
  menuButton:{width:touchTargetPreferred,height:touchTargetPreferred,alignItems:'center',justifyContent:'center',gap:5,borderWidth:1,borderColor:C.lineStrong,backgroundColor:C.panel2,borderRadius:8},menuLine:{width:23,height:3,backgroundColor:C.accentSoft,borderRadius:2},pressed:{opacity:.66},
  modalRoot:{flex:1,justifyContent:'flex-end',backgroundColor:C.overlay},sheet:{maxHeight:'82%',paddingHorizontal:14,paddingTop:16,paddingBottom:Platform.OS==='android'?20:32,backgroundColor:C.bg,borderTopWidth:2,borderTopColor:C.lineStrong,borderTopLeftRadius:18,borderTopRightRadius:18},
  sheetHeader:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginBottom:12},sheetEyebrow:{color:C.accent,fontSize:10,fontWeight:'900',letterSpacing:1.4},sheetTitle:{color:C.text,fontSize:22,fontWeight:'900'},closeButton:{width:44,height:44,alignItems:'center',justifyContent:'center'},closeText:{color:C.muted,fontSize:30,lineHeight:32},helper:{color:C.muted,fontSize:12,marginBottom:8},
  shortcutList:{gap:7},shortcut:{minHeight:52,flexDirection:'row',alignItems:'center',gap:10,paddingHorizontal:10,backgroundColor:C.panel,borderWidth:1,borderColor:C.line,borderRadius:8},shortcutNumber:{width:26,height:26,alignItems:'center',justifyContent:'center',backgroundColor:C.actionSurface,borderWidth:1,borderColor:C.action,borderRadius:6},shortcutNumberText:{color:C.text,fontWeight:'900'},shortcutIcon:{width:28,height:28},shortcutText:{flex:1,color:C.text,fontSize:15,fontWeight:'800'},chevron:{color:C.accentSoft,fontSize:26},customizeButton:{minHeight:48,alignItems:'center',justifyContent:'center',marginTop:12,borderWidth:1,borderColor:C.lineStrong,borderRadius:8},customizeText:{color:C.accentSoft,fontSize:14,fontWeight:'900'},
  choiceScroll:{maxHeight:410},choiceList:{gap:6,paddingBottom:8},choice:{minHeight:48,flexDirection:'row',alignItems:'center',gap:10,paddingHorizontal:11,backgroundColor:C.panel,borderWidth:1,borderColor:C.line,borderRadius:7},choiceSelected:{backgroundColor:C.actionSurface,borderColor:C.action},choiceBlocked:{opacity:.38},choiceIcon:{width:25,height:25},choiceIconDim:{opacity:.65},choiceText:{flex:1,color:C.text,fontSize:14,fontWeight:'800'},check:{width:24,height:24,alignItems:'center',justifyContent:'center',borderWidth:1,borderColor:C.muted,borderRadius:5},checkSelected:{backgroundColor:C.action,borderColor:C.action},checkText:{color:C.actionText,fontWeight:'900'},
  actions:{flexDirection:'row',gap:10,marginTop:10},secondaryButton:{minHeight:48,flex:1,alignItems:'center',justifyContent:'center',borderWidth:1,borderColor:C.line,borderRadius:8},secondaryText:{color:C.text,fontWeight:'800'},saveButton:{minHeight:48,flex:1,alignItems:'center',justifyContent:'center',backgroundColor:C.accent,borderRadius:8},saveDisabled:{opacity:.38},saveText:{color:C.dark?'#1A1205':'#FFFFFF',fontWeight:'900'},
});
