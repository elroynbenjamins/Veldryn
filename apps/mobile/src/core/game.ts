...
import {enhancedGearStats,equippedEffectGemBonuses,equippedGemBonuses,hasEnhancement,idleEffectGemProjectionV34} from './equipment-enhancement';
...
function simulateCombat(state:GameState,monsterId:string,elapsed:number){
  ...
  const effectGems=equippedEffectGemBonuses(state),v34Effects=idleEffectGemProjectionV34(state,m.boss),baseCritChance=CLASSES.find(def=>def.id===c.classId)?.role==='Damage'?.10:.05,setCombat=equipmentSetCombatModifiers(state,baseCritChance,.84);
  const boostedDefense=Math.max(1,Math.round(stats.defense*modifiers.combatPowerMultiplier));
  const bossPowerMultiplier=m.boss?1+effectGems.boss_power+v34Effects.bossPower:1;
  ...
  const speed=Math.max(COMBAT_SPEED_MIN,Math.min(COMBAT_SPEED_MAX,boostedPower/Math.max(1,expected)))*style.speedMultiplier*tactic.speedMultiplier*modifiers.combatSpeedMultiplier*companion.outputMultiplier*(1+monsterMastery(state,monsterId).damageBonus)*(1+effectGems.combat_speed+v34Effects.speed)*setCombat.speedMultiplier*setOutput;
  ...
  for(let i=0;i<theoreticalKills;i++){
    ...
    const damage=Math.max(1,Math.round((raw*.48 + m.level*.16)*style.damageTakenMultiplier*tactic.damageTakenMultiplier*(champion?CHAMPION_DAMAGE_MULTIPLIER:1)*modifiers.incomingDamageMultiplier*companion.incomingDamageMultiplier*(1-effectGems.damage_reduction-v34Effects.damageReduction)*setCombat.incomingDamageMultiplier*(c.preparation?preparationEffects(c.preparation).damage:1)));
    hp-=damage;
    ...
    hp=Math.min(stats.hp,hp+Math.max(1,Math.floor(stats.hp*style.recoveryPct*tactic.recoveryMultiplier*companion.recoveryMultiplier*(1+effectGems.recovery+v34Effects.recovery)*setCombat.recoveryMultiplier)));
  }
