...
exports.claimCompanionTraining = claimCompanionTraining;
...
const combat_companions_2 = require("../content/combat-companions");
...
function awardUse(state, ids, xp, bond, now) {
    ...
    for (const id of ids) {
        ...
        overflow = r.overflow;
        owned[id] = (0, progression_v2_1.awardCompanionBondXpServer)(r.progress, earnedBond);
    }
...
function claimCompanionTraining(input, now) {
    ...
    if (last === undefined)
        return { ...state, account: { ...state.account, companionSanctuary: { ...sanctuary, lastTrainingClaimAtMs: now } } };
    const days = Math.min(7, Math.floor(Math.max(0, now - last) / day));
    if (days < 1)
        return state;
    const available = Object.keys(companionOwned(state)).filter(id => { try {
    ...
    state = awardUse(state, available, combat_companions_2.COMPANION_SANCTUARY_TRAINING_XP_PER_DAY[level] * days, 0, now);
...
function executeCompanionActivity(input, type, a, now) {
    ...
    switch (type) {
        case 'companion_monthly': {
            const r = (0, trials_1.claimMonthlyCompanionChallenge)(state.account.companionTrialProgress, stringArg(a, 'id'), now);
            state = reward(state, r.reward);
            state.account.companionTrialProgress = r.progress;
            break;
        }
        case 'companion_supplies': {
            if (!(state.account.companionSanctuary?.expeditionPensLevel))
                throw new Error('Build Expedition Pens first.');
        ...
        case 'companion_bond_reward': {
            ...
            if (!owned[id] || owned[id].bondLevel < 2)
                throw new Error('Reach Bond 2 first.');
            if (state.account.companionBondRewardClaims?.includes(key))
                throw new Error('Bond reward already claimed.');
        ...
        case 'companion_trial_floor': {
            if (now < (state.account.companionBattleReadyAtMs ?? 0))
                throw new Error('Companions are recovering from battle.');
...
function recordCompanionActivity(state, source, target, units, now) {
    ...
    if (source === 'combat') {
        const oldMines = world_map_1.WORLD_ZONES.find(zone => zone.id === 'OLD_MINES');
        if (oldMines && monsters_1.MONSTERS.find(m => m.id === target)?.zone === oldMines.name)
            counters.OLD_MINES_KILLS = (counters.OLD_MINES_KILLS ?? 0) + units;
    }
    if (source === 'gathering') {
        const gatheringNodes = [...skills_1.GATHERING, ...herbalism_1.HERB_NODES];
        const node = gatheringNodes.find(g => g.id === target);
