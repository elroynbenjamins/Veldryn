...
export type EquipmentCraftSkill='smithing'|'tailoring';
export const TAILORING_EQUIPMENT_CLASSES:ReadonlySet<ClassId>=new Set<ClassId>(['WAYFINDER','HEXWEAVER','KNIFE_DANCER','DAWNKEEPER','STONECALLER']);
export function equipmentCraftSkillForClass(classId:ClassId):EquipmentCraftSkill{return TAILORING_EQUIPMENT_CLASSES.has(classId)?'tailoring':'smithing';}
...
export const TIER_CHARACTER_LEVEL_FLOOR:Record<string,number>={T1:1,T2:8,T3:16,T4:25,T5:38,T6:45,T7:52,T8:62,T9:72};
export const TIER_EQUIPMENT_CRAFT_LEVEL_FLOOR:Record<string,number>={T1:1,T2:5,T3:13,T4:22,T5:35,T6:42,T7:49,T8:59,T9:69};
...
const tailoringPathMaterial:Record<string,Record<string,string>>={
  T1:{Foundation:'BOAR_HIDE',Specialist:'WISP_DUST',Alternate:'MOSS_FIBER'},
  T2:{Foundation:'WOLF_PELT',Specialist:'THORN_SAP',Alternate:'BOAR_HIDE'},
  T3:{Foundation:'TROLL_HIDE',Specialist:'THORN_SAP',Alternate:'WOLF_PELT'},
  T4:{Foundation:'ECHO_TOUCHED_PELT',Specialist:'ECHO_QUARTZ',Alternate:'GLOAM_DUST'},
};
...
function tailoringIngredients(tier:string,path:string,multiplier:number){
  ...
  switch(tier){
    case 'T1':return mergeInputs([{itemId:'MOSS_FIBER',quantity:q(20,multiplier)},{itemId:accent??'BOAR_HIDE',quantity:q(7,multiplier)}]);
    case 'T2':return mergeInputs([{itemId:'WOLF_PELT',quantity:q(20,multiplier)},{itemId:accent??'THORN_SAP',quantity:q(8,multiplier)}]);
    case 'T3':return mergeInputs([{itemId:'TROLL_HIDE',quantity:q(14,multiplier)},{itemId:accent??'WOLF_PELT',quantity:q(10,multiplier)},{itemId:'THORN_SAP',quantity:q(7,multiplier)}]);
    case 'T4':return mergeInputs([{itemId:'TORN_OATHCLOTH',quantity:q(18,multiplier)},{itemId:accent??'ECHO_TOUCHED_PELT',quantity:q(7,multiplier)},{itemId:'OATHGLASS_SHARD',quantity:q(5,multiplier)}]);
    case 'T5':return [{itemId:'TORN_OATHCLOTH',quantity:q(18,multiplier)},{itemId:'ECHO_TOUCHED_PELT',quantity:q(7,multiplier)},{itemId:'AMBERGLASS',quantity:q(4,multiplier)}];
    case 'T6':return [{itemId:'TORN_OATHCLOTH',quantity:q(24,multiplier)},{itemId:'ECHO_TOUCHED_PELT',quantity:q(10,multiplier)},{itemId:'ASTRAL_SCRIPT',quantity:q(2,multiplier)}];
    case 'T7':return [{itemId:'TORN_OATHCLOTH',quantity:q(28,multiplier)},{itemId:'ECHO_TOUCHED_PELT',quantity:q(12,multiplier)},{itemId:'RIMEGLASS',quantity:q(5,multiplier)}];
    case 'T8':return [{itemId:'TORN_OATHCLOTH',quantity:q(32,multiplier)},{itemId:'ECHO_TOUCHED_PELT',quantity:q(16,multiplier)},{itemId:'RIMEGLASS',quantity:q(6,multiplier)},{itemId:'CHOIR_BLOOM',quantity:q(2,multiplier)}];
    case 'T9':return [{itemId:'BANNER_ASH',quantity:q(28,multiplier)},{itemId:'ECHO_TOUCHED_PELT',quantity:q(20,multiplier)},{itemId:'BLACKGLASS_CORE',quantity:q(2,multiplier)},{itemId:'RIMEGLASS',quantity:q(8,multiplier)}];
    default:return [];
  }
}
function ingredients(tier:string,path:string,multiplier:number,skillId:EquipmentCraftSkill){return skillId==='tailoring'?tailoringIngredients(tier,path,multiplier):smithingIngredients(tier,path,multiplier);}
...
export const V33_EQUIPMENT_RECIPES:V33EquipmentRecipeDef[]=(catalog.pieces as Array<Record<string,string|number>>).map(piece=>{
  ...
  const classId=classIdByName[String(piece.Class)],skillId=equipmentCraftSkillForClass(classId);
  const characterLevel=Math.max(catalogReqLevel,TIER_CHARACTER_LEVEL_FLOOR[tier]??1),craftLevel=Math.max(1,characterLevel-3,TIER_EQUIPMENT_CRAFT_LEVEL_FLOOR[tier]??1);
