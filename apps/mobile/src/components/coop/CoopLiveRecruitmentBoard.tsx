...
export function CoopLiveRecruitmentBoard({
 ...
 const [clock,setClock]=useState(nowMs);useEffect(()=>{setClock(nowMs);const started=Date.now(),timer=setInterval(()=>setClock(nowMs+(Date.now()-started)),15000);return()=>clearInterval(timer)},[nowMs]);
 const available=useMemo(()=>dungeons.filter(dungeon=>dungeon.available),[dungeons]),visible=useMemo(()=>activeCoopLiveRecruitment(posts,clock),[posts,clock]),mine=visible.find(post=>post.mine);
 ...
 return <View style={s.stack}>
  ...
  <FantasyPanel>
   <View style={s.head}><View style={s.flex}><Text style={s.kicker}>LIVE GROUP BOARD</Text><Text style={s.title}>30-minute LFG posts</Text></View><StateChip label={visible.length+' LIVE'} tone={visible.length?'selected':'neutral'}/></View>
   ...
   {mine?<View style={s.mineRow}><Text style={s.mineText}>Your post · {names.get(mine.dungeonId)??mine.dungeonId} · {coopLiveRecruitmentTime(mine,clock)}</Text><View style={s.close}><PrimaryAction label="Remove" tone="secondary" disabled={busy} onPress={onCloseMine}/></View></View>:null}
   <View style={s.posts}>{visible.filter(post=>!post.mine).map(post=><View key={post.id} style={s.post}><View style={s.head}><View style={s.flex}><Text style={s.postName}>{post.ownerName}</Text><Text style={s.dungeon}>{names.get(post.dungeonId)??post.dungeonId}</Text></View><StateChip label={post.role.toUpperCase()} tone={post.role==='tank'?'selected':post.role==='support'?'success':'neutral'}/></View><View style={s.metaRow}><Text style={s.meta}>Auto Tier ≤ {post.maxTier}</Text><Text style={s.time}>{coopLiveRecruitmentTime(post,clock)}</Text></View>{post.note?<Text style={s.note}>{post.note}</Text>:null}<PrimaryAction label="Join this search" tone="secondary" disabled={busy} onPress={()=>onJoin(post.dungeonId)}/></View>)}</View>
...
const s=StyleSheet.create({
 stack:{gap:coopSpacing.sm},head:{flexDirection:'row',alignItems:'flex-start',gap:coopSpacing.sm},flex:{flex:1,minWidth:0},kicker:{...coopTypography.meta,color:coopColors.gold,fontWeight:'900',letterSpacing:.8},title:{...coopTypography.section,color:coopColors.text},copy:{...coopTypography.body,color:coopColors.textSecondary},notice:{...coopTypography.meta,color:coopColors.gold},actions:{flexDirection:'row',gap:coopSpacing.sm},composer:{gap:coopSpacing.sm,paddingTop:coopSpacing.sm,borderTopWidth:1,borderTopColor:coopColors.goldDim},label:{...coopTypography.meta,color:coopColors.gold,fontWeight:'900'},chips:{flexDirection:'row',flexWrap:'wrap',gap:6},chip:{minHeight:44,justifyContent:'center',paddingHorizontal:9,borderWidth:1,borderColor:coopColors.goldDim,borderRadius:4,backgroundColor:coopColors.surfaceRaised},chipOn:{borderColor:coopColors.cyan,backgroundColor:'#0A4168'},chipText:{fontSize:11,color:coopColors.textSecondary,fontWeight:'800'},chipTextOn:{color:coopColors.cyan},input:{minHeight:46,color:coopColors.text,backgroundColor:coopColors.surfaceRaised,borderColor:coopColors.goldDim},meta:{...coopTypography.meta,color:coopColors.textMuted},mineRow:{flexDirection:'row',alignItems:'center',gap:coopSpacing.sm,paddingTop:coopSpacing.sm,borderTopWidth:1,borderTopColor:coopColors.goldDim},mineText:{...coopTypography.meta,color:coopColors.cyan,flex:1},close:{width:96},posts:{gap:coopSpacing.sm},post:{gap:6,padding:9,borderWidth:1,borderColor:coopColors.goldDim,borderRadius:5,backgroundColor:coopColors.surfaceRaised},postName:{...coopTypography.section,color:coopColors.text},dungeon:{...coopTypography.meta,color:coopColors.gold,fontWeight:'800'},metaRow:{flexDirection:'row',justifyContent:'space-between',gap:8},time:{...coopTypography.meta,color:coopColors.cyan,fontWeight:'900'},note:{...coopTypography.body,color:coopColors.textSecondary},empty:{...coopTypography.meta,color:coopColors.textMuted,textAlign:'center',paddingVertical:8},
});
