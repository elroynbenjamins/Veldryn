...
import {AsterfallWelcomeModal} from './src/components/AsterfallWelcomeModal';
...
import {ActiveActivity,ClassId,CombatChallengeId,CombatTacticId,GameState,RewardBundle} from './src/core/types';
...
import {cancelEquipmentCraft,claimAllReadyEquipmentCrafts,claimForgeJob,equipmentCraftQueueModel,moveWaitingEquipmentCraft,startEquipmentCraft,startGemCombine,startGemRefinement,timedEquipmentRecipe} from './src/core/equipment-crafting-queue';
...
import {attemptEquipmentUpgrade,replaceGem,socketGem,unsocketGem} from './src/core/equipment-enhancement';
import {claimResonanceCacheV1,dismantleGemV1,gemCombineRecipeIdV1,gemRefineRecipeIdV1} from './src/core/gem-progression-v1';
...
import {MasteryHallScreen} from './src/screens/MasteryHallScreen';
...
import {fetchAppUpdatePolicy,type AppUpdatePolicy} from './src/online/app-update-policy';
...
function tabLabel(language:Language,tab:Tab):string{
  switch(tab){
    case 'Home':return t(language,'nav.home');case 'World':return t(language,'nav.world');case 'Character':return t(language,'nav.character');case 'Inventory':return t(language,'nav.inventory');case 'More':return 'Account';case 'Activity':return 'Characters';case 'Progression':return 'Working Toward';case 'DailySupplies':return 'Daily Supplies';case 'AccountBonuses':return 'Account Bonuses';case 'MasteryHall':return 'Mastery Hall';
...
function VeldrynApp(){
  ...
  const adminQa=auth.session?.user.app_metadata?.admin_qa===true||(Array.isArray(auth.session?.user.app_metadata?.roles)&&auth.session!.user.app_metadata.roles.includes('admin_qa'));
  ...
  const [goalDungeonId,setGoalDungeonId]=useState<string|undefined>();
  ...
  const [preparationNotices,setPreparationNotices]=useState<RecipePreparationTransitionNotice[]>([]);
  ...
  const [updateChecked,setUpdateChecked]=useState(!serverGameplayEnabled);
  const [guestBootstrapAttempted,setGuestBootstrapAttempted]=useState(false);
  ...
  const goBack=useCallback(()=>{
    ...
    if(tab==='Skills'&&selectedSkill){setSelectedSkill(undefined);setGoalActionId(undefined);setGoalRecipeId(undefined);return true;}
    confirmProfileCustomizeExit(()=>{
      setProfileCustomizeDirty(false);
    ...
    return true;
  },[showChatOverlay,showAdminQa,showCoopUiGallery,tab,tabHistory,selectedSkill,confirmProfileCustomizeExit]);
  ...
  useEffect(()=>{if(!serverGameplayEnabled){setUpdateChecked(true);return;}let active=true;void fetchAppUpdatePolicy().then(policy=>{if(active)setUpdatePolicy(policy)}).finally(()=>{if(active)setUpdateChecked(true)});return()=>{active=false};},[]);
  useEffect(()=>{if(!serverGameplayEnabled||!updateChecked||updatePolicy?.required||auth.loading||auth.recovering||auth.session||guestBootstrapAttempted)return;let active=true;setGuestBootstrapAttempted(true);setGuestBootstrapping(true);void signInAsGuest().catch(()=>{}).finally(()=>{if(active)setGuestBootstrapping(false)});return()=>{active=false};},[auth.loading,auth.recovering,auth.session,guestBootstrapAttempted,updateChecked,updatePolicy?.required]);
  ...
  useEffect(()=>{const subscription=AppState.addEventListener('change',nextStatus=>{if(serverGameplayEnabled)return;const wasAway=appStateRef.current==='background'||appStateRef.current==='inactive';appStateRef.current=nextStatus;if(!wasAway||nextStatus!=='active'||settlingRef.current)return;const current=stateRef.current;if(!current?.activity)return;const settled=settleStartupActivity(current,Date.now());if(!settled.reward)return;settlingRef.current=true;queuePreparationNotices(current,settled.state);stateRef.current=settled.state;setState(settled.state);setCollected({reward:settled.reward,activity:settled.activity,welcomeBack:true,progressionMoments:rewardProgressionMoments(current,settled.state)});void repo.save(settled.state).catch(()=>Alert.alert('Save pending','Your returned rewards are safe in memory, but could not be saved yet.')).finally(()=>{settlingRef.current=false})});return()=>subscription.remove()},[]);
...
const next=discoverCharacterSkins(candidate);queuePreparationNotices(current,next);stateRef.current=next;setState(next);queueCustomizationUnlocks(current,next);queueFeatureUnlockMoments(current,next);try{await repo.save(next)}catch{Alert.alert('Local save failed','Progress is still in memory. Keep the app open and try another action to save again.')}}
  ...
  function changeActivity(next?:{kind:'combat'|'gathering';id:string;challengeId?:CombatChallengeId;tacticId?:CombatTacticId;goalId?:HuntGoalId}){
    ...
    try{if(next?.kind==='gathering'&&next.id.startsWith('DEWLEAF_')||next?.id?.includes('_MINT_')||next?.id?.includes('HERB')){const settled=claimActivity(state,Date.now());commit(startHerbalism(settled.state,next.id,Date.now()));presentCollected(settled.reward,state.activity,state,settled.state);setTab('Home');return;}const result=transitionActivity(state,Date.now(),next);commit(result.state);presentCollected(result.reward,state.activity,state,result.state);setTab('Home')}
  ...
  if(serverGameplayEnabled&&!updateChecked)return <StartupScreen scene={startupScene} language={recoveryLanguage}/>;
  if(serverGameplayEnabled&&updatePolicy?.required)return <RequiredUpdateScreen policy={updatePolicy}/>;
  if(serverGameplayEnabled&&(auth.loading||online.loading||guestBootstrapping))return <StartupScreen scene={startupScene} language={recoveryLanguage}/>;
  ...
  const companionsUnlocked=earlyFeatureUnlocked(state,'companions'),workingTowardUnlocked=earlyFeatureUnlocked(state,'workingToward'),dailySuppliesUnlocked=earlyFeatureUnlocked(state,'dailySupplies'),contractsUnlocked=earlyFeatureUnlocked(state,'contracts'),friendsUnlocked=earlyFeatureUnlocked(state,'friends'),socialUnlocked=earlyFeatureUnlocked(state,'social'),guildUnlocked=earlyFeatureUnlocked(state,'guild');
  ...
  const quickNavigationBadges=buildQuickNavigationBadges(navigationNotifications);
  ...
  const activePrimary:PrimaryTab|undefined=tab==='Home'?undefined:tab==='Coop'||tab==='Combat'?'World':primaryTabs.includes(tab as PrimaryTab)?tab as PrimaryTab:'More';
  const secondary=!primaryTabs.includes(tab as PrimaryTab);
  return <GameThemeProvider themeId={state.settings.uiTheme}><SafeAreaView edges={['top','left','right']} style={[s.safe,{backgroundColor:theme.bg}]}><StatusBar style={theme.dark?'light':'dark'}/><GameTopBar state={state} nowMs={now} online={serverGameplayEnabled&&!online.error} attention={quickNavigationBadges} labelForDestination={destination=>tabLabel(state.settings.language,destination)} onNavigate={destination=>{if(destination==='Dungeon'){setTab('Coop');return;}if(destination==='Quests'){openQuestMode('story');return;}setTab(destination)}} onChangeDestinations={destinations=>commit({...state,settings:{...state.settings,quickNavDestinations:destinations}})} onOpenActivity={openActiveActivity}/>{secondary&&tab!=='Coop'&&<View style={[s.backBar,{backgroundColor:theme.panel,borderColor:theme.line}]}><Pressable accessibilityRole="button" accessibilityLabel={t(state.settings.language,'common.back')} onPress={goBack} hitSlop={8} style={({pressed})=>[s.backButton,pressed&&s.backPressed]}><Image source={require('./assets/ui-icons-v2/small/back.png')} resizeMode="contain" style={s.backIcon}/><Text style={[s.backText,{color:theme.accent}]}>{t(state.settings.language,'common.back')}</Text></Pressable><Text numberOfLines={1} style={[s.backTitle,{color:theme.text}]}>{tabLabel(state.settings.language,tab)}</Text><View style={s.backSpacer}/></View>}{serverGameplayEnabled&&online.pending?<View style={{paddingHorizontal:12}}><GameButton title="Retry pending action" disabled={online.busy} onPress={()=>void perform()}/></View>:null}<View pointerEvents={serverGameplayEnabled&&online.busy?'none':'auto'} style={s.body} {...backSwipe.panHandlers}>
    {tab==='Home'&&<HomeScreen state={state} preview={preview} nowMs={now} onOpenPlanner={()=>setTab('Progression')} onOpenContracts={()=>openQuestMode('contracts')} onNavigateGoal={openWorkingTowardDestination} onOpenCombat={()=>setTab('Combat')} onOpenSkill={skillId=>{setSelectedSkill(skillId);setSkillsMode(['mining','woodcutting','fishing'].includes(skillId)?'gathering':'crafting');setTab('Skills')}} onNavigate={destination=>{if(destination==='Skills')setSelectedSkill(undefined);if(destination==='Quests'){openQuestMode('story');return;}setTab(destination)}} onQueueRemove={index=>mutateActionQueue({type:'queue_remove',args:{index}})} onQueueMove={(index,direction)=>mutateActionQueue({type:'queue_move',args:{index,direction}})} onQueueClear={()=>mutateActionQueue({type:'queue_clear'})} onQueueStart={()=>mutateActionQueue({type:'queue_start'},true)} onClaim={()=>{if(serverGameplayEnabled){void perform({type:'claim'});return;}const result=claimActivity(state,Date.now());commit(result.state);presentCollected(result.reward,state.activity,state,result.state)}} onStop={()=>changeActivity()}/>}
    {tab==='Combat'&&<CombatScreen state={state} onCommand={runCompanionCommand} initialMonsterId={goalMonsterId} onChangeRegion={()=>setTab('World')} onStart={(id,challengeId,tacticId,goalId)=>changeActivity({kind:'combat',id,...(challengeId?{challengeId}: {}),...(tacticId?{tacticId}: {}),...(goalId?{goalId}: {})})} onQueue={(id,challengeId,tacticId,goalId)=>queueActivity({kind:'combat',id,...(challengeId?{challengeId}:{}),...(tacticId?{tacticId}:{}),...(goalId?{goalId}:{})})} onQueueRemove={index=>mutateActionQueue({type:'queue_remove',args:{index}})} onQueueMove={(index,direction)=>mutateActionQueue({type:'queue_move',args:{index,direction}})} onQueueClear={()=>mutateActionQueue({type:'queue_clear'})} onQueueStart={()=>mutateActionQueue({type:'queue_start'},true)} onBoss={()=>{const now=Date.now();if(serverGameplayEnabled){void perform({type:'boss'}).then(result=>{if(!result)return;if(result.storyBossBattle){setStoryBossBattle({battle:result.storyBossBattle,message:result.message??(result.won?'Victory':'Defeat')});return;}Alert.alert(result.won?'Victory':'Not ready',result.message)});return;}const settled=claimActivity(state,now);const result=challengeFallenKnight(settled.state,now);void commit(result.state);presentCollected(settled.reward,state.activity,state,settled.state);if(result.battle){setStoryBossBattle({battle:result.battle,message:result.message});return;}Alert.alert(ot(state.settings.language,result.won?'combat.victory':'combat.notReady'),result.message)}}/>}
    {tab==='World'&&<WorldScreen state={state} goalRegionId={goalRegionId} onTravel={regionId=>{if(serverGameplayEnabled){void perform({type:'travel',args:{id:regionId}}).then(result=>{if(result)continueGoalAfterTravel(regionId)});return;}try{const result=travelToRegion(state,regionId,Date.now());void commit(result.state);presentCollected(result.reward,state.activity,state,result.state);continueGoalAfterTravel(regionId)}catch(error){Alert.alert('Cannot travel',error instanceof Error?error.message:'Please try again.')}}} onOpenCombat={()=>setTab('Combat')} onOpenSkills={()=>{setSelectedSkill(undefined);setSkillsMode('gathering');setTab('Skills')}} onCoop={coopOnlineConfigured||(__DEV__&&!serverGameplayEnabled)?()=>setTab('Coop'):undefined} onOpenWeeklyOrder={openWeeklyOrder} onOpenContracts={order=>openQuestMode('contracts',order?.id)} onRegionalRewardsChanged={serverGameplayEnabled?()=>online.refresh():undefined}/>}
    {tab==='Coop'&&<CoopExpeditionScreen state={state} language={state.settings.language} entrySource={coopEntrySource} initialDungeonId={goalDungeonId} onInitialDungeonHandled={()=>setGoalDungeonId(undefined)} initialEventLiveId={pendingEventLiveId} onInitialEventHandled={()=>setPendingEventLiveId(undefined)} onRewardsChanged={serverGameplayEnabled?()=>online.refresh():undefined} onClose={goBack}/>}
    {tab==='Quests'&&<QuestScreen state={state} mode={questMode} focusedWeeklyOrderId={questFocusOrderId} onModeChange={mode=>{setQuestMode(mode);if(mode!=='contracts')setQuestFocusOrderId(undefined)}} onClaim={id=>serverGameplayEnabled?void perform({type:'quest',args:{id}}):void commit(claimQuest(state,id))} onClaimContract={(period,id)=>{if(serverGameplayEnabled){void perform({type:'seasonal',args:{period,id}});return;}try{commit(claimSeasonalContract(state,period,id))}catch(error){Alert.alert('Cannot claim contract',error instanceof Error?error.message:'Please try again.')}}} onNavigate={destination=>{if(destination.tab==='World'&&destination.zoneId){setGoalRegionId(destination.zoneId);setTab('World');return;}setTab(destination.tab)}} onOpenWeeklyBoss={()=>openWorkingTowardDestination({kind:'combat',monsterId:'FALLEN_KNIGHT',zoneName:"King's Road",regionId:'KINGS_ROAD',button:'Open Fallen Knight',detail:'Open the Fallen Knight weekly rematch.'})} onOpenWeeklyOrder={openWeeklyOrder} onPinWeeklyOrder={pinWeeklyOrder} onQueueWeeklyOrder={queueWeeklyOrder} onStopWeeklyOrder={stopAtWeeklyOrderCompletion}/>}
    {tab==='Skills'&&<SkillsScreen key={`${skillsMode}:${selectedSkill??'all'}:${goalActionId??goalRecipeId??'none'}`} state={state} now={now} initialMode={skillsMode} initialSkill={selectedSkill} initialActionId={goalActionId} initialRecipeId={goalRecipeId} forgeResults={forgeResults} onDismissForgeResults={()=>setForgeResults(null)} onBackToHub={()=>{setSelectedSkill(undefined);setGoalActionId(undefined);setGoalRecipeId(undefined)}} onOpenCombat={()=>setTab('Combat')} onSelectSkill={id=>{if(id==='companion'){setTab('Companions');return;}setSelectedSkill(id);setSkillsMode(id==='faith'?'faith':['mining','woodcutting','fishing','herbalism'].includes(id)?'gathering':'crafting')}} onCommand={runCompanionCommand} onCharacter={()=>setTab('Character')} onInventory={()=>setTab('Inventory')} onViewToolRecipes={()=>{setSelectedSkill('smithing');setSkillsMode('crafting')}} onNavigateCraftingSource={openWorkingTowardDestination} onMoveCraftWaiting={(jobId,direction)=>{if(serverGameplayEnabled){void perform({type:'craft_move',args:{id:jobId,direction}});return;}try{void commit(moveWaitingEquipmentCraft(state,jobId,direction,Date.now()))}catch(error){Alert.alert('Cannot reorder backlog',error instanceof Error?error.message:'Please try again.')}}} onCancelCraft={jobId=>{Alert.alert('Cancel forge job?','Waiting jobs refund all reserved materials and Gold. Once crafting has started, all materials and 90% of the Gold are returned.',[{text:'Keep crafting'},{text:'Cancel craft',style:'destructive',onPress:()=>{if(serverGameplayEnabled){void perform({type:'craft_cancel',args:{id:jobId}});return;}try{void commit(cancelEquipmentCraft(state,jobId,Date.now()).state)}catch(error){Alert.alert('Cannot cancel craft',error instanceof Error?error.message:'Please try again.')}}}])}} onCraftPrerequisites={recipeId=>{if(serverGameplayEnabled){void perform({type:'craft_prerequisites',args:{id:recipeId}}).then(result=>{if(result)Alert.alert('Prerequisites crafted',result.message??'Available processing steps were completed.');});return;}try{const result=craftEquipmentPrerequisites(state,recipeId,Date.now());void commit(result.state);queueMasteryNotice(state,result.state);Alert.alert('Prerequisites crafted',result.crafted.map(row=>row.batches+'× '+row.name).join('\n'));}catch(error){Alert.alert('Cannot craft prerequisites',error instanceof Error?error.message:'Please try again.')}}} onClaimCraft={jobId=>{if(serverGameplayEnabled){void perform({type:'craft_claim',args:{id:jobId}}).then(result=>{if(result?.forgeResults?.length)setForgeResults(result.forgeResults);else if(result?.message)Alert.alert('Forge complete',result.message);});return;}try{const claimed=claimForgeJob(state,jobId,Date.now(),Math.random());void commit(claimed.state);queueMasteryNotice(state,claimed.state);if(claimed.kind==='equipment')setForgeResults([claimed.result]);else Alert.alert('Gem combined',claimed.recipe.name+' is ready.')}catch(error){Alert.alert('Cannot claim forge job',error instanceof Error?error.message:'Please try again.')}}} onClaimAllCrafts={()=>{if(serverGameplayEnabled){void perform({type:'craft_claim_all'}).then(result=>{if(result?.forgeResults?.length)setForgeResults(result.forgeResults);if(result?.message&&!result?.forgeResults?.length)Alert.alert('Forge complete',result.message);});return;}try{const claimed=claimAllReadyEquipmentCrafts(state,Date.now(),Math.random());void commit(claimed.state);queueMasteryNotice(state,claimed.state);if(claimed.results.length)setForgeResults(claimed.results);if(claimed.gemClaims>0&&!claimed.results.length)Alert.alert('Gem combines ready',`${claimed.gemClaims} gem combine${claimed.gemClaims===1?'':'s'} claimed.`)}catch(error){Alert.alert('Cannot claim forge jobs',error instanceof Error?error.message:'Please try again.')}}} onGather={id=>changeActivity({kind:'gathering',id})} onQueueGather={id=>queueActivity({kind:'gathering',id})} onQueueRemove={index=>mutateActionQueue({type:'queue_remove',args:{index}})} onQueueMove={(index,direction)=>mutateActionQueue({type:'queue_move',args:{index,direction}})} onQueueClear={()=>mutateActionQueue({type:'queue_clear'})} onQueueStart={()=>mutateActionQueue({type:'queue_start'},true)} onEquipTool={id=>{if(serverGameplayEnabled){void perform({type:'equip_tool',args:{id}});return;}try{const toolKind=itemDef(id).toolSkillId;const settled=state.activity?.kind===toolKind?claimActivity(state,Date.now()):{state,reward:null};commit(equipGatheringTool(settled.state,id));if(settled.reward)presentCollected(settled.reward,state.activity,state,settled.state)}catch(error){Alert.alert('Cannot equip tool',error instanceof Error?error.message:'Please try again.')}}} onCraft={id=>{const timed=timedEquipmentRecipe(id);if(serverGameplayEnabled){void perform({type:'craft',args:{id}}).then(result=>{if(result)Alert.alert(timed?(result.message??'Forge updated'):'Craft complete',timed?'Materials and Gold were reserved. Backlog jobs start automatically when an active forge slot becomes free.':'Your crafted items have been saved online.');});return;}try{if(timed){const started=startEquipmentCraft(state,id,Date.now());commit(started.state);Alert.alert(started.waiting?'Added to forge backlog':'Crafting started',started.waiting?`The ${Math.ceil(started.seconds/60)} minute craft will start automatically when a forge slot opens.`:`This equipment will be ready in about ${Math.ceil(started.seconds/60)} minute${Math.ceil(started.seconds/60)===1?'':'s'}.`);return;}const next=craftRecipe(state,id);const completed=!noviceSetProgress(state).unlocked&&noviceSetProgress(next).unlocked;commit(next);queueMasteryNotice(state,next);Alert.alert(ot(state.settings.language,completed?'craft.setComplete':'craft.complete'),completed?`${noviceSetProgress(next).set.name} is fully crafted and ready to equip. Equipment changes stats; skins are chosen separately on Character.`:'Your crafted items were added to Inventory, or Bank if Inventory was full. Equip them from Inventory or use Equip owned novice set on Character.',[{text:'Continue'},{text:'View character',onPress:()=>setTab('Character')}])}catch(error){Alert.alert(ot(state.settings.language,'craft.cannot'),error instanceof Error?error.message:'Please try again.')}}}/>}
    ...
    {tab==='MasteryHall'&&<MasteryHallScreen state={state} onOpenSkill={skillId=>{setSelectedSkill(skillId);setSkillsMode(['mining','woodcutting','fishing','herbalism'].includes(skillId)?'gathering':'crafting');setTab('Skills')}} onNavigate={openWorkingTowardDestination} onAchievements={()=>setTab('Achievements')} onProfile={()=>{setProfileCustomizeSection('Identity');setTab('ProfileCustomize')}}/>}
    ...
    {tab==='Character'&&<CharacterScreen state={state} onUpgrade={async itemId=>{if(serverGameplayEnabled){await perform({type:'upgrade',args:{id:itemId}});return;}try{const attempt=attemptEquipmentUpgrade(state,itemId);await commit(attempt.state);}catch(error){Alert.alert('Cannot upgrade',error instanceof Error?error.message:'Please try again.')}}} onSocket={async (itemId,gemId)=>{if(serverGameplayEnabled){await perform({type:'socket',args:{id:itemId,gemId}});return;}try{await commit(socketGem(state,itemId,gemId))}catch(error){Alert.alert('Cannot socket gem',error instanceof Error?error.message:'Please try again.')}}} onReplaceGem={async (itemId,gemId)=>{if(serverGameplayEnabled){await perform({type:'replace_socket',args:{id:itemId,gemId}});return;}try{await commit(replaceGem(state,itemId,gemId))}catch(error){Alert.alert('Cannot replace gem',error instanceof Error?error.message:'Please try again.')}}} onUnsocket={async (itemId,index)=>{if(serverGameplayEnabled){await perform({type:'unsocket',args:{id:itemId,index}});return;}try{await commit(unsocketGem(state,itemId,index))}catch(error){Alert.alert('Cannot extract gem',error instanceof Error?error.message:'Please try again.')}}} onGemRefine={async (familyId,grade)=>{if(serverGameplayEnabled){await perform({type:'gem_refine',args:{familyId,grade}});return;}try{await commit(startGemRefinement(state,gemRefineRecipeIdV1(familyId,grade),Date.now()).state)}catch(error){Alert.alert('Cannot refine gem',error instanceof Error?error.message:'Please try again.')}}} onGemCombine={async (familyId,grade)=>{if(serverGameplayEnabled){await perform({type:'gem_combine',args:{familyId,grade}});return;}try{await commit(startGemCombine(state,gemCombineRecipeIdV1(familyId,grade),Date.now()).state)}catch(error){Alert.alert('Cannot combine gems',error instanceof Error?error.message:'Please try again.')}}} onGemDismantle={async (gemId,quantity)=>{if(serverGameplayEnabled){await perform({type:'gem_dismantle',args:{gemId,quantity}});return;}try{await commit(dismantleGemV1(state,gemId,quantity))}catch(error){Alert.alert('Cannot dismantle gem',error instanceof Error?error.message:'Please try again.')}}} onGemCacheClaim={async familyId=>{if(serverGameplayEnabled){await perform({type:'resonance_cache_claim',args:{familyId}});return;}try{await commit(claimResonanceCacheV1(state,familyId,Date.now()))}catch(error){Alert.alert('Cannot claim Resonance Cache',error instanceof Error?error.message:'Please try again.')}}} onInventory={()=>setTab('Inventory')} onSave={()=>serverGameplayEnabled?online.refresh():repo.save(state)} onUnequip={async slot=>{if(serverGameplayEnabled){if(!await perform({type:'unequip',args:{slot}}))throw new Error('Equipment change was not confirmed.');}else await commit(unequipItem(state,slot));}} onCrafting={()=>{setSkillsMode('novice');setTab('Skills')}} onSelectSkin={skinId=>{if(serverGameplayEnabled){void perform({type:'skin',args:{id:skinId}});return;}try{commit(selectCharacterSkin(state,skinId))}catch(error){Alert.alert('Cannot use skin',error instanceof Error?error.message:'Please try again.')}}} onEquipSet={async()=>{if(serverGameplayEnabled){if(!await perform({type:'equip_set'}))throw new Error('Set equip was not confirmed.');return;}const settled=claimActivity(state,Date.now());const next=equipNoviceSet(settled.state);await commit(next);presentCollected(settled.reward,state.activity,state,settled.state)}}><ProfileEditor state={state} onChange={commit} onCommand={runCompanionCommand}/></CharacterScreen>}
    ...
    {tab==='Settings'&&<SettingsScreen online={serverGameplayEnabled} state={state} onChange={commit} onExport={exportSave} onImport={importSave} onOpenCoopUiGallery={__DEV__?()=>setShowCoopUiGallery(true):undefined} onLanguage={language=>commit({...state,settings:{...state.settings,language}})} onReset={()=>serverGameplayEnabled?Alert.alert('Online save','Your online character is saved on the server.'):Alert.alert('Reset local save?','This deletes prototype progress only.',[{text:'Cancel'},{text:'Reset',style:'destructive',onPress:async()=>{await repo.reset();setState(newGame(Date.now()));setCurrentTab('Home');setTabHistory([])}}])}/>}
    ...
    {tab==='Profile'&&<ProfileScreen state={state} onNavigate={destination=>{if(destination==='Customize'){setProfileCustomizeSection('Appearance');setTab('ProfileCustomize');return;}setTab(destination)}}/>}
    {tab==='ProfileCustomize'&&<ProfileCustomizeScreen state={state} initialSection={profileCustomizeSection} onChange={commit} onNavigateSource={destination=>setTab(destination)} onDirtyChange={setProfileCustomizeDirty}/>} 
  ...
  {creationWelcome&&<AsterfallWelcomeModal visible name={creationWelcome.name} classId={creationWelcome.classId} onContinue={()=>{setCreationWelcome(null);setCurrentTab('World')}}/>}
