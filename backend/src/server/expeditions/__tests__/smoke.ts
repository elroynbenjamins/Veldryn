...
import { normalizeChatForMatching, moderateChatMessage } from '../../chat/moderation';
import { prepareExpeditionRun } from '../create-run';
import { generateBoonOffer } from '../boon-generation';
import { resolveSimulatedEncounter } from '../encounter-resolver';

assert.equal(normalizeChatForMatching('f . u - c_k'), 'fuck');
const mod = moderateChatMessage('f . u - c_k', [{id:'1',term:'fuck',severity:2,action:'mask'}]);
assert.equal(mod.action, 'mask');

const prepared = prepareExpeditionRun({
...
assert.ok(prepared.route.length >= 8);
assert.equal(prepared.route.at(-1)?.type, 'boss');

const offer1 = generateBoonOffer({secret:'test-secret',runId:prepared.run.id,characterId:prepared.members[0].characterId,nodeIndex:2,contentVersion:'4.4',ownedBoonIds:[]});
const offer2 = generateBoonOffer({secret:'test-secret',runId:prepared.run.id,characterId:prepared.members[0].characterId,nodeIndex:2,contentVersion:'4.4',ownedBoonIds:[]});
assert.deepEqual(offer1, offer2);
assert.equal(offer1.length, 3);

const encounter = resolveSimulatedEncounter({secret:'test-secret',runId:prepared.run.id,nodeIndex:0,tier:1,nodeDifficultyMultiplier:1,regionalMechanicMultiplier:1,routeRiskMultiplier:1,executionScore:1,mode:'server_simulation',members:[
...
assert.ok(encounter.successProbability >= .03 && encounter.successProbability <= .98);
