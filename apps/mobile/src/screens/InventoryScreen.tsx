...
export function InventoryScreen({state,onEquip,onFood,onEat,onSell,onSalvage,onDeposit,onDepositMaterials,onUpgradeStorage,onWithdraw,onOverflow}:{state:GameState;onEquip:(id:string)=>void;onFood:(id:string)=>void;onEat:(id:string)=>void;onSell:(id:string)=>void;onSalvage:(id:string)=>void;onDeposit:(id:string,quantity:number)=>void;onDepositMaterials:()=>void;onUpgradeStorage:(location:StorageLocation)=>void;onWithdraw:(id:string,quantity:number)=>void;onOverflow:()=>void}){
  ...
  const renderStack=(stack:ItemStack)=>{
    ...
    return <ItemCard key={`${location}:${item.id}`} item={item} quantity={stack.quantity} equipped={equippedId?itemDef(equippedId):undefined} selectedFood={selectedFood} healAmount={recoveryAmount(state,item.id)} transferQuantity={amount} transferIssue={transferError(state,item.id,amount,location)} numberMode={state.settings.numberMode}
...
const s=StyleSheet.create({root:{padding:spacing.lg,gap:spacing.md},h:{...typography.hero,color:C.text},title:{...typography.title,color:C.text},sub:{...typography.body,color:C.muted},warning:{...typography.body,color:C.warning},label:{...typography.caption,color:C.accent,fontWeight:'900'},upgradeLabel:{...typography.caption,color:C.muted,fontWeight:'900',marginBottom:spacing.xs},rarityLegend:{gap:spacing.sm,paddingRight:spacing.md},rarityKey:{minHeight:32,justifyContent:'center',borderWidth:1,borderRadius:99,paddingHorizontal:spacing.sm},rarityKeyText:{...typography.caption,fontWeight:'900'},row:{flexDirection:'row',gap:spacing.sm},flex:{flex:1},input:{minHeight:48,paddingHorizontal:spacing.md,borderRadius:10,borderWidth:1,borderColor:C.line,color:C.text,backgroundColor:C.panel,fontSize:16}});
