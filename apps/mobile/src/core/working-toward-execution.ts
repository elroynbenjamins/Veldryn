...
export type WorkingTowardExecutionState='ready'|'queued'|'travel'|'blocked'|'full'|'unsupported';
...
export function workingTowardExecutionPlan(state:GameState,goal:ProgressionGoal):WorkingTowardExecutionPlan{
 ...
 return {goal,view,destination,executionState,executionLabel,queueActivity,queueBlocker,alreadyQueued,queueFull,stopRule,stopRuleActive};
}
...
export function workingTowardExecutionOverview(state:GameState):WorkingTowardExecutionOverview{
 ...
 const focus=[...plans].sort((a,b)=>{
  const priority=(row:WorkingTowardExecutionPlan)=>row.view.status==='complete'?0:row.executionState==='ready'?1:row.executionState==='queued'?2:row.view.status==='active'?3:4;
