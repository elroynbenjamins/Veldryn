...
export function OnlineWorldChat({playerName,language,unlockedEmoteIds=[],embedded=false}:{playerName:string;language:Language;unlockedEmoteIds?:readonly string[];embedded?:boolean}){
 ...
 const send=async()=>{const body=text.trim();if(!body||busy)return;if(chatEmoteCount(body)>8){Alert.alert('World chat','Use at most 8 emotes in one message.');return}const locked=chatUnavailableEmoteIds(body,unlockedEmoteIds);if(locked.length){Alert.alert('World chat','One or more emotes in this message are still locked.');return}setBusy(true);try{await postWorldMessage(WORLD_CHANNELS[channel].id,body,playerName);setText('');await load();}catch(reason){Alert.alert('World chat',reason instanceof Error?reason.message:'Unable to send message.');}finally{setBusy(false)}};
 ...
 const content=<View style={s.root}>
  ...
  <View style={s.compose}><TextInput accessibilityLabel="World chat message" value={text} onChangeText={setText} onSubmitEditing={()=>void send()} placeholder={ot(language,'chat.placeholder')} maxLength={300} style={s.input}/><ChatEmotePicker unlockedIds={unlockedEmoteIds} onPick={token=>setText(value=>(value+token).slice(0,300))}/><Pressable accessibilityRole="button" accessibilityLabel="Send message" accessibilityState={{disabled:busy||!text.trim()}} disabled={busy||!text.trim()} onPress={()=>void send()} style={({pressed})=>[s.send,(pressed||busy||!text.trim())&&s.sendDim]}><UiIcon name="next" size={24}/></Pressable></View>
