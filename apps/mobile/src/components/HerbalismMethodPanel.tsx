...
import {environmentForZone,environmentSummary} from '../core/world-weather';
...
export function HerbalismMethodPanel({state,onCommand}:{state:GameState;onCommand:(command:GameCommand)=>Promise<void>}){
 ...
 const selected=herbalismMethod(state.character?.herbalismMethodId,level),active=state.activity?.kind==='herbalism';
 const env=environmentForZone(state.currentRegionId),essence=HERBALISM_ESSENCE_BY_ZONE[state.currentRegionId];
 return <Panel>
  ...
  <View style={s.methods}>{HERBALISM_METHODS.map(method=>{
   ...
   return <Pressable key={method.id} accessibilityRole="button" accessibilityState={{selected:chosen,disabled:!unlocked||active}} disabled={!unlocked||active} onPress={()=>void onCommand({type:'herbalism_method',args:{method:method.id}})} style={({pressed})=>[s.method,chosen&&s.methodOn,!unlocked&&s.methodLocked,pressed&&s.pressed]}>
  ...
  {active?<Text style={s.notice}>Stop the current Herbalism activity before changing method. This prevents rerolling already-earned offline progress.</Text>:null}
  <View style={s.weather}><View style={s.flex}><Text style={s.eyebrow}>CURRENT CONDITIONS</Text><Text style={s.weatherName}>{env.weatherSymbol} {env.weatherName} · {env.seasonSymbol} {env.seasonName}</Text><Text style={s.copy}>{environmentSummary('herbalism',env)}</Text></View>{essence?<View style={s.essence}><Text style={s.essenceLabel}>RARE FIND</Text><Text style={s.essenceName}>{itemDef(essence.itemId).name}</Text><Text style={s.meta}>{(essence.baseChance*100).toFixed(essence.baseChance<.015?1:2)}% base/action</Text></View>:null}</View>
 </Panel>;
}
function styles(C:ThemeColors){return StyleSheet.create({
 ...
 weather:{flexDirection:'row',alignItems:'center',gap:8,paddingTop:7,borderTopWidth:1,borderTopColor:C.line},weatherName:{...typography.bodyStrong,color:C.text},essence:{maxWidth:170,alignItems:'flex-end'},essenceLabel:{fontSize:8,color:C.special,fontWeight:'900',letterSpacing:.7},essenceName:{...typography.caption,color:C.text,fontWeight:'900',textAlign:'right'}
});}
