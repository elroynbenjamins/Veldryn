...
export function EquipmentCraftQueuePanel({state,onClaim,onClaimAll,onCancel}:{state:GameState;onClaim:(jobId:string)=>void;onClaimAll:()=>void;onCancel:(jobId:string)=>void}){
  ...
  return <View style={s.card}>
    ...
    {model.jobs.length?<View style={s.jobs}>{model.jobs.map(job=><View key={job.id} style={[s.job,job.ready&&s.jobReady]}><View style={s.flex}><Text style={s.jobName}>{job.name}</Text><Text style={s.meta}>{job.ready?'Finished · ready to claim':duration(job.remainingSeconds)+' remaining'}</Text>{!job.ready&&<View style={s.track}><View style={[s.fill,{width:(Math.max(2,Math.min(100,100-(job.remainingSeconds/Math.max(1,(job.completesAtMs-job.startedAtMs)/1000))*100))+'%') as `${number}%`}]}/></View>}</View>{job.ready?<GameButton compact title="Claim" onPress={()=>onClaim(job.id)}/>:<View style={s.jobActions}><Text style={s.running}>CRAFTING</Text><GameButton compact title="Cancel" tone="secondary" onPress={()=>onCancel(job.id)}/></View>}</View>)}</View>:<Text style={s.empty}>No equipment is being crafted. Start a gear recipe below to use a slot.</Text>}
    {model.ready>1?<GameButton title={'Claim all finished · '+model.ready} onPress={onClaimAll}/>:null}
    {model.active>0?<Text style={s.cancelNote}>Cancel returns all reserved materials and 90% of the Gold. The remaining 10% is the forge setup fee.</Text>:null}
...
function makeStyles(C:ThemeColors){const E=equipmentTheme(C);return StyleSheet.create({
  ...
  jobs:{gap:6},job:{minHeight:62,flexDirection:'row',alignItems:'center',gap:spacing.sm,padding:spacing.sm,borderWidth:1,borderColor:C.line,borderRadius:radii.sm,backgroundColor:C.panel2},jobReady:{borderColor:C.good,backgroundColor:C.goodSurface},jobName:{...typography.bodyStrong,color:C.text},jobActions:{alignItems:'flex-end',gap:4},running:{...typography.caption,color:E.goldSoft,fontWeight:'900'},cancelNote:{fontSize:9,lineHeight:13,color:C.muted},
