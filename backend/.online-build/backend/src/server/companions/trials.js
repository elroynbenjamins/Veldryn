...
exports.claimMonthlyCompanionChallenge = claimMonthlyCompanionChallenge;
...
function claimMonthlyCompanionChallenge(progress, challengeId, serverNowMs) {
    const current = (0, trial_season_1.rolloverCompanionTrialSeason)(progress, serverNowMs).progress;
    const season = (0, content_1.companionTrialSeasonDefinition)(current.season.seasonKey);
    const challenge = content_1.COMPANION_WEEKLY_CHALLENGES.find(c => c.id === challengeId);
    if (!challenge || !season.specialChallenges.includes(challengeId))
        throw new Error('monthly_companion_challenge_unavailable');
    if (!current.season.monthlyChallengeCompletion[challengeId])
        throw new Error('monthly_companion_challenge_incomplete');
    const claimed = current.season.monthlyChallengeClaims ?? [];
    if (claimed.includes(challengeId))
        throw new Error('monthly_companion_challenge_already_claimed');
