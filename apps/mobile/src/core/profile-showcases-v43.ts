...
export function newProfileExtensionState(accountId:string,nowMs:number):ProfileExtensionState{if(!accountId)throw new Error('account_required');return {schemaVersion:43,accountId,revision:0,visibility:'public',worldFeedOptOut:false,bio:'',achievementShowcaseIds:[],collectionShowcase:[],recordShowcaseIds:[],masteryShowcaseActionIds:[],updatedAtMs:nowMs}}
...
function assertOwned(state:ProfileExtensionState,ownership:ProfileOwnershipSnapshot){
 ...
 if(state.masteryShowcaseActionIds.some(id=>!ownership.masteredProfessionActionIds.includes(id)))throw new Error('mastery_not_mastered');
...
export function applyProfileExtensionEdit(state:ProfileExtensionState,edit:ProfileExtensionEdit,ownership:ProfileOwnershipSnapshot,nowMs:number){
 ...
 if(edit.masteryShowcaseActionIds!==undefined){const rows=unique(edit.masteryShowcaseActionIds);if(rows.length>MAX_SHOWCASE)throw new Error('too_many_masteries');state.masteryShowcaseActionIds=rows}
