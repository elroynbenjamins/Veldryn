...
function claimActivity(state, nowMs) {
    ...
    if (state.activity.kind !== 'combat') {
        ...
        return { state: refreshQuests((0, live_events_1.applyEventDiscoveries)((0, live_events_1.applyEventDrops)(next, reward.eventDrops ?? []), reward.eventDiscoveries ?? [])), reward };
    }
    ...
    return { state: refreshQuests((0, live_events_1.applyEventDiscoveries)((0, live_events_1.applyEventDrops)(next, reward.eventDrops ?? []), reward.eventDiscoveries ?? []), state.activity.targetId, reward.kills), reward };
}
...
function craftRecipe(state, recipeId, nowMs = Date.now()) {
    ...
    return refreshQuests((0, live_events_1.grantEventActivity)(next, 'crafting', nowMs));
}
...
function challengeFallenKnight(state, nowMs = Date.now()) {
    ...
    next = refreshQuests((0, live_events_1.grantEventActivity)(next, 'boss', nowMs));
    return { state: next, won: true, message: `Fallen Knight defeated at readiness ${ready.total}/100. The road toward Sunscar is open.` };
}
