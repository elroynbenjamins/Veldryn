...
export function GuildScreen({state,onChange,onlineDirectory,onlineManagement,onlinePve,online=false}:{online?:boolean;state:GameState;onChange:(next:GameState)=>void;onlineDirectory?:ReactNode;onlineManagement?:ReactNode;onlinePve?:ReactNode}){
  ...
  if(online)return <ScrollView contentContainerStyle={s.root}><Text style={s.h}>Guild</Text>{onlineDirectory}{onlineManagement}{onlinePve}</ScrollView>;
  ...
  return <ScrollView contentContainerStyle={s.root}>
    ...
    <View style={s.tabs}>{(['Overview','PvE','Roster'] as const).map(value=><View key={value} style={s.flex}><GameButton title={value} tone={section===value?'primary':'secondary'} onPress={()=>setSection(value)}/></View>)}</View>
