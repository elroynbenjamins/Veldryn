...
function at(index:number,level=1):FirstSessionTutorialHost{return {character:{level},activity:null,unlockedMonsterIds:['MOSS_RAT'],quests:questIds.map((questId,i)=>({questId,status:i<index?'claimed':i===index?'active':'locked',progress:0}))};}
...
check('level 7 gets the wolf objective after scouting',()=>{const s=scouted(at(2,7));assert.equal(step(s)?.id,'ironwood_hunt');assert.ok(step(s)?.hint.includes('Ironwood Forest'))});
