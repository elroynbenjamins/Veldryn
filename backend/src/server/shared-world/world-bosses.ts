export type WorldBossState='scheduled'|'active'|'defeated'|'expired'|'cancelled'|'finalized';
export type WorldBossRoleProfile='damage'|'tank'|'support'|'hybrid';

export interface WorldBossPhaseDefinition {
  id:string;
...
export interface WorldBossDefinition {
  ...
  dailyScoredAttempts:number;
  attemptDurationSeconds:number;
  postDefeatEchoWindowHours:number;
  minimumParticipationAttemptsForVictoryReward:number;
  roleWeights:Record<WorldBossRoleProfile,WorldBossRoleWeights>;
  phases:readonly WorldBossPhaseDefinition[];
  personalImpactMilestones:readonly number[];
  participationRewardBundleId:string;
  victoryRewardBundleId:string;
  prestigeRewardBundleId?:string;
  eventTags:readonly string[];
}

export interface WorldBossRoleWeights {
  directDamage:number;
  damagePrevented:number;
  effectiveHealing:number;
  utilityPerSecond:number;
  survivalBonusPerSecond:number;
}

export interface WorldBossScaleInput {
  eligibleCombatAccounts7d:number;
  expectedParticipationFraction?:number;
  expectedAttemptsPerParticipant?:number;
  medianImpactPerAttempt:number;
  targetClearFraction?:number;
  minHp?:number;
  maxHp?:number;
}

export interface WorldBossScaleSnapshot {
  eligibleCombatAccounts7d:number;
  expectedParticipationFraction:number;
  expectedAttemptsPerParticipant:number;
  medianImpactPerAttempt:number;
...
export interface WorldBossCombatResult {
  ...
  accountId:string;
  characterId:string;
  roleProfile:WorldBossRoleProfile;
  durationSeconds:number;
  directDamage:number;
  damagePrevented:number;
  effectiveHealing:number;
  utilityUptimeSeconds:number;
  survived:boolean;
  defeatedPlayer:boolean;
}

export interface WorldBossImpactResult {
  raidImpact:number;
  globalDamage:number;
  performanceBreakdown:{direct:number;mitigation:number;healing:number;utility:number;survival:number};
}

export interface WorldBossDamageApplication {
  previousHp:number;
  remainingHp:number;
...
export function validateWorldBossDefinition(definition:WorldBossDefinition):string[]{
  const errors:string[]=[];
  if(!definition.id.trim()) errors.push('id_required');
  if(!definition.regionId.trim()) errors.push('region_required');
  if(!Number.isInteger(definition.version)||definition.version<1) errors.push('version_invalid');
  if(definition.durationHours<12||definition.durationHours>96) errors.push('duration_out_of_range');
  if(definition.dailyScoredAttempts<1||definition.dailyScoredAttempts>12) errors.push('daily_attempts_out_of_range');
  if(definition.attemptDurationSeconds<30||definition.attemptDurationSeconds>300) errors.push('attempt_duration_out_of_range');
  if(definition.postDefeatEchoWindowHours<0||definition.postDefeatEchoWindowHours>24) errors.push('echo_window_out_of_range');
  if(definition.phases.length<2) errors.push('phases_required');
  let previous=1.01;
  for(const phase of definition.phases){
    if(phase.startsAtHpFraction>=previous||phase.startsAtHpFraction<0||phase.startsAtHpFraction>1) errors.push('phase_thresholds_invalid');
    previous=phase.startsAtHpFraction;
  }
  if(Math.abs((definition.phases[0]?.startsAtHpFraction??0)-1)>0.000001) errors.push('first_phase_must_start_at_full_hp');
  for(const [role,weights] of Object.entries(definition.roleWeights)){
    ...
    if(total<=0||!Number.isFinite(total)) errors.push(`role_weights_invalid:${role}`);
  }
  let milestone=0; for(const p of definition.personalImpactMilestones){if(p<=milestone) errors.push('milestones_not_increasing'); milestone=p;}
  return [...new Set(errors)];
}

export function deriveWorldBossScale(input:WorldBossScaleInput):WorldBossScaleSnapshot {
...
export function phaseForHp(definition:WorldBossDefinition,remainingHp:number,maxHp:number):WorldBossPhaseDefinition {
  ...
  for(let i=definition.phases.length-1;i>=0;i--){
    ...
    if(fraction<=phase.startsAtHpFraction) return phase;
  }
 ...
 */
export function computeWorldBossImpact(definition:WorldBossDefinition,result:WorldBossCombatResult):WorldBossImpactResult {
  const w=definition.roleWeights[result.roleProfile];
  if(!w) throw new Error('world_boss_role_profile_not_configured');
...
export function applyWorldBossDamage(remainingHp:number,requestedDamage:number):WorldBossDamageApplication {
...
export function canStartScoredBossAttempt(input:{state:WorldBossState;attemptsToday:number;validAttemptsTotal?:number;echoAttemptsTotal?:number;definition:WorldBossDefinition;nowMs:number;endsAtMs:number;defeatedAtMs?:number}):{allowed:boolean;echoOnly:boolean;reason?:string}{
  if(input.state==='active'&&input.nowMs<input.endsAtMs){
    if(input.attemptsToday>=input.definition.dailyScoredAttempts) return {allowed:false,echoOnly:false,reason:'daily_attempt_limit'};
  ...
  if(input.state==='defeated'&&input.defeatedAtMs!=null){
    ...
    if(input.nowMs<echoEnds && input.nowMs<input.endsAtMs){
      if((input.validAttemptsTotal??0)>0) return {allowed:false,echoOnly:false,reason:'already_participated_before_defeat'};
      if((input.echoAttemptsTotal??0)>0) return {allowed:false,echoOnly:false,reason:'echo_attempt_already_used'};
...
export function eligibleForWorldBossVictoryReward(validAttempts:number,definition:WorldBossDefinition):boolean{
  return validAttempts>=definition.minimumParticipationAttemptsForVictoryReward;
}

const roleWeights:Record<WorldBossRoleProfile,WorldBossRoleWeights>={
...
const phases=(theme:string):readonly WorldBossPhaseDefinition[]=>[
...
export const WORLD_BOSS_TEMPLATE_POOL:readonly WorldBossDefinition[]=[
...
for(const definition of WORLD_BOSS_TEMPLATE_POOL){const errors=validateWorldBossDefinition(definition); if(errors.length) throw new Error(`invalid_world_boss:${definition.id}:${errors.join(',')}`);}
