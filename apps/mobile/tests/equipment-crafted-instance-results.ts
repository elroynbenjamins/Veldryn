...
const epicStats=enhancedGearStats(state,epicClaim.instance.id),epicQuote=upgradeQuote(state,epicClaim.instance.id);
...
const commonStats=enhancedGearStats(state,commonClaim.instance.id),commonQuote=upgradeQuote(state,commonClaim.instance.id);
ok(epicStats.attack>commonStats.attack||epicStats.defense>commonStats.defense||epicStats.hp>commonStats.hp,'Epic exact copy must have stronger stats than its Common duplicate');
ok(epicQuote.gold>commonQuote.gold,'Higher-rarity exact copy must have the higher upgrade Gold cost');
ok(craftedGearInstances(state).find(row=>row.id===epicClaim.instance.id)?.rarity==='epic','Common duplicate must not mutate the Epic copy');
...
ok(normalized.account.gearInstances?.filter(row=>row.acquireSource==='craft').length===2,'Save normalization must preserve crafted gear instances');
ok(normalized.account.gearInstances?.some(row=>row.id===epicClaim.instance.id&&row.rarity==='epic'),'Save normalization must preserve exact crafted instance rarity');
