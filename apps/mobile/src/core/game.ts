...
import {grantProfessionMasteryToState} from './profession-mastery-v40';
...
export function previewActivityReward(state:GameState,nowMs:number):RewardBundle{
  ...
  return {...reward,masteryMaterialRemainders:materialRemainders,eventDrops:activityEventDrops(state,reward,nowMs),eventDiscoveries:activityEventDiscoveries(state,'combat',reward.kills,nowMs)};
}
...
export function claimActivity(state:GameState,nowMs:number){
  ...
  if(state.activity.kind!=='combat'){
    ...
    let progressed=grantProfessionMasteryToState(next,state.activity.targetId,reward.kills,nowMs);
...
export function craftRecipe(state:GameState,recipeId:string,nowMs=Date.now()):GameState{
  ...
  let progressed=grantProfessionMasteryToState(next,r.id,1,nowMs);
