...
import {JournalFilter,QuestDestination,journalActionSummary,journalEntries,questDestination} from '../core/quest-journal';
...
const JOURNAL_FILTER_OPTIONS:{id:JournalFilter;label:string}[]=[
  {id:'current',label:'Current'},
  {id:'all',label:'All chapters'},
  {id:'claimed',label:'Completed'},
...
export function QuestScreen({state,onClaim,onClaimContract,onNavigate,onOpenWeeklyOrder,onPinWeeklyOrder,onQueueWeeklyOrder,onStopWeeklyOrder}:{state:GameState;onClaim:(id:string)=>void;onClaimContract:(period:SeasonalPeriod,id:string)=>void;onNavigate:(destination:QuestDestination)=>void;onOpenWeeklyOrder:(order:WeeklyOrder)=>void;onPinWeeklyOrder:(order:WeeklyOrder)=>void;onQueueWeeklyOrder:(order:WeeklyOrder)=>void;onStopWeeklyOrder:(order:WeeklyOrder)=>void}){
  const C=useGameTheme(),equipmentColors=equipmentTheme(C),s=useMemo(()=>makeStyles(C),[C]),{width,fontScale}=useWindowDimensions(),stackAttention=width<360||fontScale>=1.25;
  const [filter,setFilter]=useState<JournalFilter>('current'),[query,setQuery]=useState(''),[filterOpen,setFilterOpen]=useState(false);
  ...
  const entries=journalEntries(state,filter,query),storySummary=journalActionSummary(state);
  ...
  const challengeRows=[...daily,...weekly,...monthly],claimedChallengeIds=new Set(state.account.seasonalContractClaimIds??[]),challengeReady=challengeRows.filter(row=>row.progress>=row.required&&!claimedChallengeIds.has(row.id)).length;
  const boardComplete=contractBoard.orders.filter(order=>order.progress>=order.target).length,claimed=storySummary.claimed,ready=storySummary.ready;
  const filterLabel=JOURNAL_FILTER_OPTIONS.find(option=>option.id===filter)?.label??'Current';
  ...
  return <><ScrollView contentContainerStyle={s.root} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
    ...
    <View style={s.attentionRow}><JournalStat stack={stackAttention} label="REWARDS" value={String(storySummary.ready)} tone={storySummary.ready?'good':'muted'}/><JournalStat stack={stackAttention} label="ACTIVE" value={String(storySummary.active)} tone="info"/><JournalStat stack={stackAttention} label="WEEKLY" value={boardComplete+'/'+contractBoard.orders.length} tone={boardComplete===contractBoard.orders.length&&contractBoard.orders.length?'good':'accent'}/><JournalStat stack={stackAttention} label="CACHES" value={String(challengeReady)} tone={challengeReady?'special':'muted'}/></View>
    <View style={s.filterSummary}><View style={s.flex}><Text style={s.filterEyebrow}>STORY VIEW</Text><Text style={s.filterTitle}>{filterLabel} · {entries.length} chapter{entries.length===1?'':'s'}</Text></View><Pressable accessibilityRole="button" accessibilityLabel="Filter story chapters" onPress={()=>setFilterOpen(true)} style={({pressed})=>[s.filterToggle,pressed&&s.pressed]}><Text style={s.filterToggleText}>FILTER</Text><Text style={s.filterToggleMark}>⌄</Text></Pressable></View>
    ...
    {showContracts&&<Panel><Text style={s.sub}>Class-aligned personal challenges award progressively stronger rarity caches.</Text><ContractRows state={state} label={`DAILY · ${daily[0]?.className??''}`} quests={daily} onClaim={claimContract}/><ContractRows state={state} label="WEEKLY" quests={weekly} onClaim={claimContract}/><ContractRows state={state} label="MONTHLY" quests={monthly} onClaim={claimContract}/></Panel>}
  </ScrollView>
    <GameModalSurface visible={filterOpen} reduceMotion={state.settings.reduceMotion} onClose={()=>setFilterOpen(false)} backdropLabel="Close story filters" surfaceStyle={s.filterSheet}>
      <GameModalHeader eyebrow="ASTERFALL JOURNAL" title="Filter story chapters" onClose={()=>setFilterOpen(false)}/>
      <View style={s.filterList}>{JOURNAL_FILTER_OPTIONS.map(option=>{const count=option.id==='current'?storySummary.active+storySummary.ready:option.id==='all'?storySummary.total:option.id==='claimed'?storySummary.claimed:storySummary.locked;return <JournalFilterOption key={option.id} label={option.label+' · '+count} selected={filter===option.id} onPress={()=>{setFilter(option.id);setFilterOpen(false)}}/>})}</View>
      <GameButton title="Reset to Current" tone="secondary" onPress={()=>{setFilter('current');setQuery('');setFilterOpen(false)}}/>
    </GameModalSurface>
  </>;
}
function roman(act:QuestAct){return act===1?'I':act===2?'II':'III'}
function JournalStat({label,value,tone,stack=false}:{label:string;value:string;tone:'good'|'info'|'special'|'accent'|'muted';stack?:boolean}){const C=useGameTheme(),s=useMemo(()=>makeStyles(C),[C]),color=tone==='good'?C.good:tone==='info'?C.info:tone==='special'?C.special:tone==='accent'?C.accent:C.muted;return <View style={[s.journalStat,stack&&s.journalStatStack]}><Text style={s.journalStatLabel}>{label}</Text><Text style={[s.journalStatValue,{color}]}>{value}</Text></View>}
function JournalFilterOption({label,selected,onPress}:{label:string;selected:boolean;onPress:()=>void}){const C=useGameTheme(),s=useMemo(()=>makeStyles(C),[C]);return <Pressable accessibilityRole="radio" accessibilityState={{selected}} onPress={onPress} style={({pressed})=>[s.filterOption,selected&&s.filterOptionSelected,pressed&&s.pressed]}><Text style={[s.filterOptionText,selected&&s.filterOptionTextSelected]}>{label}</Text><Text style={[s.filterOptionMark,selected&&s.filterOptionMarkSelected]}>{selected?'✓':'›'}</Text></Pressable>}
function WeeklyOrderRows({state,orders,reduceMotion,onOpenOrder,onPinOrder,onQueueOrder,onStopOrder}:{state:GameState;orders:WeeklyOrder[];reduceMotion:boolean;onOpenOrder:(order:WeeklyOrder)=>void;onPinOrder:(order:WeeklyOrder)=>void;onQueueOrder:(order:WeeklyOrder)=>void;onStopOrder:(order:WeeklyOrder)=>void}){
 ...
 const ordered=[...orders].sort((a,b)=>Number(b.progress>=b.target)-Number(a.progress>=a.target)||(b.progress/Math.max(1,b.target))-(a.progress/Math.max(1,a.target))||a.slot-b.slot);
