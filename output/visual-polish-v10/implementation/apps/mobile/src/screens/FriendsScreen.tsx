...
export function FriendsScreen(){
  ...
  return <ScrollView contentContainerStyle={s.root} keyboardShouldPersistTaps="handled">
    ...
    <View style={s.sectionTabs}>{(['Friends','Requests','Find'] as const).map(value=><View key={value} style={s.flex}><GameButton title={value==='Requests'&&incoming.length?`Requests · ${incoming.length}`:value} selected={section===value} tone={section===value?'primary':'secondary'} onPress={()=>setSection(value)}/></View>)}</View>
