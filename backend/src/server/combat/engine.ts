...
export function simulateCombat(input: CombatInput): CombatResult {
  ...
  const applyDamage=(now:number, source:CombatantState, target:CombatantState, effect:AbilityEffect, abilityId:string, eventType:'damage'|'dot_tick'='damage')=>{
    ...
    if(target.hp<=0&&target.alive){gemOnKillV1(now,source,target);target.alive=false;target.downed=target.definition.team==='players';events.push({atMs:now,type:target.downed?'down':'death',targetId:target.definition.id,actorId:source.definition.id});}
  };
  ...
  const applyEffect=(now:number, source:CombatantState,target:CombatantState,effect:AbilityEffect,abilityId:string)=>{
    ...
    if(effect.kind==='buff'||effect.kind==='debuff'){target.modifiers.push({sourceId:source.definition.id,tag:effect.tag??'generic',value:effect.value??0,expiresAt:now+(effect.durationMs??5000),createdAt:now,kind:effect.kind});if(effect.kind==='debuff')gemOnDebuffAppliedV1(now,source,target);else gemOnBuffAppliedV1(now,source,target);return;}
  };
    ...
    for(const boss of living(enemies).filter(x=>x.definition.boss)){
      for(const phase of (boss.definition.phases||[]).sort((a,b)=>b.hpPct-a.hpPct)){
        if(hpPct(boss)<=phase.hpPct && !boss.triggeredPhases.includes(phase.id)){
          boss.triggeredPhases.push(phase.id); events.push({atMs:now,type:'phase',actorId:boss.definition.id,abilityId:phase.id,detail:`hp<=${phase.hpPct}`});gemOnBossPhaseV1(now,players);
