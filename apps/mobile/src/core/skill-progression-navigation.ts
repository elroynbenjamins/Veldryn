...
export function recipeProgressionSources(state:GameState,recipe:Recipe,inputs:ReadonlyArray<{itemId:string;quantity:number;inventory:number;bank:number}>):RecipeProgressionSource[]{
 ...
 const modeled=[...sorted].filter(row=>row.estimatedSeconds!==undefined&&!row.key.startsWith('prerequisite:')).sort((a,b)=>(b.estimatedSeconds??0)-(a.estimatedSeconds??0),bottleneck=modeled.length>1?modeled[0]?.key:undefined;
