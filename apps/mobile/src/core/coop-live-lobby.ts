...
export interface LiveReadyView {readyCheckId:string;rosterRevision:number;status:'open'|'refilling'|'committed'|'requeued'|'expired'|'cancelled';closesAtMs:number;refillEndsAtMs:number|null;serverNow:number;dungeonId?:string;tier?:number;members:Array<{characterId:string;role:'tank'|'damage'|'support';self:boolean;accepted:boolean}>;}
