    ...
    <View style={s.overlay}>
      ...
      <View style={s.sheet}>
        ...
        <ScrollView style={s.scroll} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
          ...
          {sockets&&<Section title="SOCKETS"><Text style={s.body}>{sockets.capacity?sockets.filled+'/'+sockets.capacity+' active typed sockets':'No active sockets at this rarity.'}</Text><Text style={s.bodyStrong}>{'STAT · '+(!sockets.statUnlocked?'Locked until Uncommon':sockets.statGemName??'Empty')}</Text><Text style={s.bodyStrong}>{'EFFECT · '+(!sockets.effectUnlocked?'Locked until Rare':sockets.effectGemName??'Empty')}</Text>{gearDecision?.gems.map(gem=><Text key={gem.id} style={s.body}>{(gem.kind==='effect'?'✦ ':'◆ ')+gem.name+' · '+gem.detail}</Text>)}{sockets.legacyGemNames.length>0&&<><Text style={s.warningText}>{'Legacy sockets · '+sockets.legacyGemNames.length+' retained'}</Text>{sockets.legacyGemNames.map((name,index)=><Text key={'legacy-'+index} style={s.note}>{'◇ '+name+' · extract from Enhance > Gems'}</Text>)}</>}</Section>}
