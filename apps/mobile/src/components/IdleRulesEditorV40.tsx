...
export function IdleRulesEditorV40({state,rules,activeRuleId,busy,onSave}:{state:GameState;rules:IdleRuleSet[];activeRuleId?:string;busy:boolean;onSave:(rules:IdleRuleSet[],activeId?:string)=>Promise<void>}){
 const C=useGameTheme(),s=useMemo(()=>makeStyles(C),[C]),character=state.character!;
 ...
 return <><Panel>
  ...
  {rules.length?rules.map(rule=>{const generated=rule.id.startsWith('goal-rule:');return <View key={rule.id} style={[s.rule,activeRuleId===rule.id&&s.active]}><View style={s.flex}><View style={s.ruleTitleRow}><Text style={s.ruleName}>{rule.name}</Text>{generated?<Text style={s.generated}>GOAL RULE</Text>:null}</View>{rule.conditions.map(condition=><Text key={condition.id} style={s.condition}>• {displayCondition(condition,state)}</Text>)}<Text style={s.safety}>Food safety {rule.stopIfOutOfFood?'ON':'OFF'} · Overflow safety {rule.stopIfRewardsWouldOverflow?'ON':'OFF'} · finish current cycle</Text></View><View style={s.ruleActions}><GameButton compact title={activeRuleId===rule.id?'Active':'Use'} tone={activeRuleId===rule.id?'primary':'secondary'} disabled={busy} onPress={()=>void onSave(rules,activeRuleId===rule.id?undefined:rule.id)}/><GameButton compact title="Delete" tone="danger" disabled={busy} onPress={()=>void remove(rule.id)}/></View></View>}):<Text style={s.empty}>No custom rule yet. Existing combat food safety still applies.</Text>}
  <View style={s.actions}><View style={s.flex}><GameButton compact title="Safe 1h" tone="secondary" disabled={busy||quickBlocked(1)} onPress={()=>void quick(1)}/></View><View style={s.flex}><GameButton compact title="Safe 4h" tone="secondary" disabled={busy||quickBlocked(4)} onPress={()=>void quick(4)}/></View></View>
 ...
 <GameModalSurface visible={open} reduceMotion={state.settings.reduceMotion} onClose={()=>setOpen(false)} backdropLabel="Cancel Idle Rule creation">
  ...
  <View style={s.footer}><View style={s.flex}><GameButton title="Cancel" tone="secondary" onPress={()=>{setOpen(false);resetDraft()}}/></View><View style={s.flex}><GameButton title="Save & Use Rule" disabled={busy||(!conditions.length&&!foodSafety&&!overflowSafety)} onPress={()=>void saveNew()}/></View></View>
 </GameModalSurface></>;
}
function makeStyles(C:ThemeColors){return StyleSheet.create({
 ...
 switchRow:{flexDirection:'row',alignItems:'center',gap:8,paddingVertical:7,borderTopWidth:1,borderTopColor:C.line},footer:{flexDirection:'row',gap:spacing.sm,paddingTop:spacing.sm,borderTopWidth:1,borderTopColor:C.line}
});}
