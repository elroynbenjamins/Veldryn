...
import {migrateToPerInstanceGear} from './gear-instances';
...
export function claimEquipmentCraft(state:GameState,jobId:string,nowMs:number,rarityRoll=Math.random()){
  const projected=migrateToPerInstanceGear(withProjectedQueue(state,nowMs)),queue=equipmentCraftingQueue(projected),job=queue.find(row=>row.id===jobId);
  ...
  let next=awardOwnerSkillXp(granted.state,job.ownerCharacterId,recipe);
