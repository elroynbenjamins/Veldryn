...
const fresh_start_v33_1 = require("../src/core/fresh-start-v33");
function ok(value, message) { if (!value)
    throw new Error(message); }
const state = (0, game_1.createCharacter)((0, game_1.newGame)(1000), 'IRONWARDEN', 'Fresh start', 'female');
...
ok((0, fresh_start_v33_1.isV33EquipmentPieceId)('T1P_001') && !(0, fresh_start_v33_1.isV33EquipmentPieceId)('STONEHEART_CHEST'), 'v33 ID boundary');
ok((0, fresh_start_v33_1.isKnownV33EquipmentPieceId)('T1P_001') && !(0, fresh_start_v33_1.isKnownV33EquipmentPieceId)('T9P_999'), 'known v33 IDs');
ok((0, fresh_start_v33_1.isLegacyEquipmentSetItemId)('STONEHEART_CHEST'), 'legacy set gear identified');
ok(!(0, fresh_start_v33_1.isLegacyEquipmentSetItemId)('WORN_BLADE'), 'starter gear preserved');
const migrated = (0, save_migrations_1.migrateSave)(raw);
...
const rosterRaw = JSON.parse(JSON.stringify(state));
...
const roster = (0, save_migrations_1.migrateSave)(rosterRaw);
...
malformed.inventory.stacks = [{ itemId: 'T9P_999', quantity: 1 }];
ok((0, fresh_start_v33_1.invalidV33EquipmentIds)(malformed).join(',') === 'T9P_999', 'invalid v33 save IDs detected');
let rejected = false;
try {
    (0, save_migrations_1.migrateSave)(malformed);
}
catch {
    rejected = true;
}
ok(rejected, 'invalid v33 save rejected');
