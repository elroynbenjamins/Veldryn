...
exports.SAMPLE_MINI_EVENT_CONTRACTS = exports.SAMPLE_WEEKLY_CONTRACTS = exports.DEFAULT_MINIMUM_PERSONAL_CONTRIBUTION_RATE = exports.BASE_POINTS_PER_EXPECTED_MINUTE = exports.PARTY_POINTS_PER_STANDARD_HOUR = exports.CONTRIBUTION_CHALLENGE_MULTIPLIER = void 0;
exports.scoreContribution = scoreContribution;
exports.createEmptyPartyContractProgress = createEmptyPartyContractProgress;
exports.expectedObjectiveMinutes = expectedObjectiveMinutes;
exports.objectivePointBudget = objectivePointBudget;
exports.buildPartyContractBudget = buildPartyContractBudget;
exports.normalizedPointsForCumulativeUnits = normalizedPointsForCumulativeUnits;
exports.normalizedContributionDelta = normalizedContributionDelta;
exports.recordVerifiedContribution = recordVerifiedContribution;
exports.evaluateContractCompletion = evaluateContractCompletion;
exports.utcWeekKey = utcWeekKey;
exports.isMiniEventActive = isMiniEventActive;
exports.rankPartyScores = rankPartyScores;
exports.CONTRIBUTION_CHALLENGE_MULTIPLIER = {
    routine: 1,
    demanding: 1.1,
    elite: 1.2,
    boss: 1.35,
};
exports.PARTY_POINTS_PER_STANDARD_HOUR = 1000;
function scoreContribution(input) {
    if (!Number.isFinite(input.units) || input.units <= 0)
        return 0;
    const seconds = Math.max(1, input.profile.expectedSecondsPerUnit) * input.units;
    return Math.max(1, Math.round((seconds / 3600) * exports.PARTY_POINTS_PER_STANDARD_HOUR * exports.CONTRIBUTION_CHALLENGE_MULTIPLIER[input.profile.challenge]));
}
exports.BASE_POINTS_PER_EXPECTED_MINUTE = 10;
exports.DEFAULT_MINIMUM_PERSONAL_CONTRIBUTION_RATE = 0.08;
...
function createEmptyPartyContractProgress() {
    return { objectiveUnits: {}, memberPoints: {}, processedContributionKeys: {} };
}
function expectedObjectiveMinutes(objective) {
    if (!Number.isFinite(objective.targetUnits) || objective.targetUnits <= 0)
        throw new Error('invalid_contract_target_units');
 ...
 */
function objectivePointBudget(objective) {
    const minutes = expectedObjectiveMinutes(objective);
    return Math.max(1, Math.round(minutes * exports.BASE_POINTS_PER_EXPECTED_MINUTE * DIFFICULTY_MULTIPLIER[objective.difficulty]));
}
function buildPartyContractBudget(definition) {
    ...
    if (definition.objectives.length === 0)
        throw new Error('party_contract_requires_objective');
    ...
    const personalRate = definition.minimumPersonalContributionRate ?? exports.DEFAULT_MINIMUM_PERSONAL_CONTRIBUTION_RATE;
    if (!Number.isFinite(personalRate) || personalRate < 0 || personalRate > 0.5)
        throw new Error('invalid_personal_contribution_rate');
    return {
...
function normalizedPointsForCumulativeUnits(objective, units) {
...
function normalizedContributionDelta(objective, previousUnits, submittedDeltaUnits) {
 ...
 */
function recordVerifiedContribution(budget, progress, participatingAccountIds, input) {
    if (!input.idempotencyKey || input.idempotencyKey.length < 8)
        throw new Error('invalid_contribution_idempotency_key');
    ...
    if (!participatingAccountIds.includes(input.accountId))
        throw new Error('contributor_not_party_participant');
    const objective = budget.objectives.find(candidate => candidate.id === input.objectiveId);
    if (!objective)
        throw new Error('party_contract_objective_not_found');
    const delta = normalizedContributionDelta(objective, progress.objectiveUnits[input.objectiveId] ?? 0, input.deltaUnits);
    const next = {
...
function evaluateContractCompletion(budget, progress, participatingAccountIds) {
    const objectivesComplete = budget.objectives.every(objective => (progress.objectiveUnits[objective.id] ?? 0) >= objective.targetUnits);
    const totalEarnedPoints = budget.objectives.reduce((sum, objective) => sum + normalizedPointsForCumulativeUnits(objective, progress.objectiveUnits[objective.id] ?? 0), 0);
    const eligibleAccountIds = participatingAccountIds.filter(accountId => (progress.memberPoints[accountId] ?? 0) >= budget.minimumPersonalPoints);
...
function isMiniEventActive(window, atMs) {
    return atMs >= window.startsAtMs && atMs < window.endsAtMs;
}
function rankPartyScores(scores) {
...
exports.SAMPLE_WEEKLY_CONTRACTS = [
    {
...
exports.SAMPLE_MINI_EVENT_CONTRACTS = [
    {
