...
function fullCompanionCombatResult(result:CompanionCombatResult):result is CombatResult{return Array.isArray(result.events)&&Array.isArray(result.enemies)&&Array.isArray(result.players)&&result.players.every(player=>'stats' in player.definition&&'name' in player.definition);}
export function companionBattlePlayback(result:CompanionCombatResult):CompanionBattlePlaybackSnapshot|undefined{
 if(!fullCompanionCombatResult(result))return undefined;
