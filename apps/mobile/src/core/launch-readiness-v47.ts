...
import {encounterUnlocked,regionTravelAvailability} from './world-navigation';
...
export function weeklyOrderCandidatesFromCurrentContent(state:GameState):WeeklyOrderCandidate[]{
 const level=state.character?.level??1,availableZones=WORLD_ZONES.filter(zone=>regionTravelAvailability(state,zone)==='available'),availableZoneNames=new Set(availableZones.map(zone=>zone.name)),availableZoneIds=new Set(availableZones.map(zone=>zone.id));
 const hunts=MONSTERS.filter(monster=>!monster.boss).map(monster=>{const available=encounterUnlocked(state,monster)&&availableZoneNames.has(monster.zone);return {id:`monster:${monster.id}`,kind:'hunt' as const,title:`Defeat ${monster.name}`,monsterId:monster.id,regionId:WORLD_ZONES.find(zone=>zone.name===monster.zone)?.id,activityId:`combat:${monster.id}`,source:{kind:'monster' as const,id:monster.id,label:monster.name,available,reason:available?undefined:level<monster.unlockLevel?`Requires level ${monster.unlockLevel}`:'Requires regional Exploration discovery'},estimatedPerHour:Math.max(1,Math.floor(3600/Math.max(1,monster.secondsPerKill))),available,priority:monster.level<=level?20+Math.abs(level-monster.level):80};});
 const bossHunts=MONSTERS.filter(monster=>monster.boss).map(monster=>{const storyCleared=state.defeatedBossIds.includes(monster.id),available=storyCleared&&level>=monster.unlockLevel&&availableZoneNames.has(monster.zone);return {id:`boss:${monster.id}`,kind:'hunt' as const,title:`Oathglass Bounty: ${monster.name}`,brief:'Defeat the Fallen Knight once during the UTC week. A rewarded rematch counts; the one-time story clear does not.',monsterId:monster.id,boss:true,regionId:WORLD_ZONES.find(zone=>zone.name===monster.zone)?.id,activityId:`boss:${monster.id}`,source:{kind:'monster' as const,id:monster.id,label:monster.name,available,reason:storyCleared?undefined:'Defeat this boss in the story first'},estimatedPerHour:1,available,priority:0,reward:{rewardRef:'weekly_order_boss_fallen_knight',label:'Oathglass Bounty Cache'}};});
