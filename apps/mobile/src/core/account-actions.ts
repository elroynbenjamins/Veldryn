...
function mergeSeenItems(state:GameState,ids:string[]):GameState{
...
function managementBlocker(slot:CharacterSlot){
 ...
 if(Object.values(slot.character.gearEnhancements??{}).some(value=>(value?.gemIds?.length??0)>0||Boolean(value?.statGemId)||Boolean(value?.effectGemId)))return 'Socketed gems are protected. Extract them before continuing.';
 return undefined;
}
...
export function switchAccountCharacter(state:GameState,id:string,now:number){
 ...
 const remaining=(state.otherCharacters??[]).filter(x=>x.character.id!==id),active=snapshot(state);
