...
import {GUILD_BANNERS,normalizeGuildBannerId,type GuildBannerId} from '../core/guild-customization';
import {guildBannerSourceByKey} from '../theme/guild-customization-assets';
...
export function GuildCrest({size=48,bannerId}:{size?:number;bannerId?:GuildBannerId|string}){const banner=GUILD_BANNERS.find(entry=>entry.id===normalizeGuildBannerId(bannerId))??GUILD_BANNERS[0],source=guildBannerSourceByKey.get(banner.assetKey);return <View accessibilityLabel={`${banner.name} guild crest`} style={[s.guildCrest,{width:size,height:size,borderColor:banner.secondary,backgroundColor:banner.primary}]}>{source?<Image accessible={false} source={source} resizeMode="cover" style={StyleSheet.absoluteFill}/>:<Text style={[s.guildEmblem,{fontSize:Math.max(18,Math.round(size*.48)),color:banner.secondary}]}>{banner.emblem}</Text>}</View>;}
const s=StyleSheet.create({avatar:{borderRadius:radii.md,backgroundColor:'#101b29',alignItems:'center',justifyContent:'center'},badge:{minHeight:28,flexDirection:'row',alignItems:'center',gap:5,paddingRight:8},roleIcon:{width:24,height:24},role:{...typography.caption,color:C.muted},guildCrest:{borderWidth:2,borderRadius:radii.md,alignItems:'center',justifyContent:'center'},guildEmblem:{fontWeight:'900'}});
