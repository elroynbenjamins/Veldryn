...
export type FirstSessionTutorialId='first_hunt'|'first_hunt_collect'|'first_skill'|'first_skill_collect'|'ironwood_scout'|'ironwood_scout_collect'|'ironwood_prepare'|'ironwood_hunt'|'gear_check'|'level_ten'|'core_loop_complete'|'claim_qst001'|'claim_qst002'|'claim_qst003'|'claim_qst004'|'claim_qst005';
...
const STEPS:readonly FirstSessionTutorialStep[]=[
 ...
 {id:'ironwood_prepare',questId:'QST_003',eyebrow:'BEFORE IRONWOOD',title:'Prepare for the wolves',body:'The road is mapped. Ironwood Wolves still require character Level 7, while Exploration remains its own world-discovery skill.',hint:'Hunt enemies you can safely fight and collect combat rewards to raise your character level. Combat also adds a small amount of Exploration XP.',actionLabel:'Show World',destination:'World',highlightPrimary:'World'},
...
export function firstSessionTutorialStep(state:FirstSessionTutorialHost,completed:readonly string[]=[]):FirstSessionTutorialStep|undefined{
 ...
 else if(questId==='QST_003'){const roadDiscovered=state.unlockedMonsterIds?.includes('FIELD_WISP')===true;id=!roadDiscovered?(state.activity?.kind==='exploration'&&state.activity.targetId==='SCOUT_GREENFIELDS'?'ironwood_scout_collect':'ironwood_scout'):state.character.level<7?'ironwood_prepare':'ironwood_hunt';}
