...
export function ProfileScenePreview({state,backgroundId}:{state:GameState;backgroundId:string}){
 ...
 return <View accessibilityLabel={`${character.name}'s profile preview with ${background?.name??base?.name}`} style={s.frame}>
  ...
  <View style={s.info}><PlayerStyledName name={character.name} nameStyle={effectivePlayerNameStyle(state)} reduceMotion={state.settings.reduceMotion} style={s.name}/><Text style={s.meta}>Lv. {character.level} · {className}</Text><Text style={s.title}>{character.profileTitle??'New Adventurer'}</Text>{(pet||petCollectible)&&<Text style={s.meta}>{pet?.name??petCollectible?.name}</Text>}</View>
 </View>;
}
