    ...
    <View style={s.overlay}>
      ...
      <View style={s.sheet}>
        ...
        <ScrollView style={s.scroll} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
          ...
          {sockets&&<Section title="SOCKETS"><Text style={s.body}>{sockets.capacity?sockets.filled+'/'+sockets.capacity+' filled':'No sockets at this rarity.'}</Text>{gearDecision?.gems.length?gearDecision.gems.map(gem=><Text key={gem.id} style={s.body}>{'◆ '+gem.name+' · +'+Math.round(gem.percent*100)+'% '+gem.stat}</Text>):sockets.gemNames.map((name,index)=><Text key={index} style={s.body}>{'◆ '+name}</Text>)}</Section>}
