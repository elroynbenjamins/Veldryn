...
exports.QUEST_RARITIES = void 0;
exports.seasonalQuestBoard = seasonalQuestBoard;
const classes_1 = require("../content/classes");
const rng_1 = require("./rng");
exports.QUEST_RARITIES = {
...
function periodKey(period, date = new Date()) {
    ...
    if (period === 'weekly') {
        ...
        return `${utc.getUTCFullYear()}-W${Math.ceil(((utc.getTime() - yearStart) / 86400000 + 1) / 7)}`;
    }
    return `${utc.getUTCFullYear()}-${utc.getUTCMonth() + 1}`;
}
function pick(list, seed, index) { return list[(0, rng_1.hash32)(`${seed}:${index}`) % list.length]; }
function rarityFor(period, key, index, classId) {
    const roll = (0, rng_1.hash32)(`${period}:${key}:${classId}:${index}:rarity`) % 100;
    if (period === 'daily')
        return roll < 55 ? 'common' : roll < 90 ? 'uncommon' : 'rare';
    if (period === 'weekly')
        return roll < 45 ? 'uncommon' : roll < 87 ? 'rare' : 'epic';
    return roll < 48 ? 'rare' : roll < 88 ? 'epic' : 'legendary';
}
function cacheFor(rarity) { return rarity === 'common' ? ['MOSS_FIBER', 4] : rarity === 'uncommon' ? ['COPPER_ORE', 5] : rarity === 'rare' ? ['ASTER_IRON_ORE', 4] : rarity === 'epic' ? ['OATHGLASS_SHARD', 3] : ['OATHGLASS_SHARD', 7]; }
function contractProgress(state, tag) {
    const character = state.character;
    if (tag === 'combat')
        return Math.floor(character.xp / 80);
    if (tag === 'gathering')
        return Math.floor(state.skills.reduce((sum, skill) => sum + skill.xp, 0) / 40);
    if (tag === 'crafting')
        return Object.values(character.equipment).filter(Boolean).length + (character.craftedNoviceItemIds ?? []).length;
    if (tag === 'equipment')
        return Object.values(character.equipment).filter(Boolean).length;
    if (tag === 'exploration')
        return state.unlockedMonsterIds.length;
    return state.defeatedBossIds.length;
}
function seasonalQuestBoard(state, period, date = new Date()) {
    const character = state.character;
    if (!character)
        return [];
    const classDef = classes_1.CLASSES.find(item => item.id === character.classId);
    ...
    return rows.map((row, index) => {
        const [name, description, base, tag] = row, rarity = rarityFor(period, key, index, character.classId), meta = exports.QUEST_RARITIES[rarity], required = Math.max(1, Math.round(base * volume)), progress = Math.min(required, contractProgress(state, tag)), [rewardItemId, baseQty] = cacheFor(rarity);
