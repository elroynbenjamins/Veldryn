...
function simulateCombat(input) {
    ...
    const maxMs = input.maxDurationMs ?? 180000;
    ...
    const applyDamage = (now, source, target, effect, abilityId, eventType = 'damage') => {
        ...
        if (hpPct(target) < (0, calculations_1.clamp)(effect.executeBelowHpPct ?? 0, 0, 1))
            raw *= 1 + (0, calculations_1.clamp)(effect.executeBonus ?? 0, 0, 1);
        ...
        for (const shield of target.reflectiveShields ?? []) {
            ...
            if (!used || !owner || !source.alive || source === target)
                continue;
        ...
        if (target.reflectiveShields)
            target.reflectiveShields = target.reflectiveShields.filter(s => s.remaining > 0);
    ...
    const applyEffect = (now, source, target, effect, abilityId) => {
        ...
        if (effect.kind === 'shield') {
            ...
            target.shield += amt;
            if (effect.shieldReflectPct && amt > 0)
                (target.reflectiveShields ?? (target.reflectiveShields = [])).push({ remaining: amt, rate: (0, calculations_1.clamp)(effect.shieldReflectPct, 0, .5), sourceId: source.definition.id });
