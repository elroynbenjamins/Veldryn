...
async function settleEquipmentCraftV33(repo, receipt) {
    ...
    if (await repo.hasMutationKey(receipt.idempotencyKey))
        return { duplicate: true, firstCraft: false, skinProgress: setSkinProgressV33(receipt.characterId, piece.setId, await repo.listCraftedPieceIds(receipt.characterId)) };
    ...
    const progress = setSkinProgressV33(receipt.characterId, piece.setId, await repo.listCraftedPieceIds(receipt.characterId));
    let skinUnlocked;
    ...
    return { duplicate: false, firstCraft, skinProgress: progress };
}
