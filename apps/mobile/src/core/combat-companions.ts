...
export function companionFullInvestmentSummary(def:CompanionDefinition){
  ...
  return {
    leveling,
    ascensionGold:ascension.reduce((sum,cost)=>sum+cost.gold,0),
    ascensionEssence:ascension.reduce((sum,cost)=>sum+cost.companionEssence,0),
    bondstones:ascension.reduce((sum,cost)=>sum+cost.bondstones,0),
  };
}
