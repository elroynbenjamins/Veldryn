...
import {mobileRawGemItemIdV1} from '../src/content/gems-v1';
...
const wretch=MONSTERS.find(row=>row.id==='LANTERN_WRETCH')!,wretchPace=combatBaselineProjection(wretch),rawEffect=wretch.drops.find(drop=>drop.itemId===mobileRawGemItemIdV1('effect_opening_strike',1))!,rawEffectExpected=dropExpectation(rawEffect.chance,rawEffect.min,rawEffect.max,wretchPace.killsPerHour);
ok(rawEffectExpected.averageFindSeconds>=3*3600&&rawEffectExpected.averageFindSeconds<=12*3600,'Rare build-defining unrefined Effect Gems should remain multi-hour chase drops');
ok(dropPaceBand(rawEffectExpected.averageFindSeconds).band==='chase','Build-defining unrefined Effect Gems should remain explicitly classified as chase rewards');
