...
import {COMPANION_HOUSING_MILESTONES,COMPANION_HOUSING_UPGRADES,companionHousingLevelCap} from '../src/core/companion-housing';

assert.equal(COMPANION_EXPEDITION_STAMINA,100);
assert.equal(companionFoodStamina('COOKED_SILVERFIN'),5.5);
assert.equal(companionFoodNeededForStamina('COOKED_SILVERFIN'),19);
assert.equal(companionFoodNeededForStamina('ASHLANDS_EMBER_STEW'),2);
assert.deepEqual(COMPANION_HOUSING_MILESTONES.map(x=>x.levelCap),[10,20,25,35]);
