...
legacy.account.unlockedCosmeticPetIds=['PET_001','PET_002'];legacy.character!.ownedPetIds=['PET_001','PET_001','PET_002'];
let mult=characterPermanentMultipliers(legacy);const pebble=PET_PERMANENT_BOOSTS.PET_001,cinder=PET_PERMANENT_BOOSTS.PET_002;
ok(!!pebble?.miningYieldMultiplier&&!!cinder?.miningYieldMultiplier,'canonical pet boost definitions exist');
ok(Math.abs(mult.miningYieldMultiplier-1.01)<1e-10,'two inactive owned pets contribute exactly +0.50% each without duplicate stacking');
legacy.character!.selectedCosmeticPetId='PET_001';mult=characterPermanentMultipliers(legacy);ok(Math.abs(mult.miningYieldMultiplier-1.03)<1e-10,'selected pet adds full +2.00% active bonus on top of owned passives');
legacy.character!.selectedCosmeticPetId='PET_004';mult=characterPermanentMultipliers(legacy);ok(Math.abs(mult.miningYieldMultiplier-1.01)<1e-10&&mult.incomingDamageMultiplier===1,'unowned selected pet cannot grant perk');
