...
ok(!skills.includes("skillId==='hunting'")&&!skills.includes("'hunting','exploration'"),'Retired Hunting must not reappear as a player skill.');
...
ok(activity.includes('preview.explorationXp')&&activity.includes('Exploration XP'),'Combat reward cards must surface passive Exploration XP separately from character XP');
...
ok(idleRulesEditor.includes('useWindowDimensions')&&idleRulesEditor.includes("stackActions=width<360||fontScale>=1.25"),'Advanced Idle Rules must adapt controls for narrow screens and larger text');
...
ok(event.includes('<AnnualEventCalendarPanel state={state}/>')&&event.includes('PAST FESTIVALS')&&event.includes('history.slice(0,showEventArchive?history.length:6)'),'Inactive Event Hub must remain a calendar plus capped festival history even while live event gameplay is disabled');
