...
function companionUnlockRequirementMet(state, requirement) {
    ...
    if (requirement.type === 'boss_kills')
        return (state.defeatedBossIds ?? []).filter(id => id === target).length >= amount;
...
function claimSanctuaryTraining(state, nowMs) {
    ...
    return { ...clean, account: { ...clean.account, combatCompanionProgress: progress, companionSanctuary: { ...sanctuary, lastTrainingClaimAtMs: last + days * day } } };
}
function claimSanctuaryEssence(state, nowMs) {
    ...
    if (periods < 1)
        return clean;
    return { ...clean, account: { ...clean.account, companionEssence: (clean.account.companionEssence ?? 0) + combat_companions_1.COMPANION_SANCTUARY_WEEKLY_ESSENCE[level] * periods, companionSanctuary: { ...sanctuary, lastEssenceClaimAtMs: last + periods * week } } };
}
