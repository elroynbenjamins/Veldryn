...
const oneShotRematch=simulateFallenKnightStoryBattle({name:'Overgeared',classId:'WAYFINDER',maxHp:5000,currentHp:5000,attack:800,defense:500,power:20000,accuracy:.99,evasion:.45,critChance:.75,critMultiplier:2.5,haste:.5,damageMultiplier:2,actionSpeedMultiplier:1.5,incomingDamageMultiplier:.5,foodHeal:0,foodQuantity:0,autoEatThresholdPct:40},'one-shot-rematch','rematch');
if(!oneShotRematch.won||oneShotRematch.events.filter(event=>event.type==='player_hit').length!==1)throw new Error('Overgeared weekly rematches must be allowed to one-shot the Fallen Knight');
if(oneShotRematch.events.some(event=>event.type==='phase'))throw new Error('One-shot rematches must not force story phase transitions');
