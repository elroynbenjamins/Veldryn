...
export function companionUnlockFacts(state:GameState):CompanionUnlockFacts {
  ...
  for(const [progressId,originId] of [['REG_002','REG_SUNSCAR'],['REG_003','REG_FROSTMARCH'],['REG_004','REG_ASHLANDS']] as const)if((state.regionalProgressById?.[progressId]?.storyCompleted??0)>=10)regionCompletion.add(originId);
