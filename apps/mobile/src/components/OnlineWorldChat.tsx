...
export function OnlineWorldChat({playerName,language,embedded=false}:{playerName:string;language:Language;embedded?:boolean}){
  const [open,setOpen]=useState(embedded),[channel,setChannel]=useState(0),[text,setText]=useState(''),[rows,setRows]=useState<WorldMessage[]>([]),[busy,setBusy]=useState(false);
