...
async function supportAccount(env,payload){
  ...
  const [index,authUser,characters,partyMembers,guildMembers,eventProgress,claims,bindings,cases,notes,alerts] = await Promise.all([
  ...
  return {account:index??{account_id:accountId},authUser,characters,partyMembers,parties,guildMembers,guilds,eventProgress,claims,bindings,cases:cases??[],notes:notes??[],alerts:alerts??[],warnings};
}
