...
export function QuestScreen({state,onClaim,onClaimContract,onNavigate,onOpenWeeklyOrder,onPinWeeklyOrder,onQueueWeeklyOrder,onStopWeeklyOrder}:{state:GameState;onClaim:(id:string)=>void;onClaimContract:(period:SeasonalPeriod,id:string)=>void;onNavigate:(destination:QuestDestination)=>void;onOpenWeeklyOrder:(order:WeeklyOrder)=>void;onPinWeeklyOrder:(order:WeeklyOrder)=>void;onQueueWeeklyOrder:(order:WeeklyOrder)=>void;onStopWeeklyOrder:(order:WeeklyOrder)=>void}){
  ...
  return <><ScrollView contentContainerStyle={s.root} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
    ...
    <View style={s.attentionRow}><JournalStat label="REWARDS" value={String(storySummary.ready)} tone={storySummary.ready?'good':'muted'}/><JournalStat label="ACTIVE" value={String(storySummary.active)} tone="info"/><JournalStat label="WEEKLY" value={boardComplete+'/'+contractBoard.orders.length} tone={boardComplete===contractBoard.orders.length&&contractBoard.orders.length?'good':'accent'}/><JournalStat label="CACHES" value={String(challengeReady)} tone={challengeReady?'special':'muted'}/></View>
...
function JournalStat({label,value,tone}:{label:string;value:string;tone:'good'|'info'|'special'|'accent'|'muted'}){const C=useGameTheme(),s=useMemo(()=>makeStyles(C),[C]),color=tone==='good'?C.good:tone==='info'?C.info:tone==='special'?C.special:tone==='accent'?C.accent:C.muted;return <View style={s.journalStat}><Text style={s.journalStatLabel}>{label}</Text><Text style={[s.journalStatValue,{color}]}>{value}</Text></View>}
