...
export function DungeonCombatStage({run,enemyLabel,boss=false}:{run:CoopRunView;enemyLabel?:string;boss?:boolean}){
 ...
 const toggleInspect=(key:string)=>{setInspectKey(current=>current===key?undefined:key);if(!complete&&!reduceMotion)setPaused(true);};
