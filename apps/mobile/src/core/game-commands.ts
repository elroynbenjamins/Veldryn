...
export function validateGameCommand(value:unknown):GameCommand{
 ...
 if(row.type==='start'){
  const start=args as Record<string,unknown>,kind=oneOf(start.kind,['combat','gathering']);
  if(start.challengeId!==undefined){oneOf(start.challengeId,COMBAT_CHALLENGE_IDS);if(kind!=='combat')throw new Error('invalid_challenge_activity');}
 }
