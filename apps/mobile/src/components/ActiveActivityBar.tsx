...
export function ActiveActivityBar({state,nowMs,onOpen}:{state:GameState;nowMs:number;onOpen:()=>void}){
 ...
 useEffect(()=>{
  ...
  if(!combat||state.settings.reduceMotion)return;
  const pause=Math.max(240,Math.round(cycleSeconds*650));
  const animation=Animated.loop(Animated.sequence([Animated.delay(pause),Animated.timing(hitEffect,{toValue:1,duration:110,easing:Easing.out(Easing.cubic),useNativeDriver:true}),Animated.timing(hitEffect,{toValue:0,duration:280,easing:Easing.in(Easing.quad),useNativeDriver:true})]));
...
function makeStyles(C:ThemeColors){const equipmentColors=equipmentTheme(C);return StyleSheet.create({root:{minHeight:64,flexDirection:'row',alignItems:'center',gap:10,paddingHorizontal:12,paddingVertical:8,backgroundColor:C.panel,borderBottomWidth:1},combat:{borderBottomColor:C.bad},skilling:{borderBottomColor:C.info},pressed:{opacity:.78},art:{width:42,height:42,alignItems:'center',justifyContent:'center',borderRadius:10,backgroundColor:C.stage},copy:{flex:1,minWidth:0,gap:4},line:{flexDirection:'row',alignItems:'baseline',gap:8},name:{...typography.bodyStrong,color:C.text,flex:1},time:{...typography.caption,color:C.muted,fontVariant:['tabular-nums']},meta:{flexDirection:'row',alignItems:'center',gap:8},kind:{fontSize:10,lineHeight:13,fontWeight:'900',letterSpacing:.8},combatText:{color:C.bad},skillText:{color:C.info},cycle:{fontSize:9,lineHeight:12,color:C.muted,fontWeight:'800'},combatStats:{position:'relative',flexDirection:'row',alignItems:'center',gap:8},hpText:{fontSize:10,color:C.warning,fontWeight:'800'},damageText:{fontSize:10,color:C.bad,fontWeight:'900'},takenText:{fontSize:10,color:C.good,fontWeight:'800',marginLeft:'auto'},hitSlash:{position:'absolute',left:'35%',top:5,width:46,height:3,borderRadius:3},hitSlashBright:{backgroundColor:'#fff1ad',shadowColor:'#ffd36e',shadowOpacity:.9,shadowRadius:4},hitSlashSoft:{backgroundColor:'#ffd36e',height:2,shadowColor:'#fff1ad',shadowOpacity:.65,shadowRadius:3},track:{height:6,overflow:'hidden',borderRadius:3,backgroundColor:C.panel2,position:'relative'},fill:{height:'100%',borderRadius:3},combatFill:{backgroundColor:C.bad},skillFill:{backgroundColor:C.info},chevron:{color:equipmentColors.goldSoft,fontSize:28,lineHeight:32}});}
