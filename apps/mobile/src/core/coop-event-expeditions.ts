...
export function presentEventExpeditionRun(projection:CoopEventRunServerProjection):CoopRunView{
  ...
  const run:CoopRunView={
    ...
    roleSlots:projection.team.map(member=>({memberId:member.memberId,role:member.role,name:member.displayName,echo:member.kind==='echo',classId:member.classId,bodyPresentation:member.bodyPresentation,companionId:member.companionId,currentHp:member.currentHp,maximumHp:member.maximumHp,ready:member.downed===undefined?true:!member.downed})),
    options,
