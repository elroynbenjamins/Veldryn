...
import {GameState,GearEnhancementState,GemStat,ItemStack} from './types';
...
const SOCKETS:Record<ItemRarity,number>={common:0,uncommon:1,rare:1,epic:2,legendary:2,mythic:3};
...
export function gemSocketCapacity(itemId:string){const item=itemDef(itemId);return item.type==='gear'?SOCKETS[itemRarity(item)]:0;}
...
export function socketGem(state:GameState,itemId:string,gemId:string){
  requireEquipped(state,itemId);const gem=itemDef(gemId);if(gem.type!=='gem'||!gem.gemStat)throw new Error('That item is not a gem');
  const enhancement=gearEnhancement(state,itemId),capacity=gemSocketCapacity(itemId);if(enhancement.gemIds.length>=capacity)throw new Error(capacity?'All sockets are filled':'This rarity has no gem sockets');
