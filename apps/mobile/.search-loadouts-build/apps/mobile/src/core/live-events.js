...
function eventLifecycle(state, nowMs = Date.now()) { const runtime = state.account.liveEvent; if (!runtime?.enabled)
    ...
    return null; const claimEndsAtMs = runtime.endsAtMs + definition.claimGraceDays * 86400000; if (nowMs < runtime.startsAtMs)
...
function eventDailyGift(state, nowMs = Date.now()) { const event = activeLiveEvent(state, nowMs); if (!event)
    return null; const startDay = Date.parse(new Date(event.runtime.startsAtMs).toISOString().slice(0, 10) + 'T00:00:00.000Z'), today = Date.parse(utcDayKey(nowMs) + 'T00:00:00.000Z'), eventDay = Math.max(1, Math.floor((today - startDay) / 86400000) + 1), giftDay = (eventDay - 1) % event.definition.dailyGifts.length + 1, gift = event.definition.dailyGifts.find(entry => entry.day === giftDay); const id = `${event.definition.id}:${utcDayKey(nowMs)}`; return { id, eventDay, giftDay, gift, claimed: (state.account.eventDailyGiftClaimIds ?? []).includes(id), nextAtMs: nextUtcDayMs(nowMs) }; }
...
function setLocalEventEnabled(state, enabled, nowMs = Date.now()) { const runtime = enabled ? { eventId: 'EVT_ANNUAL_009_2026', enabled: true, startsAtMs: nowMs - 60000, endsAtMs: nowMs + 21 * 86400000 } : undefined; return { ...state, account: { ...state.account, liveEvent: runtime } }; }
