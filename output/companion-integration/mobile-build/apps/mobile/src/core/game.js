...
exports.GATHER_TIME_SCALE = exports.OFFLINE_CAP_SECONDS = exports.MAX_OFFLINE_CAP_HOURS = exports.BASE_OFFLINE_CAP_HOURS = void 0;
...
function newGame(nowMs) {
    return {
        ...
        skills: ['mining', 'woodcutting', 'fishing', 'smithing', 'cooking'].map(skillId => ({ skillId: skillId, xp: 0, level: 1 })),
...
function startCombat(state, monsterId, nowMs) {
    ...
    return { ...state, activity: { kind: 'combat', targetId: monsterId, startedAtMs: nowMs, lastClaimAtMs: nowMs, environment: (0, world_weather_1.captureActivityEnvironment)(monsterId, nowMs) } };
}
...
function simulateCombat(state, monsterId, elapsed) {
    ...
    const speed = Math.max(COMBAT_SPEED_MIN, Math.min(COMBAT_SPEED_MAX, boostedPower / Math.max(1, expected))) * style.speedMultiplier * modifiers.combatSpeedMultiplier * companion.outputMultiplier;
    ...
    for (let i = 0; i < theoreticalKills; i++) {
        ...
        const damage = Math.max(1, Math.round((raw * .48 + m.level * .16) * style.damageTakenMultiplier * modifiers.incomingDamageMultiplier * companion.incomingDamageMultiplier));
        hp -= damage;
...
function previewActivityReward(state, nowMs) {
    ...
    if (state.activity.kind !== 'combat') {
        const g = skills_1.GATHERING.find(x => x.id === state.activity.targetId);
        if (!g)
        ...
        const reward = { xp: Math.floor(actions * g.xp * effect.xpMultiplier * multipliers.skillXpMultiplier), gold: 0, items: quantity ? [{ itemId: g.itemId, quantity }] : [], kills: actions, elapsedSeconds: elapsed };
    ...
    const reward = { xp: Math.floor(sim.kills * m.xp * effect.xpMultiplier * multipliers.characterXpMultiplier), gold: Math.floor(sim.kills * m.gold * effect.goldMultiplier * multipliers.goldMultiplier), items, kills: sim.kills, elapsedSeconds: elapsed, foodConsumed: sim.foodConsumed, endHp: sim.endHp, stoppedReason: sim.stoppedReason };
    return { ...reward, eventDrops: (0, live_events_1.activityEventDrops)(state, reward, nowMs), eventDiscoveries: (0, live_events_1.activityEventDiscoveries)(state, 'combat', reward.kills, nowMs) };
}
...
function claimActivity(state, nowMs) {
    ...
    if (state.activity.kind !== 'combat') {
        ...
        const next = { ...state, skills, ...routed, activity: { ...state.activity, lastClaimAtMs: nowMs } };
    ...
    const unlocked = monsters_1.MONSTERS.filter(m => !m.boss && m.unlockLevel <= level).map(m => m.id);
    ...
    return { state: (0, companion_runtime_1.recordCompanionActivity)(refreshQuests((0, live_events_1.applyEventDiscoveries)((0, live_events_1.applyEventDrops)(next, reward.eventDrops ?? []), reward.eventDiscoveries ?? []), state.activity.targetId, reward.kills), 'combat', state.activity.targetId, reward.kills, nowMs), reward };
}
function stopActivity(state) { return { ...state, activity: null }; }
...
function sellItem(state, itemId, quantity = 1) { if (!state.character || quantity <= 0)
    return state; const discovered = (0, character_skins_1.discoverCharacterSkins)(state), d = (0, items_1.itemDef)(itemId); if (d.type === 'gear' && (0, equipment_enhancement_1.hasEnhancement)(discovered, itemId))
...
function startGathering(state, targetId, nowMs) { const g = skills_1.GATHERING.find(x => x.id === targetId); if (!g)
