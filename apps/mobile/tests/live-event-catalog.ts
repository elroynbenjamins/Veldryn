...
equal(harvest!.visualKey,'harvestwake','Harvestwake should resolve through centralized visuals');
ok(harvest!.shop.some(offer=>offer.reward.kind==='companion'&&offer.reward.id==='EVT_UNIT_006'),'Harvestwake should expose its event companion through the generic reward path');
...
equal(generic.collectionTitle,'Example Event collection','Generic collection title derives from event name');
equal(generic.shopTitle,'EVENT SHOP','Generic event UI defaults to Shop wording');
equal(generic.projectNoun,'event project','Generic event UI does not assume a winter project');

console.log('PASS: live event catalog, companion reward metadata and generic copy validate');
